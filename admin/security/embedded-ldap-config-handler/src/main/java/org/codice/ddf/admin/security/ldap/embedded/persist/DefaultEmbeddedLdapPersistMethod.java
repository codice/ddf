...
import static org.codice.ddf.admin.security.ldap.commons.LdapConfiguration.CREDENTIAL_STORE;
import static org.codice.ddf.admin.security.ldap.commons.LdapConfiguration.LDAP_USE_CASES;
import static org.codice.ddf.admin.security.ldap.commons.LdapConfiguration.LOGIN;
import static org.codice.ddf.admin.security.ldap.commons.LdapConfiguration.LOGIN_AND_CREDENTIAL_STORE;
...
import org.codice.ddf.admin.security.ldap.embedded.EmbeddedLdapConfiguration;
...
public class DefaultEmbeddedLdapPersistMethod extends PersistMethod<EmbeddedLdapConfiguration> {
    ...
    public static final Map<String, String> REQUIRED_FIELDS = ImmutableMap.of(
            LDAP_USE_CASE, "Host name of the ldap url to attempt to connect to." + Arrays.toString(LDAP_USE_CASES.toArray()));


    public static final Map<String, String> SUCCESS_TYPES = ImmutableMap.of(
            SUCCESSFUL_PERSIST, "Successfully started and saved Embedded LDAP configurations.");

    public static final Map<String, String> FAILURE_TYPES = ImmutableMap.of(
            FAILED_STARTING_LDAP, "Failed to start Embedded LDAP or install a default configuration file.");
    ...
    public TestReport persist(EmbeddedLdapConfiguration configuration) {
        ...
        if(report.containsFailedResults()) {
            return new TestReport(new ConfigurationMessage(FAILURE, FAILED_STARTING_LDAP, FAILURE_TYPES.get(FAILED_STARTING_LDAP)));
        }

        return new TestReport(new ConfigurationMessage(SUCCESS, SUCCESSFUL_PERSIST, SUCCESS_TYPES.get(SUCCESSFUL_PERSIST)));
    }
}
