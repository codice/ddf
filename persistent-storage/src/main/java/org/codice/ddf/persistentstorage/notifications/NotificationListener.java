 ...
 * 
 **/
package org.codice.ddf.persistentstorage.notifications;
...
//        entryData.addIdProperty(UUID.randomUUID().toString().replaceAll("-", ""));
        entryData.addTextProperty("userid",  userId);
        entryData.addTextProperty("timestamp", timestamp);
        entryData.addTextProperty("application", application);
        entryData.addTextProperty("title",  title);
        entryData.addTextProperty("message", message);
        persistentStore.addEntry(PersistentStore.NOTIFICATION_TYPE, entryData);
    }
}
