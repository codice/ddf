...
public class TemporalResultComparator implements Comparator<Result> {
    ...
    /**
	 * Constructs the comparator with the specified sort order, either temporal
	 * ascending or temporal descending.
	 * 
	 * @param sortOrder
	 *            the temporal sort order, if null is passed in, then the
	 *            default SortOrder applies, which is SortOrder.DESCENDING.
	 */
     ...
     * -1 if A is non-null and B is null
     * 0 if both A and B are null
     * 1 if temporal ascending and A > B; -1 if temporal ascending and B > A
     * -1 if temporal descending and A > B; 1 if temporal descending and B > A
     */
    @Override
	public int compare( Result contentA, Result contentB ) {
			...
			if(effectiveDateA == null && effectiveDateB != null){
				...
				logger.debug( "effectiveDateA is not null: " + effectiveDateA + " and effectiveDateB is null" );
				return -1;
			}
			else if(effectiveDateA == null && effectiveDateB == null){
