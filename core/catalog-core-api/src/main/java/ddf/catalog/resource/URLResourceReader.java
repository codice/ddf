 ...
 * @deprecated - As of DDF v2.1.0.ALPHA1. URLResourceReader has been moved to /ddf/trunk/catalog/resource, resource-impl is now a separate bundle containing this ResourceReader implementation
 */
public class URLResourceReader implements ResourceReader {
    ...
    public URLResourceReader( MimeTypeMapper mimeTypeMapper ) 
    {
        ...
        if ( mimeTypeMapper == null ) logger.debug( "mimeTypeMapper is NULL" );
    ...
    /**
     * Retrieves a {@link Resource} based on a {@link URI} and provided
     * arguments. 
     * A connection is made to the {@link URI} to obtain the {@link Resource}'s
     * {@link InputStream} and build a {@link ResourceResponse} from that. 
     * If the {@link URI}'s scheme
     * is HTTP or HTTPS, the {@link Resource}'s name gets set to the {@link URI} passed in,
     * otherwise, if it is a file scheme, the name is set to the actual file
     * name.
     * 
    ...
    private ResourceResponse doRetrieveProduct(URI resourceURI, String productName) throws IOException,
	    ResourceNotFoundException 
	{
