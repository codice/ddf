...
public class CatalogFrameworkImplTest {
	 ...
	 */
	@Test
	public void testGetResource_WhenNonNullResourceRequest_ExpectPostResourcePluginToReceiveResourceResponseWithNonNullResourceRequest() throws Exception{
	    
	    //Setup
	    /*
	     * Prepare to capture the ResourceResponse argument passed into PostResourcePlugin.process().
	     * We will verify that it contains a non-null ResourceRequest in the verification section of this 
	     * test.
	     * 
	     */
	    ...
	    /*
	     * We verify (see verification section of test) that PostResourcePlugin.process() receives a ResourceResponse 
	     * with a non-null ResourceRequest. We assume that it works correctly and returns a ResourceResponse with 
	     * a non-null ResourceRequest, so we return our mockResouceResponse that contains a non-null ResourceRequest.
	     */
        ...
        CatalogFramework catalogFrameworkUnderTest = new CatalogFrameworkImpl(
            ...
            null) { 
                                                                               ...
                                                                               Map<String, Serializable> properties)
                     {
                         ...
                         LOGGER.debug("resourceResponse.getRequest() expected returned value: null;  actual returned value: " + resourceResponse.getRequest());
        ...
        ResourceResponse resourceResponse = catalogFrameworkUnderTest.getResource(mockResourceRequest, resourceSiteName);
        LOGGER.debug("resourceResponse: " + resourceResponse);
        
        //Verify
        /*
         * Verify that when PostResoucePlugin.process() is called, the ResourceResponse argument contains a non-null ResourceRequest.
         */
        ...
        /*
         * We really don't need to assert this since we return our mockResourceResponse from PostResourcePlugin.process()
         */
	...
	public void testGetSites() {
		...
		// Mock register the federated sources
	...
	public void testGetFederatedSourcesDuplicates() {
		...
		SourcePollerRunner runner = new SourcePollerRunner();
		SourcePoller poller = new SourcePoller(runner);
		for (FederatedSource source : expectedSources) {
			runner.bind(source);
		}
	...
	/**
	 * Tests that you can get a resource's (product) options. Covers the
	 * case where the source ID specified is actually the local catalog provider's site name 
	 * (so this reduces down to a getResourceOptions for local provider); and the case
	 * where a federated source is specified.
	 * 
	 * Test for DDF-1763.
	 * 
	 * @throws Exception
    ...
    public void testGetResourceOptions() throws Exception
    {
        ...
        CatalogFrameworkImpl framework = new CatalogFrameworkImpl(
                ...
                new ArrayList<PostResourcePlugin>(),
                new ArrayList<ConnectedSource>(), federatedSources,
                resourceReaders, strategy, null, poller);
        framework.bind( provider );
        framework.setId("ddf");
    ...
    /**
     * Tests that multiple ResourceReaders with the same scheme will be invoked if
     * the first one did not return a Response.
     * 
     * @throws Exception
     */
	@Test
    public void testGetResourceToTestSecondResourceReaderWithSameSchemeGetsCalledIfFirstDoesNotReturnAnything() throws Exception
    {
        ...
        CatalogFrameworkImpl framework = new CatalogFrameworkImpl(
                ...
                new ArrayList<PreResourcePlugin>(),
                new ArrayList<PostResourcePlugin>(),
                new ArrayList<ConnectedSource>(), null,
                resourceReaders, strategy, null, poller);
        framework.bind( provider );
        framework.setId(DDF);
        ...
        // This line is equivalent to verify(resourceReader1, times(1)).retrieveResource(any(URI.class), anyMap());
