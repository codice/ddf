...
{   
    private static final transient Logger LOGGER = Logger.getLogger(ContentDirectoryMonitorTest.class);
    ...
    public void testRouteCreationWithCopyIngestedFiles() throws Exception
    {
        ...
        RouteDefinition routeDefinition = createRoute(monitoredDirectory, directive, copyIngestedFiles);
    ...
    public void testMoveFolder() throws Exception
    {
        ...
        template.sendBodyAndHeader("file://" + monitoredDirectory, fileContents, Exchange.FILE_NAME, INPUT_FILENAME);
    ...
    /**
     * Verify if route has a failure then the file being processed is moved
     * to the .errors directory.
     * 
     * @throws Exception
     */
    /*TODO: how to fail the route
    @Test
    public void testMoveFailedFolder() throws Exception
    {
        String monitoredDirectory = "target/inbox";
        String directive = "PROCESS";
        boolean copyIngestedFiles = true;
        ...
        when(contentFramework.create(isA(CreateRequest.class), isA(Request.Directive.class))).thenReturn(createResponse);
        ...
        // Verify the file moved to the .errors directory
        File target = new File(monitoredDirectory + "/.errors/" + INPUT_FILENAME);
        assertTrue("File not moved to .errors folder", target.exists());
    ...
    public void testUpdateExistingDirectoryMonitor() throws Exception
    {
        ...
        template.sendBodyAndHeader("file://" + newMonitoredDirectory, fileContents, Exchange.FILE_NAME, "input_2.txt");
        ...
        template.sendBodyAndHeader("file://" + monitoredDirectory, fileContents, Exchange.FILE_NAME, "input_3.txt");
    ...
    public void testMultipleDirectoryMonitors() throws Exception
    {
        ...
        RouteDefinition firstRouteDefinition = createRoute(firstMonitoredDirectory, directive, copyIngestedFiles);
        
        String secondMonitoredDirectory = "target/inbox_2";
        directive = "STORE";
        copyIngestedFiles = true;
        
        RouteDefinition secondRouteDefinition = createRoute(secondMonitoredDirectory, directive, copyIngestedFiles);
        
        // Put file in first monitored directory
        String fileContents = "text file 1";
        File file = writeTextFile(INPUT_FILEPATH, fileContents);
        template.sendBodyAndHeader("file://" + firstMonitoredDirectory, fileContents, Exchange.FILE_NAME, INPUT_FILENAME);
        
        fileContents = "text file 2";
        file = writeTextFile("target/input_2.txt", fileContents);
        template.sendBodyAndHeader("file://" + secondMonitoredDirectory, fileContents, Exchange.FILE_NAME, "input_2.txt");
...
/************************************************************************************/
        ...
        boolean copyIngestedFiles) throws Exception
    {
        ...
        // Did not work because it expects the route to be "adviced" already exists. So the above init()
        // call created the initial route with the "content:framework" node in it, and then this 
        // AdviceWithRouteBuilder replaced the content:framework with mock:result, but created a second
...
//        camelContext.addComponent(ContentComponent.NAME, contentComponent);
        ...
        String directive, boolean copyIngestedFiles)
    {
        ...
        // expect 4 outputs: SetHeader(operation), SetHeader(directive), SetHeader(contentUri), To(content:framework)
