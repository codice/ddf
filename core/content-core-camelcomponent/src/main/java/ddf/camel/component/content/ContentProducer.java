...
public class ContentProducer extends DefaultProducer
{
    ...
    /**
     * Constructs the {@link Producer} for the custom Camel ContentComponent.
     * This producer would map to a Camel <to> route node with a URI like
     * <code>content:framework</code>
     * 
     * @param endpoint
    ...
    public void process(Exchange exchange) throws ContentComponentException, ContentFrameworkException
    {
        ...
        {
            throw new ContentComponentException("Unable to process file " + ingestedFile.getName() + "  -  Must specify an operation of create, read, update, or delete");
        }

        String directive = in.getHeader(Request.DIRECTIVE, String.class);
        if (StringUtils.isEmpty(directive))
        {
            throw new ContentComponentException("Unable to process file " + ingestedFile.getName() + "  -  Must specify a directive of STORE, PROCESS, or STORE_PROCESS");
        }
        ...
        {            
            ...
            if (!StringUtils.isEmpty(mimeType))
            {
                ...
                CreateResponse createResponse = endpoint.getComponent().getContentFramework().create( createRequest, requestDirective );
            ...
            else
            {
                ...
                throw new ContentComponentException("Unable to determine mime type for the file " + ingestedFile.getName());
            }
        }
        finally
        {
