...
/**
 * Consumer for the custom Camel CatalogComponent. This {@link org.apache.camel.Consumer} would map to a Camel <from>
...
public class TransformerConsumer extends DefaultConsumer {
	...
	/**
	 * Constructs the consumer for the custom Camel CatalogComponent. This {@link Consumer} would map to a Camel <from>
	 * route node with a URI like <code>catalog:queryresponsetransformer</code>
	 * 
	 * @param endpoint
	 ...
	 * @param processor
	 */
	public TransformerConsumer(Class<?> transformerClass, CatalogEndpoint endpoint, Processor processor) {
	...
	protected void doStart() throws CatalogTransformerException {
		...
		// (This is how the CatalogContentPlugin will be able to look up this transformer by mimetype)
	...
	protected <T> T transform(Class<T> objClass, Object upstreamResponse, Map<String, Serializable> arguments)
		...
		try {
			...
			// Result should be a BinaryContent - getBody(BinaryContent) will return null if it isn't
