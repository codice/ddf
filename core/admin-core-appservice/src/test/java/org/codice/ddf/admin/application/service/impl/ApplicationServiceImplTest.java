...
public class ApplicationServiceImplTest {
     ...
     *       {@code beforeClass()} method because they are modified by  
     *       individual tests as part of the setup for individual test
    ...
    public void testGetApplications() throws Exception {
        Set<Repository> activeRepos = new HashSet<Repository>(Arrays.asList(
                noMainFeatureRepo1, noMainFeatureRepo2));
        ApplicationService appService = new ApplicationServiceImpl(
                createMockFeaturesService(activeRepos, null, null), 
     ...
     *    <li>The bundle state and extended bundle state of each bundle
     *        specified in each dependency feature is {@link Bundle#ACTIVE} and
    ...
    public void testGetApplicationStatusReturnsActiveStatusWhenAllFeaturesInstalledAndAllBundlesActive()
        throws Exception {
        FeaturesService featuresService = createMockFeaturesService(
                mainFeatureRepo, null, null);

        ApplicationService appService = new ApplicationServiceImpl(
                featuresService, bundleContext, bundleStateServices);
    ...
    public void testGetApplicationStatusReturnsInactiveWhenMainFeatureNotInstalledAndAllBundlesActive()
        ...
        assertEquals("More than one application was returned from mainFeatureRepo", 
                1, appService.getApplications().size());
    ...
    public void testGetApplicationStatusReturnsInactiveWhenBundleStateIsResolved() throws Exception {
        ApplicationService appService = getAppServiceWithBundleInGivenState(
                mainFeatureRepo, Bundle.RESOLVED);
    ...
    public void testGetApplicationStatusReturnsInactiveWhenBundleStateIsStarting() 
        ...
        ApplicationService appService = getAppServiceWithBundleInGivenState(
                mainFeatureRepo, Bundle.STARTING);
    ...
    public void testGetApplicationStatusReturnsInactiveWhenBundleStateIsStopping() throws Exception {
        ApplicationService appService = getAppServiceWithBundleInGivenState(
                mainFeatureRepo, Bundle.STOPPING);
     ...
     *    <li>Main feature is installed</li>
     *    <li>All dependency features are installed</li>
     *    <li>One dependency feature contains a Bundle that is in an inactive
     *        state</li>
     * </ul>
     * 
     * @throws Exception
    ...
    public void testGetApplicationStatusReturnsFailedStatusWhenFeatureDependencyContainsInactiveBundle()
        ...
        FeaturesService featuresService = createMockFeaturesService(
                mainFeatureRepo, null, inactiveBundleNames);
    ...
    public void testGetApplicationStatusReturnsInactiveWhenBundleStateIsInstalled()
        throws Exception {
        ApplicationService appService = getAppServiceWithBundleInGivenState(
                mainFeatureRepo, Bundle.INSTALLED);
    ...
    public void testGetApplicationStatusReturnsInactiveWhenBundleStateIsUninnstalled()
        throws Exception {
        ApplicationService appService = getAppServiceWithBundleInGivenState(
                mainFeatureRepo, Bundle.UNINSTALLED);
     ...
     * {@link BundleState#Active} state. This simulates a condition wherein
     * the {@code getApplicationStatus} method must discern between conditions
     * that, independent of each other, would produce 
    ...
    public void testGetApplicationStatusReturnsFailedWhenBundleStateServiceStatesIncludeActiveResolvedAndFailure()
        ...
        when(bundleStateServices.get(0).getState(bundles[0])).thenReturn(
                BundleState.Resolved);
        when(bundleStateServices.get(0).getState(bundles[1])).thenReturn(
                BundleState.Failure);
     ...
     */
    @Test
    public void testIsApplicationStartedReturnsTrueForActiveApplicationState() 
        throws Exception {
        
        ApplicationService appService = new ApplicationServiceImpl(
                createMockFeaturesService(mainFeatureRepo, null, null), 
                bundleContext, bundleStateServices);
     ...
     */
    @Test
    public void testIsApplicationStartedReturnsFalseForInactiveApplicationState() 
        throws Exception {
     ...
     */
    @Test
    public void testIsApplicationStartedReturnsFalseForFailedApplicationState() 
        throws Exception {
        ...
        ApplicationService appService = new ApplicationServiceImpl(
                createMockFeaturesService(mainFeatureRepo, null, inactiveBundles), 
                bundleContext, bundleStateServices);
    ...
    public void testGetActiveApplicationStatus() throws Exception {
        Set<Repository> activeRepos = new HashSet<Repository>(
                Arrays.asList(noMainFeatureRepo1));
        ApplicationService appService = new ApplicationServiceImpl(
                createMockFeaturesService(activeRepos, null, null), 
                bundleContext, bundleStateServices);
    ...
    public void testIgnoreApplications() throws Exception {
        ...
        assertNotNull(applications);
        assertEquals(1, applications.size());
        assertEquals(noMainFeatureRepo2.getName(), 
                applications.iterator().next().getName());
    ...
    public void testApplicationTree() throws Exception {
        ...
        assertEquals("main-feature2", mainAppNode.getChildren().iterator()
                .next().getApplication().getName());
     ...
     * received bundle state and the rest of the bundles in an 
     * {code Bundle#ACTIVE} state.
     * 
     * @param repository The {@link Repository} from which to build the 
     *                   {@code ApplicationService}.
     * 
     * @param bundleState The state, as defined in the {@link Bundle} interface,
     *                    to set for one of the {@code Bundle}s in the 
     *                    {@code ApplicationService} 
     *                    
    ...
    private ApplicationService getAppServiceWithBundleInGivenState(
        ...
        if (null != bundle) {
            ...
            appService = new ApplicationServiceImpl(featuresService, 
                    bundleContext, bundleStateServices);
        }
     ...
     *                   {@code ApplicationService}.
     * 
     * @param bundleState The {@link BundleState} to set for one of the 
     ...
     * 
     * @param featuresService The {@link FeaturesService} from which to obtain
     *                        a {@code Bundle}
     *                        
    ...
    private FeaturesService createMockFeaturesService(Repository repo,
            Set<String> notInstalledFeatureNames, 
            Set<String> inactiveBundleLocations)
        throws Exception {
     ...
     *              
     * @param notInstalledFeatures A {@code Set} of {@code Feature}s that 
     *                             the {@code FeaturesService} should report as
     *                             not installed
     *                             
     * @param inactiveBundles A {@code Set} of {@link BundleInfo}s containing
     *                        the locations of {@code Bundle}s that should
     *                        be set to inactive and for which the 
     *                        {@link BundleStateService} contained in index 0
     *                        of {@link #bundleStateServices} should report a
    ...
    private FeaturesService createMockFeaturesService(Set<Repository> repos,
            Set<Feature> notInstalledFeatures, Set<BundleInfo> inactiveBundles) 
        throws Exception {
                ...
                for (Feature feature : repo.getFeatures()) {
                    ...
                    for (Feature depFeature : feature.getDependencies()) {
                        ...
                        logger.trace("Dependency Feature name/version: " + 
                                depFeature.getName() + "/" + depFeature.getVersion());
                    }
                }
            }
        }
        ...
        for (Repository curRepo : repos) {
            for (Feature curFeature : curRepo.getFeatures()) {
                ...
                // TODO: File Karaf bug that necessitates this, then reference it here.
        ...
        when(featuresService.listFeatures()).thenReturn(
                featuresSet.toArray(new Feature[] {}));
