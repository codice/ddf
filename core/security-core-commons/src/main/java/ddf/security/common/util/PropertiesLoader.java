...
/**
 * Utility class that attempts several different methods for loading in properties files from the classpath
 * or file system.
 * @author tustisos
 *
 */
...
{
    private static XLogger logger = new XLogger(LoggerFactory.getLogger(PropertiesLoader.class));
    ...
    public static Properties loadProperties(String propertiesFile, ClassLoader classLoader)
    {
        ...
        if(propertiesFile != null)
        {
            try
            {
                ...
                {
                    logger.debug("Attempting to load properties from "+ propertiesFile +" with Spring PropertiesLoaderUtils.");
                }
            ...
            if(error || properties.isEmpty())
            {
                if(classLoader != null)
                {
                    try
                    {
                        if(logger.isDebugEnabled())
                        {
                            logger.debug("Attempting to load properties from "+ propertiesFile +" with Spring PropertiesLoaderUtils with class loader.");
                        }
                ...
                else
                {
                    try
                    {
                        ...
                        properties = PropertiesLoaderUtils.loadAllProperties(propertiesFile, PropertiesLoader.class.getClassLoader());
                        error = false;
                    }
                    catch (IOException e)
                    {
