...
public class GeotoolsFilterAdapterImpl implements FilterAdapter, FilterVisitor,
		ExpressionVisitor {
	...
	public Object visit(Divide expression, Object delegate) {
		throw new UnsupportedOperationException(
				Divide.NAME + " expression not supported.");
	}
	...
	public Object visit(Multiply expression, Object delegate) {
		throw new UnsupportedOperationException(
				Multiply.NAME + " expression not supported.");
	}
	...
	public Object visit(Subtract expression, Object delegate) {
		throw new UnsupportedOperationException(
				Subtract.NAME + " expresssion not supported.");
	}

	public Object visitNullFilter(Object delegate) {
		throw new UnsupportedOperationException(
				"Null filter not supported by Filter Adapter.");
	}
	...
	public Object visit(And filter, Object delegate) {
		...
		throw new UnsupportedOperationException(
				"No valid operands for And filter.");
	}
	...
	public Object visit(Not filter, Object delegate) {
		...
		return ((FilterDelegate<Object>) delegate).not(child.accept(this,
				delegate));
	}
	...
	public Object visit(PropertyIsBetween filter, Object delegate) {
		...
		if (filter.getExpression() instanceof PropertyName
			...
			propertyName = (String) filter.getExpression().accept(this,
					delegate);
	...
	public Object visit(PropertyIsEqualTo filter, Object delegate) {
		...
		if (literal instanceof String) {
			return ((FilterDelegate<?>) delegate).propertyIsEqualTo(
					(String) propertyName, (String) literal,
					filter.isMatchingCase());
		} else if (literal instanceof Date) {
			return ((FilterDelegate<?>) delegate).propertyIsEqualTo(
					(String) propertyName, (Date) literal);
		} else if (literal instanceof Instant) {
			return ((FilterDelegate<?>) delegate).propertyIsEqualTo(
					propertyName, ((Instant) literal).getPosition().getDate());
		} else if (literal instanceof Period) {
			return ((FilterDelegate<?>) delegate).propertyIsEqualTo(
					propertyName, ((Period) literal).getBeginning()
							.getPosition().getDate(), ((Period) literal)
							.getEnding().getPosition().getDate());
		} else if (literal instanceof Integer) {
			return ((FilterDelegate<?>) delegate).propertyIsEqualTo(
					(String) propertyName, ((Integer) literal).intValue());
		} else if (literal instanceof Short) {
			return ((FilterDelegate<?>) delegate).propertyIsEqualTo(
					(String) propertyName, ((Short) literal).shortValue());
		} else if (literal instanceof Long) {
			return ((FilterDelegate<?>) delegate).propertyIsEqualTo(
					(String) propertyName, ((Long) literal).longValue());
		} else if (literal instanceof Float) {
			return ((FilterDelegate<?>) delegate).propertyIsEqualTo(
					(String) propertyName, ((Float) literal).floatValue());
		} else if (literal instanceof Double) {
			return ((FilterDelegate<?>) delegate).propertyIsEqualTo(
					(String) propertyName, ((Double) literal).doubleValue());
		} else if (literal instanceof Boolean) {
			return ((FilterDelegate<?>) delegate).propertyIsEqualTo(
					(String) propertyName, ((Boolean) literal).booleanValue());
		} else if (literal instanceof byte[]) {
			return ((FilterDelegate<?>) delegate).propertyIsEqualTo(
					(String) propertyName, (byte[]) literal);
		} else {
			return ((FilterDelegate<?>) delegate).propertyIsEqualTo(
					(String) propertyName, literal);
		}
	...
	public Object visit(PropertyIsNotEqualTo filter, Object delegate) {
		...
		if (literal instanceof String) {
			return ((FilterDelegate<?>) delegate).propertyIsNotEqualTo(
					(String) propertyName, (String) literal,
					filter.isMatchingCase());
		} else if (literal instanceof Date) {
			return ((FilterDelegate<?>) delegate).propertyIsNotEqualTo(
	...
	public Object visit(PropertyIsGreaterThan filter, Object delegate) {
		...
		if (filter.getExpression1() instanceof Literal) {
			...
			Filter lessThan = FF.less(FF.property(propertyName),
					FF.literal(literal));
			return lessThan.accept(this, delegate);
		}

		if (literal instanceof String) {
			return ((FilterDelegate<?>) delegate).propertyIsGreaterThan(
					propertyName, (String) literal);
		} else if (literal instanceof Date) {
			return ((FilterDelegate<?>) delegate).propertyIsGreaterThan(
					propertyName, (Date) literal);
		} else if (literal instanceof Integer) {
			return ((FilterDelegate<?>) delegate).propertyIsGreaterThan(
					propertyName, ((Integer) literal).intValue());
		} else if (literal instanceof Short) {
			return ((FilterDelegate<?>) delegate).propertyIsGreaterThan(
					propertyName, ((Short) literal).shortValue());
		} else if (literal instanceof Long) {
			return ((FilterDelegate<?>) delegate).propertyIsGreaterThan(
					propertyName, ((Long) literal).longValue());
		} else if (literal instanceof Float) {
			return ((FilterDelegate<?>) delegate).propertyIsGreaterThan(
					propertyName, ((Float) literal).floatValue());
		} else if (literal instanceof Double) {
			return ((FilterDelegate<?>) delegate).propertyIsGreaterThan(
					propertyName, ((Double) literal).doubleValue());
		} else if (literal instanceof Boolean) {
			return ((FilterDelegate<?>) delegate).propertyIsGreaterThan(
					propertyName, ((Boolean) literal).booleanValue());
		} else if (literal instanceof byte[]) {
			return ((FilterDelegate<?>) delegate).propertyIsGreaterThan(
					propertyName, (byte[]) literal);
		} else {
			return ((FilterDelegate<?>) delegate).propertyIsGreaterThan(
	...
	public Object visit(PropertyIsGreaterThanOrEqualTo filter, Object delegate) {
		...
		if (filter.getExpression1() instanceof Literal) {
			// convert literal >= property to property <= literal
			Filter lessThanOrEqual = FF.lessOrEqual(FF.property(propertyName),
					FF.literal(literal));
	...
	public Object visit(PropertyIsLessThan filter, Object delegate) {
		...
		if (filter.getExpression1() instanceof Literal) {
			// convert literal < property to property > literal
			Filter greaterThan = FF.greater(FF.property(propertyName),
					FF.literal(literal));
			return greaterThan.accept(this, delegate);
		}

		if (literal instanceof String) {
			return ((FilterDelegate<?>) delegate).propertyIsLessThan(
					propertyName, (String) literal);
		} else if (literal instanceof Date) {
			return ((FilterDelegate<?>) delegate).propertyIsLessThan(
					propertyName, (Date) literal);
	...
	public Object visit(PropertyIsLessThanOrEqualTo filter, Object delegate) {
		...
		if (filter.getExpression1() instanceof Literal) {
			// convert literal <= property to property >= literal
			Filter greaterThanOrEqual = FF.greaterOrEqual(
					FF.property(propertyName), FF.literal(literal));
	...
	public Object visit(PropertyIsLike filter, Object delegate) {
		...
		String pattern = normalizePattern(filter.getLiteral(), wildcard,
		...
		if (filter.getExpression() instanceof FuzzyFunction) {
			...
			propertyName = ((PropertyName) (fuzzy.getParameters().get(0)))
					.getPropertyName();
			isFuzzy = true;
		...
		boolean isXpathSearch = (propertyName.indexOf('/') != -1 || propertyName
				.indexOf('@') != -1);

		if (!isFuzzy && !isXpathSearch) {
			return ((FilterDelegate<?>) delegate).propertyIsLike(propertyName,
					pattern, matchCase);
		} else if (isFuzzy && !isXpathSearch) {
			// TODO check if wildcards are escaped
			return ((FilterDelegate<?>) delegate).propertyIsFuzzy(propertyName,
					pattern);
		} else if (!isFuzzy && isXpathSearch) {
			if (pattern.trim().isEmpty()
					|| pattern.trim().equals(FilterDelegate.WILDCARD_CHAR)) {
				return ((FilterDelegate<?>) delegate).xpathExists(propertyName);
			} else {
				return ((FilterDelegate<?>) delegate).xpathIsLike(propertyName,
						pattern, matchCase);
			}
		} else if (isFuzzy && isXpathSearch) {
			// TODO check if wildcards are escaped
			return ((FilterDelegate<?>) delegate).xpathIsFuzzy(propertyName,
					pattern);
		} else {
			throw new UnsupportedOperationException(
					"Unsupported operands for PropertyIsLike.");
		}
	}

	private String normalizePattern(String pattern, String wildcard,
			String singleChar, String escapeChar) {
	...
	public Object visit(PropertyIsNull filter, Object delegate) {
		if (filter.getExpression() instanceof PropertyName) {
			String propertyName = (String) filter.getExpression().accept(this,
					delegate);
	...
	public Object visit(Beyond filter, Object delegate) {
		double distance = normalizeDistance(filter.getDistance(),
		...
		if (distance != 0) {
    		return ((FilterDelegate<?>) delegate).beyond(filterValues.propertyName,
    				wkt, distance);
		} else {
	...
	public Object visit(Contains filter, Object delegate) {
		...
		return ((FilterDelegate<?>) delegate).contains(
				filterValues.propertyName, wkt);
	}
	...
	public Object visit(DWithin filter, Object delegate) {
		...
		return ((FilterDelegate<?>) delegate).dwithin(
				filterValues.propertyName, wkt, distance);
	}
	...
	public Object visit(Intersects filter, Object delegate) {
		...
		return ((FilterDelegate<?>) delegate).intersects(
				filterValues.propertyName, wkt);
	}
	...
	public Object visit(Within filter, Object delegate) {
		...
		return ((FilterDelegate<?>) delegate).within(filterValues.propertyName,
				wkt);
	}
	...
	public Object visit(Crosses filter, Object delegate) {
		...
		return ((FilterDelegate<?>) delegate).crosses(
				filterValues.propertyName, wkt);
	}
	...
	public Object visit(Disjoint filter, Object delegate) {
		...
		return ((FilterDelegate<?>) delegate).disjoint(
				filterValues.propertyName, wkt);
	}
	...
	public Object visit(Overlaps filter, Object delegate) {
		...
		return ((FilterDelegate<?>) delegate).overlaps(
				filterValues.propertyName, wkt);
	}
	...
	public Object visit(Touches filter, Object delegate) {
		...
		return ((FilterDelegate<?>) delegate).touches(
				filterValues.propertyName, wkt);
	}
	...
	private String geometryToWkt(Object literal) {
		...
		if (literal instanceof GeometryImpl) {
			...
			com.vividsolutions.jts.geom.Geometry jtsGeometry = surface
					.getJTSGeometry();
			wkt = jtsGeometry.toText();
		} else if (literal instanceof com.vividsolutions.jts.geom.Geometry) {
			com.vividsolutions.jts.geom.Geometry jtsGeometry = (com.vividsolutions.jts.geom.Geometry) literal;
			wkt = jtsGeometry.toText();
		} else {
			throw new UnsupportedOperationException(
	...
	public Object visit(After after, Object delegate) {
		...
		if (literal instanceof Date) {
			return ((FilterDelegate<?>) delegate).after(propertyName,
					(Date) literal);
		} else if (literal instanceof Instant) {
			return ((FilterDelegate<?>) delegate).after(propertyName,
	...
	public Object visit(During during, Object delegate) {
		...
		if (filterValues.literal instanceof Period) {
			...
			return ((FilterDelegate<?>) delegate).during(
					filterValues.propertyName, start, end);
		}
		// Relative
			...
			return ((FilterDelegate<?>) delegate).relative(
					filterValues.propertyName, duration.getTimeInMillis());
		} else {
			throw new UnsupportedOperationException(
	...
	private ExpressionValues getExpressions(BinarySpatialOperator filter,
			Object delegate) {
		return getExpressions(filter.getExpression1(), filter.getExpression2(),
				delegate);
	}

	private ExpressionValues getExpressions(BinaryComparisonOperator filter,
			Object delegate) {
		return getExpressions(filter.getExpression1(), filter.getExpression2(),
				delegate);
	}

	private ExpressionValues getExpressions(BinaryTemporalOperator filter,
			Object delegate) {
		return getExpressions(filter.getExpression1(), filter.getExpression2(),
				delegate);
	}

	private ExpressionValues getExpressions(Expression expression1,
			Expression expression2, Object delegate) {
		String propertyName;
		Object literal;
		...
		if (expression1 instanceof PropertyName
			...
			literal = expression2.accept(this, delegate);
				...
				&& expression2 instanceof PropertyName) {
