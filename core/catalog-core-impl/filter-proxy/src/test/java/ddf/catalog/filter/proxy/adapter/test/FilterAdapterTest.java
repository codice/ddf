...
public class FilterAdapterTest {
	...
	private static final PropertyName TEST_PROPERTY = FF
	...
	public void propertyIsEqualTo() {
		...
		assertFilterEquals("Test=" + EPOCH.toString(),
				FF.equals(TEST_PROPERTY, FF.literal(EPOCH)));
		assertFilterEquals("Test=" + EPOCH.toString(),
		...
		assertFilterEquals("Test=5i",
				FF.equals(TEST_PROPERTY, FF.literal(new Integer(5))));
		assertFilterEquals("Test=5s",
				FF.equals(TEST_PROPERTY, FF.literal(new Short((short) 5))));
		assertFilterEquals("Test=5l",
				FF.equals(TEST_PROPERTY, FF.literal(new Long(5))));
		assertFilterEquals("Test=5.0d",
				FF.equals(TEST_PROPERTY, FF.literal(new Double(5))));
		assertFilterEquals("Test=5.0f",
				FF.equals(TEST_PROPERTY, FF.literal(new Float(5))));
		assertFilterEquals("Test={5,6}b",
				FF.equals(TEST_PROPERTY, FF.literal(new byte[] { 5, 6 })));
		assertFilterEquals("Test=true",
				FF.equals(TEST_PROPERTY, FF.literal(true)));
		// test Object case
	...
	public void propertyIsEqualToUri() throws URISyntaxException {
		...
		URI uri = null;
		uri = new URI(url);
		assertFilterEquals("Test=" + url + "o",
				FF.equals(TEST_PROPERTY, FF.literal(uri)));
	}
	...
	public void propteryIsWithNulls() {
		...
		assertFilterFails(FF
				.greaterOrEqual(FF.property(null), FF.literal(null)));
		...
		assertFilterFails(FF.between(FF.property(null), FF.literal(null),
				FF.literal(null)));
	...
	public void propertyIsNotEqualTo() {
		...
		assertFilterEquals("Test!=" + EPOCH.toString() + " to "
				+ EPOCH_PLUS_DAY.toString(),
				FF.notEqual(TEST_PROPERTY, FF.literal(EPOCH_DAY_PERIOD)));
		assertFilterEquals("Test!=5i",
				FF.notEqual(TEST_PROPERTY, FF.literal(new Integer(5))));
		assertFilterEquals("Test!=5s",
				FF.notEqual(TEST_PROPERTY, FF.literal(new Short((short) 5))));
		assertFilterEquals("Test!=5l",
				FF.notEqual(TEST_PROPERTY, FF.literal(new Long(5))));
		assertFilterEquals("Test!=5.0d",
				FF.notEqual(TEST_PROPERTY, FF.literal(new Double(5))));
		assertFilterEquals("Test!=5.0f",
				FF.notEqual(TEST_PROPERTY, FF.literal(new Float(5))));
		assertFilterEquals("Test!={5,6}b",
				FF.notEqual(TEST_PROPERTY, FF.literal(new byte[] { 5, 6 })));
		assertFilterEquals("Test!=true",
				FF.notEqual(TEST_PROPERTY, FF.literal(true)));
		// test Object case
		assertFilterEquals("Test!=[1, 2, 3]o",
	...
	public void propertyIsGreaterThan() {
		...
		assertFilterEquals("Test>5i",
				FF.greater(TEST_PROPERTY, FF.literal(new Integer(5))));
		assertFilterEquals("Test>5s",
				FF.greater(TEST_PROPERTY, FF.literal(new Short((short) 5))));
		assertFilterEquals("Test>5l",
				FF.greater(TEST_PROPERTY, FF.literal(new Long(5))));
		assertFilterEquals("Test>5.0d",
				FF.greater(TEST_PROPERTY, FF.literal(new Double(5))));
		assertFilterEquals("Test>5.0f",
				FF.greater(TEST_PROPERTY, FF.literal(new Float(5))));
		// test Object case
		assertFilterEquals("Test>[1, 2, 3]o",
	...
	public void propertyIsGreaterThanOrEqualTo() {
		assertFilterEquals("Test>=foo",
				FF.greaterOrEqual(TEST_PROPERTY, FOO_LITERAL));
		assertFilterEquals("Test>=" + EPOCH.toString(),
		...
		assertFilterEquals(
				"Test>=5s",
				FF.greaterOrEqual(TEST_PROPERTY,
						FF.literal(new Short((short) 5))));
		assertFilterEquals("Test>=5l",
				FF.greaterOrEqual(TEST_PROPERTY, FF.literal(new Long(5))));
		assertFilterEquals("Test>=5.0d",
	...
	public void propertyIsLessThan() {
		...
		assertFilterEquals("Test<" + EPOCH.toString(),
				FF.less(TEST_PROPERTY, FF.literal(EPOCH)));
		assertFilterEquals("Test<5i",
				FF.less(TEST_PROPERTY, FF.literal(new Integer(5))));
		assertFilterEquals("Test<5s",
				FF.less(TEST_PROPERTY, FF.literal(new Short((short) 5))));
		assertFilterEquals("Test<5l",
				FF.less(TEST_PROPERTY, FF.literal(new Long(5))));
		assertFilterEquals("Test<5.0d",
				FF.less(TEST_PROPERTY, FF.literal(new Double(5))));
		assertFilterEquals("Test<5.0f",
				FF.less(TEST_PROPERTY, FF.literal(new Float(5))));
		// test Object case
		assertFilterEquals("Test<[1, 2, 3]o",
	...
	public void propertyIsLessThanOrEqualTo() {
		assertFilterEquals("Test<=foo",
				FF.lessOrEqual(TEST_PROPERTY, FOO_LITERAL));
		assertFilterEquals("Test<=" + EPOCH.toString(),
				FF.lessOrEqual(TEST_PROPERTY, FF.literal(EPOCH)));
		assertFilterEquals("Test<=5i",
				FF.lessOrEqual(TEST_PROPERTY, FF.literal(new Integer(5))));
		assertFilterEquals("Test<=5s",
				FF.lessOrEqual(TEST_PROPERTY, FF.literal(new Short((short) 5))));
		assertFilterEquals("Test<=5l",
				FF.lessOrEqual(TEST_PROPERTY, FF.literal(new Long(5))));
		assertFilterEquals("Test<=5.0d",
				FF.lessOrEqual(TEST_PROPERTY, FF.literal(new Double(5))));
		assertFilterEquals("Test<=5.0f",
				FF.lessOrEqual(TEST_PROPERTY, FF.literal(new Float(5))));
		// test Object case
		assertFilterEquals(
				"Test<=[1, 2, 3]o",
				FF.lessOrEqual(TEST_PROPERTY,
						FF.literal(Arrays.asList(1, 2, 3))));
	}
	...
	public void propertyIsBetween() {
		...
		assertFilterEquals(
				...
				FF.between(TEST_PROPERTY, FF.literal(EPOCH),
		...
		assertFilterEquals(
				"1i<=Test<=5i",
				FF.between(TEST_PROPERTY, FF.literal(new Integer(1)),
		...
		assertFilterEquals(
				"1l<=Test<=5l",
				FF.between(TEST_PROPERTY, FF.literal(new Long(1)),
						FF.literal(new Long(5))));
		assertFilterEquals(
				"1.0d<=Test<=5.0d",
				FF.between(TEST_PROPERTY, FF.literal(new Double(1)),
						FF.literal(new Double(5))));
		assertFilterEquals(
				"1.0f<=Test<=5.0f",
				FF.between(TEST_PROPERTY, FF.literal(new Float(1)),
						FF.literal(new Float(5))));
		// test Object case
		assertFilterEquals("[1, 2, 3]o<=Test<=[2, 3, 4]o", FF.between(
				TEST_PROPERTY, FF.literal(Arrays.asList(1, 2, 3)),
				FF.literal(Arrays.asList(2, 3, 4))));
	}

	@Test(expected=UnsupportedQueryException.class)
	public void bbox() throws UnsupportedQueryException {
		new GeotoolsFilterAdapterImpl().adapt(FF.bbox(TEST_PROPERTY.toString(), 1.0, 1.0, 2.0, 2.0, null), null);
	}
	...
	public void propertyIsLike() {
		...
		assertFilterEquals("like(Test,foo*?)",
				FF.like(TEST_PROPERTY, "foo*?\\"));
		...
		assertFilterEquals("fuzzy(Test,foo)", FF.like(
				new FuzzyFunction(Arrays.asList((Expression) (TEST_PROPERTY)),
		...
		// xpath exists
		assertFilterEquals("xpath(" + xpath + ")",
				FF.like(FF.property(xpath), ""));
		assertFilterEquals("xpath(" + xpath + ")",
				FF.like(FF.property(xpath), "*"));
		...
		// xpath is fuzzy
		assertFilterEquals("xpath(" + xpath + ",fuzzy(foo*?))", FF.like(
				new FuzzyFunction(Arrays.asList((Expression) (FF
						.property(xpath))), FF.literal("")), "foo*?\\"));
		assertFilterEquals("xpath(" + xpath + ",fuzzy(foo*?))", FF.like(
				new FuzzyFunction(Arrays.asList((Expression) (FF
						.property(xpath))), FF.literal("")), "foo!@#", "!",
				"@", "#"));
	...
	public void reversedPropertyNameAndLiteral() {
		...
		assertFilterEquals("Test<=foo",
				FF.greaterOrEqual(FOO_LITERAL, TEST_PROPERTY));
		assertFilterEquals("Test>foo", FF.less(FOO_LITERAL, TEST_PROPERTY));
		assertFilterEquals("Test>=foo",
				FF.lessOrEqual(FOO_LITERAL, TEST_PROPERTY));
	}

	@Test
	public void unsupportedExpressions() {
		// divide
		assertFilterFails(FF.equals(TEST_PROPERTY,
				FF.divide(FF.literal(6), FF.literal(3))));
		// two expressions
	...
	public void spatialBeyondFilter() {
		...
		// feet
		assertFilterEquals("beyond(Test,wkt(" + POLYGON_WKT
				+ "),3.048006096012192)", FF.beyond(TEST_PROPERTY_VALUE,
				polygonGeometry, DISTANCE_10, UomOgcMapping.FOOT.name()));
	...
	public void spatialDWithinFilter() {
		...
		assertFilterEquals("dwithin(Test,wkt(" + POLYGON_WKT + "),10.0)",
				FF.dwithin(TEST_PROPERTY_VALUE, polygonGeometry,
						DISTANCE_10, UomOgcMapping.METRE.name()));
		// as feet
		assertFilterEquals("dwithin(Test,wkt(" + POLYGON_WKT
				+ "),3.048006096012192)", FF.dwithin(TEST_PROPERTY_VALUE,
				polygonGeometry, DISTANCE_10, UomOgcMapping.FOOT.name()));
	}
	...
	public void spatialWrappedGeometry()
		...
		assertFilterEquals("dwithin(Test,wkt(" + MULTIPOLYGON_WKT + "),10.0)",
				FF.dwithin(TEST_PROPERTY_VALUE, geometry, DISTANCE_10,
						UomOgcMapping.METRE.name()));
	...
	public void temporialDuringFilter() {
		// Absolute
		assertFilterEquals("during(Test," + EPOCH.toString() + ","
				+ EPOCH_PLUS_DAY.toString() + ")",
