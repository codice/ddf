...
public final class ContextualEvaluator 
{
	...
	private static final String[] DEFAULT_XPATH_SELECTORS = new String[] { DEFAULT_XPATH_1, DEFAULT_XPATH_2 };
			...
			throws IOException, ParseException 
	{
		...
		// Handle case where no search phrase is specified. Contextual criteria should then specify text path(s)
		...
		if ( cec.isCaseSensitiveSearch() )
		{
		    ...
		    queryParser = new QueryParser( Version.LUCENE_30, CASE_SENSITIVE_FIELD_NAME, new CaseSensitiveStandardAnalyzer( Version.LUCENE_30 ) );
		...
		else
		{
		    ...
		    queryParser = new QueryParser( Version.LUCENE_30, FIELD_NAME, new StandardAnalyzer( Version.LUCENE_30 ) );
		}
    ...
    /**
     * Create a field with the specified field name and value, and add it to a Lucene Document to be added to the specified
     * IndexWriter.
     * 
     * @param indexWriter
     * @param fieldName
     ...
     * case-sensitive indexed text. Use the default XPath selectors to extract the indexable text from the specified XML document.
     * 
	 ...
	 * @throws IOException
	 */
	public static Directory buildIndex( String fullDocument ) 
	    throws IOException 
	{
    ...
    /**
     * Build one Lucene index for the specified XML Document that contains both case-insensitive and
     * case-sensitive indexed text. Use the provided XPath selectors to extract the indexable text from 
        ...
        throws IOException 
    {
		...
		CaseSensitiveStandardAnalyzer caseSensitiveStandardAnalyzer = new CaseSensitiveStandardAnalyzer( Version.LUCENE_30 );

		// 1. create the index
		Directory index = new RAMDirectory();

		// Retrieve the text from the document that can be indexed using the specified XPath selectors
		...
		// NOTE: the boolean arg in the IndexWriter constructor means to create a new index, overwriting any existing index
		IndexWriter indexWriter = new IndexWriter( index, standardAnalyzer, true, IndexWriter.MaxFieldLength.UNLIMITED );
		
		// Add the indexable text to the case-insensitive index writer, assigning it the "case-insensitive" field name
		addDoc( indexWriter, FIELD_NAME, indexableText );
		indexWriter.close();
		
		// Create a second IndexWriter using the custom case-sensitive StandardAnalyzer
		// NOTE: set boolean to false to append the case-sensitive indexed text to the existing index (populated by first IndexWriter)
		IndexWriter csIndexWriter = new IndexWriter( index, caseSensitiveStandardAnalyzer, false, IndexWriter.MaxFieldLength.UNLIMITED );
		
		// Add the indexable text to the case-sensitive index writer, assigning it the "case-sensitive" field name
    ...
    /**
     * Extract the text from the specified XML Document that is to be indexed using the specified XPath selectors.
     * 
     * @param document
    ...
    private static String getIndexableText( String document, String[] xpathSelectors ) 
    {
        ...
        try 
        {
            ...
            //TODO  Is this safe for all cases? Can there be multiple default namespaces such that this would screw up the metadata?
            
            // Treat the "default namespace" (i.e., xmlns="http://some.namespace") the same as the "no namespace" (i.e., xmlns="") 
            // so that user-specified XPath Selectors do not need to specify a namespace for expressions in the default namespace
            // (For example, user can specify //fileTitle vs. //namespace:fileTitle, where a NamespaceContext/NamespaceResolver
            ...
            for ( String xpath : xpathSelectors )
            {
                ...
                //NodeList nodeList = (NodeList) xHelper.evaluate( xpath, XPathConstants.NODESET, new NamespaceResolver() );
                ...
                { 
                    ...
                    else if ( node.getNodeType() == Node.ELEMENT_NODE )
                    {
                        ...
                        // getTextContent() concatenates *all* text from all descendant Text nodes without
                        // any white space between each Text node's value, e.g., JohnDoe vs. John Doe
                        // That's not good ...
