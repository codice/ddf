        ...
        validateUpdatedNode: function(jsonModel, failList, action) {
             if(action === "install") {
                for(var i = 0; i < this.collection.length; i++) {
                    if((jsonModel.appId === this.collection.models[i].attributes.appId) &&
                        this.collection.models[i].attributes.state !== 'ACTIVE') {
                        failList.push(this.get('appId'));
                        this.collection.models[i].attributes.error = true;
                        break;
                    } else {
                        this.collection.models[i].attributes.removeFlag = false;
                    }
                }
             } else {
                for(var j = 0; j < this.collection.length; j++) {
                    if((jsonModel.appId === this.collection.models[j].attributes.appId) &&
                        this.collection.models[j].attributes.state !== 'INACTIVE') {
                        failList.push(this.get('appId'));
                        this.collection.models[j].attributes.error = true;
                        break;
                    } else {
                        this.collection.models[j].attributes.removeFlag = false;
                    }
                }
             }
        }
    ...
    Applications.TreeNodeCollection = Backbone.Collection.extend({
        ...
        updateAction: function(statusUpdate, action) {
            ...
            for(var k = 0; k < this.length; k++) {
                if(this.models[k].attributes.removeFlag === true) {
                    totalCount++;
                }
            }
            ...
            if(action === "install") {
                for(var i = (this.length-1); i >= 0; i--) {
                    if(that.models[i].attributes.removeFlag === true) {
                        promiseArr.push(that.models[i].installAction(internalStatusUpdate));
                    }
                }
            } else {
                for(var j = 0; j < this.length; j++) {
                    if(that.models[j].attributes.removeFlag === true) {
                        promiseArr.push(that.models[j].uninstallAction(internalStatusUpdate));
                    }
                }
            }
            return Q.all(promiseArr);
        ...
        validateUpdate: function(jsonModel, numNodes, statusUpdate, action) {
            ...
            for(var i = 0; i < jsonModel.length; i++) {
                if(jsonModel[i].removeFlag === true) {
                    this.models[i].validateUpdatedNode(jsonModel[i], failList, action);
                }
            }
