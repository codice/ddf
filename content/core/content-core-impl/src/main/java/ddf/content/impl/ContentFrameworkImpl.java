...
public class ContentFrameworkImpl implements ContentFramework
{
    ...
    public CreateResponse create( CreateRequest createRequest, Request.Directive directive ) throws ContentFrameworkException
    {
        ...
        try
        {
            ContentItem contentItem = new IncomingContentItem( id, incomingContentItem.getInputStream(), incomingContentItem.getMimeTypeRawData(), incomingContentItem.getFilename() );
        ...
        if ( directive == Directive.PROCESS || directive == Directive.STORE_AND_PROCESS )
        {
            ...
            catch ( PluginExecutionException e ) 
            {
                ...
                if (directive == Directive.STORE_AND_PROCESS)
                {
                    ...
                    {
                        logger.warn( "Unable to perform delete because no content storage provider is installed or there is a problem with the content storage provider.", e2 );
                    }
                    
                    // Re-throw the exception (this will fail the Camel route that may have
                    // started this request)
                    throw new ContentFrameworkException("Content Plugin processing failed. Did not store item in content repository and did not create catalog entry.\n" + e.getMessage(), e);
                }
                else
                {
                    // Re-throw the exception (this will fail the Camel route that may have
                    // started this request)
                    throw new ContentFrameworkException("Content Plugin processing failed. Did not create catalog entry.\n" + e.getMessage(), e);
                }
            }
        }
    ...
    public UpdateResponse update( final UpdateRequest updateRequest, Request.Directive directive ) throws ContentFrameworkException
    {
        ...
        if ( directive == Directive.PROCESS || directive == Directive.STORE_AND_PROCESS )
        {
            ...
            catch ( PluginExecutionException e ) 
            {
                ...
                // Re-throw the exception (this will fail the Camel route that may have
                // started this request)
                throw new ContentFrameworkException("Content Plugin processing failed. " + e.getMessage(), e);
            }
        }
