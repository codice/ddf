...
/**
 * 
 * ContentItem is the POJO representing the information about the content
 * to be stored in the {@link StorageProvider}.
 * 
 * A ContentItem encapsulates the content's globally unique ID, mime type,
 * and input stream (i.e., the actual content).
 * 
...
{    
     ...
     * 
     * This is used by the {@link StorageProvider}
     * when the content item is stored in the content repository and will be of the
     * form "content:<GUID>".
     * 
     * Or this is used by the endpoint when the client specifies the content item's location
     * which means the content is stored remote/external to DDF.
     */
    ...
    /**
     * Return the mime type raw data for the content item, e.g., image/nitf or text/xml;id=ddms20
     * 
     ...
     * @throws IOException
     *              if the total number of bytes is unknown or not applicable (e.g., in the case of a live stream)
     */
