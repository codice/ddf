...
/**
 * The REST Endpoint for the Content Framework that provides URLs to create, read,
 * update, and delete content in the Content Repository.
 * 
 * @author rodgersh
...
public class ContentEndpoint
{
    ...
    /**
     * Create an entry in the Content Repository and/or the Metadata Catalog based on the
     * request's directive. The input request is in multipart/form-data format, with the 
     * expected parts of the body being the directive (STORE, PROCESS, STORE_AND_PROCESS),
     * and the file, with optional filename specified, followed by the contents to be stored.
     * If the filename is not specified for the contents in the body of the input request, then
     * the default filename "file" will be used, with the file extension determined based upon
     * the MIME type.
     * 
     * A sample multipart/form-data request would look like:
     *    Content-Type: multipart/form-data; boundary=ARCFormBoundaryfqeylm5unubx1or 
     *
     *    --ARCFormBoundaryfqeylm5unubx1or
     *    Content-Disposition: form-data; name="directive"
     *
     *    STORE_AND_PROCESS
     *    --ARCFormBoundaryfqeylm5unubx1or--
     *     Content-Disposition: form-data; name="myfile.json"; filename="C:\DDF\geojson_valid.json"
    ...
    public Response updateCatalogOnly( InputStream stream, @HeaderParam("Content-Type") String contentType,
        @HeaderParam( "contentUri" ) String contentUri )
        throws ContentEndpointException
    {
        logger.trace( "ENTERING: update" );
        logger.debug( "contentUri = " + contentUri );
    ...
    protected Response doRead( String id ) throws ContentEndpointException
    {
        ...
        catch (Exception e)
        {
            ...
            Response.ResponseBuilder responseBuilder = Response.ok( "Content Item " + id + " does not exist.\n" + e.getMessage() );
        ...
        throws ContentEndpointException
    {
        ...
        catch ( Exception e )
        {
            ...
            Response.ResponseBuilder responseBuilder = Response.ok( "Content Item " + id + " not updated.\n" + e.getMessage() );
            responseBuilder.status( Response.Status.NOT_FOUND );
            response = responseBuilder.build();
        }
    ...
    protected Response doDelete( String id, String directive, String contentUri ) throws ContentEndpointException
    {
        ...
        catch ( ContentFrameworkException e )
        {
            ...
            Response.ResponseBuilder responseBuilder = Response.ok( "Content Item " + id + " not found.\n" + e.getMessage() );
            responseBuilder.status( Response.Status.NOT_FOUND );
            response = responseBuilder.build();
        }
