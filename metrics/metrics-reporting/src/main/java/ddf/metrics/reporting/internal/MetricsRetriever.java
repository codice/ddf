...
{
    /**
     * Retrieves the PNG-formatted graph of the metric's data from the specified RRD
     * file over the specified time range. The vertical axis label defaults to the
     * metric's name, and the title for the graph defaults to the metric's name and the
     * time range (e.g., catalogQueries for Apr 10 2013 09:14:43 to Apr 10 2013 09:29:43). 
     * 
    ...
    /**
     * Retrieves the PNG-formatted graph of the metric's data from the specified RRD
     * file over the specified time range, using the input y-axis label and title on
    ...
    /**
     * Retrieves the metric's data as a CSV (Comma Separated Values) formatted string from 
     * the specified RRD file over the specified time range. This CSV string will consist
     * of the first row being the column headers (namely Timestamp,Value), and the remaining
     * lines will each contain the timestamp and value for a single sample of the metric's data. 
     * 
     * @param rrdFilename name of the RRD file to retrieve the metric's data from
     * @param startTime start time, in seconds since Unix epoch, to retrieve metric's data
     * @param endTime end time, in seconds since Unix epoch, to retrieve metric's data
    ...
    /**
     * Retrieves the metric's data as an XML formatted string from 
     * the specified RRD file over the specified time range. No XML schema is defined or
     * referenced in the returned XML.
     * 
     ...
     * </pre>
     * 
     * Note that <totalCount> is only present for metrics that are counters, i.e., values that
     * would always increment and never decrement, e.g., catalogQueries metric.
     * 
    ...
    /**
     * Retrieves the metric's data as an XLS (Excel spreadsheet) formatted stream from 
     * the specified RRD file over the specified time range. This XLS stream will consist
     * of:
     * <ul>
     ...
     * <li>the remaining rows will each contain the timestamp and value for a single sample of the metric's data</li>
    ...
    /**
     * Retrieves the metric's data as a PPT (PowerPoint) formatted stream from 
     * the specified RRD file over the specified time range. This PPT stream will consist
     * of a single slide containing:
     * <ul>
    ...
    /**
     * Retrieves the metric's data as a JSON formatted string from 
     * the specified RRD file over the specified time range.
     * 
     ...
     * one worksheet for each metric. Refer to the createXlsData() method for a description
    ...
    /**
     * Returns a PPT (PowerPoint) formatted stream over the specified time range that contains
     * one slide for each metric. Refer to the createPptData() method for a description
     * of what each slide would contain.
     * 
     * @param metricName name of the metric to retrieve data for
     * @param metricsDir directory containing all of the metrics' RRD files, typically
     *     <DDF_INSTALL_DIR>/data/metrics
