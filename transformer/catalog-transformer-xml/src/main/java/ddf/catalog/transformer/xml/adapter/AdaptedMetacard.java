...
public class AdaptedMetacard implements Metacard {
	...
	public AdaptedMetacard(Metacard metacard) {
		if (metacard == null) {
			...
			this.metacardType = metacard.getMetacardType() != null ? metacard
					.getMetacardType() : BasicTypes.BASIC_METACARD;
			for (AttributeDescriptor descriptor : metacardType
					.getAttributeDescriptors()) {
				if (descriptor != null) {
					this.setAttribute(metacard.getAttribute(descriptor
							.getName()));
				}
			}
		}
	 ...
	 * 
	 * @see
	 * ddf.catalog.data.MetacardImpl#setAttribute(ddf.catalog.data.Attribute)
	 */
	@Override
