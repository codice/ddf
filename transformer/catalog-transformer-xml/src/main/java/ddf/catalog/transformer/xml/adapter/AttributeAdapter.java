...
public class AttributeAdapter extends
		XmlAdapter<AbstractAttributeType, Attribute> {
	...
	@Override
	public AbstractAttributeType marshal(Attribute attribute)
		...
		AttributeDescriptor descriptor = metacardType
				.getAttributeDescriptor(attribute.getName());
		if (descriptor == null || Metacard.ID.equals(descriptor.getName())
				|| descriptor.getType() == null) {
			return null;
		}

		element = convertAttribute(attribute, descriptor.getType()
				.getAttributeFormat());
		return element;
	}

	private AbstractAttributeType convertAttribute(Attribute attribute,
			AttributeFormat format) throws CatalogTransformerException {
		AbstractAttributeType element = null;
		...
		try {
			...
			throw new CatalogTransformerException(
					"Could not marshall Attribute", e);
		}
		return element;
	}
	...
	public Attribute unmarshal(AbstractAttributeType element)
			...
			JAXBException, ConversionFailedException {

		Attribute attribute = null;

		if (element instanceof Base64BinaryElement) {
			attribute = Base64BinaryAdapter
