...
/**
 * Implements the {@link MetacardTransformer} interface to transform a single
 * {@link Metacard} instance to GeoJSON. This class places what is returned by
 * {@link Metacard#getLocation()} in the geometry JSON object in the GeoJSON
 * output. The rest of the attributes of the Metacard are placed in the
 * properties object in the JSON. See geojson.org for the GeoJSON specification.
 * 
...
public class GeoJsonMetacardTransformer implements MetacardTransformer {
			...
			if (attribute != null) {
				...
				case DATE:
					if (attribute.getValue() != null) {
						properties.put(attribute.getName(), ISO_8601_DATE_FORMAT.format(((Date) attribute.getValue())));
					}
					break;
				case BINARY:
					if (attribute.getValue() != null) {
					...
					break;
				//Since we just need the string version of Doubles, Longs, Floats, Integers, or Shorts, no conversion 
				//or processing is necessary.  The toString method on each of those classes suffices.   
				case DOUBLE:
				case LONG:
				case FLOAT:
				case INTEGER:
						...
						try {
							...
							if (geoJsonGeometry == null) {
								throw new CatalogTransformerException(
										"Could not perform transform: unsupported geometry [" + attribute.getValue()
												+ "]");
							}
							rootObject.put(CompositeGeometry.GEOMETRY_KEY, geoJsonGeometry.toJsonMap());
	...
	public String toString() {
		return MetacardTransformer.class.getName() +" {Impl=" + this.getClass().getName() + ", id=" + ID + ", MIME Type="
				+ DEFAULT_MIME_TYPE + "}";
	}
