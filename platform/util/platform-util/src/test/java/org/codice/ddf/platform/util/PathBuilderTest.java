...
public class PathBuilderTest {
    ...
    @Before
    public void setup() {
        Path pathToTemp = Paths.get(tempDir.getRoot()
                .getAbsolutePath());
        pathTokens = new ArrayList<>();
        pathToTemp.iterator()
                .forEachRemaining(p -> pathTokens.add(p.toString()));
    }

    @Test
    public void pathWithNoSubs() {
        assertThat(new PathBuilder(pathTokens.get(0)).build(),
                is(equalTo(Paths.get(pathTokens.get(0)))));
    }

    @Test
    public void pathWithOneSubElement() {
        assertThat(new PathBuilder(pathTokens.get(0), pathTokens.get(1)).build(),
    ...
    public void pathWithAllSubElement() {
        ...
        assertThat(new PathBuilder(pathTokens.get(0), subElemsArr).build(),
    ...
    public void pathWithNullRootAndSubElements() {
        String[] subElems = pathTokens.subList(1, pathTokens.size())
                .toArray(new String[pathTokens.size() - 1]);
