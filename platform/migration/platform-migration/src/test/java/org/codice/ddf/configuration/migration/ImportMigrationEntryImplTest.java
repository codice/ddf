...
public class ImportMigrationEntryImplTest extends AbstractMigrationTest {
    ...
    public void restoreWhenFileIsReadOnly() throws Exception {
        ...
        doReturn(true).when(absoluteFile)
                .setWritable(true);
        doReturn(true).when(absoluteFile)
                .setWritable(false);
    ...
    public void restoreWhenFileIsReadOnlyAndCannotMakeItWriteable() throws Exception {
        ...
        when(mockReport.wasIOSuccessful(any(ThrowingRunnable.class))).thenAnswer(it -> {
        ...
        when(absoluteFile.canWrite()).thenReturn(false, false, true);
        // must reverse mockito calls otherwise using when() will actually call the setWritable() on the spy and affect the file system
        doReturn(false).when(absoluteFile)
                .setWritable(true);
    ...
    public void restoreWithConsumer() throws Exception {
        ...
        BiThrowingConsumer<MigrationReport, Optional<InputStream>, IOException> mockConsumer = mock(
                BiThrowingConsumer.class);
        final ImportMigrationEntryImpl entry = new ImportMigrationEntryImpl(mockContext,
                ENTRY_NAME);
