...
public class AbstractMigrationReportTest extends AbstractMigrationTest {
    ...
    protected static final String[] POTENTIAL_WARNING_MESSAGE_STRINGS = Stream.of(MESSAGE_STRINGS)
            .filter(m -> !m.startsWith("info"))
            .toArray(String[]::new);

    protected static final String[] ERRORS = Stream.of(MESSAGE_STRINGS)
            .filter(m -> m.startsWith("error"))
            .toArray(String[]::new);

    protected static final String[] WARNINGS = Stream.of(MESSAGE_STRINGS)
            .filter(m -> m.startsWith("warning"))
            .toArray(String[]::new);

    protected static final String[] INFOS = Stream.of(MESSAGE_STRINGS)
            .filter(m -> m.startsWith("info"))
            .toArray(String[]::new);
