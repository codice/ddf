 ...
 *
 * <ul>
 *   <li>It is a <code>BundleActivator</code> which is called when the bundle is started and
 *       stopped.
 *   <li>It is a <code>BundleListener</code> which gets informed when the states of bundles change.
 *       Mostly this is needed to unbind any bound configuration in case a bundle is uninstalled.
 *   <li>It is a <code>ServiceListener</code> which gets informed when <code>ManagedService</code>
 *       and <code>ManagedServiceFactory</code> services are registered and unregistered. This is
 *       used to provide configuration to these services. As a service listener it also listens for
 *       {@link PersistenceManager} instances being registered to support different configuration
 *       persistence layers.
 *   <li>A {@link ConfigurationAdminFactory} instance is registered as the <code>ConfigurationAdmin
 *       </code> service.
 *   <li>A {@link FilePersistenceManager} instance is registered as a default {@link
 *       PersistenceManager}.
 *   <li>Last but not least this instance manages all tasks laid out in the specification such as
 *       maintaining configuration, taking care of configuration events, etc.
 * </ul>
 *
 * <p>The default {@link FilePersistenceManager} is configured with a configuration location taken
 * from the <code>felix.cm.dir</code> framework property. If this property is not set the <code>
...
public class ConfigurationManager implements BundleActivator, BundleListener {
   ...
   * "felix.cm.loglevel"). The log level setting is only used if there is no OSGi LogService
   * available. Otherwise this setting is ignored.
   *
   * <p>This value of this property is expected to be an integer number corresponding to the log
   * level values of the OSGi LogService. That is 1 for errors, 2 for warnings, 3 for informational
   * messages and 4 for debug messages. The default value is 2, such that only warnings and errors
   * are logged in the absence of a LogService.
   */
  ...
  /**
   * The actual list of {@link PersistenceManager persistence managers} to use when looking for
   * configuration data. This list is built from the {@link #persistenceManagerMap}, which is
   * ordered according to the {@link RankingComparator}.
   */
   ...
   *
   * <p>On bundle startup this map is loaded from persistence and validated against the locations of
   * installed bundles: Entries pointing to bundle locations not currently installed are removed.
   *
   * <p>The map is written to persistence on each change.
   */
  ...
  public void start(BundleContext bundleContext) {
    ...
    configurationListenerTracker =
        new ServiceTracker(bundleContext, ConfigurationListener.class.getName(), null);
    configurationListenerTracker.open();
    syncConfigurationListenerTracker =
        new ServiceTracker(bundleContext, SynchronousConfigurationListener.class.getName(), null);
    ...
    persistenceManagerTracker =
        new ServiceTracker(bundleContext, PersistenceManager.class.getName(), null);
    ...
    props.put(
        ...
        "Configuration Admin Service Specification 1.5 Implementation");
    props.put(Constants.SERVICE_VENDOR, "Apache Software Foundation");
  ...
  ConfigurationImpl[] getCachedConfigurations() {
    synchronized (configurations) {
      return configurations.values().toArray(new ConfigurationImpl[configurations.size()]);
    }
  }
    ...
    if (dynamicBindings != null) {
      try {
        ...
        log(
            ...
            "Failed storing dynamic configuration binding for {0} to {1}",
            new Object[] {pid, location, ioe});
      }
    }
  }
  ...
  ConfigurationImpl createFactoryConfiguration(String factoryPid, String location)
      throws IOException {
    return cacheConfiguration(createConfiguration(createPid(factoryPid), factoryPid, location));
  }

  /**
   * Returns a targeted configuration for the given service PID and the reference target service.
   *
   * <p>A configuration returned has already been checked for visibility by the bundle registering
   * the referenced service. Additionally, the configuration is also dynamically bound if needed.
   *
   * @param rawPid The raw service PID to get targeted configuration for.
   * @param target The target <code>ServiceReference</code> to get configuration for.
   * @return The best matching targeted configuration or <code>null</code> if there is no
   *     configuration at all.
   * @throwss IOException if an error occurrs reading configurations from persistence.
   */
  ConfigurationImpl getTargetedConfiguration(final String rawPid, final ServiceReference target)
      throws IOException {
      ...
      for (String candidate : names) {
        ...
        if (config != null && !config.isDeleted()) {
          ...
          log(
              ...
              new Object[] {config.getPid(), toString(target), config.getBundleLocation()});
        }
      }
    } else {
      log(
  ...
  /**
   * Returns the {@link ConfigurationImpl} with the given PID if available in the internal cache or
   * from any persistence manager. Otherwise <code>null</code> is returned.
   *
   * @param pid The PID for which to return the configuration
   * @return The configuration or <code>null</code> if non exists
   * @throws IOException If an error occurrs reading from a persistence manager.
   */
  ConfigurationImpl getConfiguration(String pid) throws IOException {
    ...
    if (config != null) {
      log(
          LogService.LOG_DEBUG,
          "Found cached configuration {0} bound to {1}",
          new Object[] {pid, config.getBundleLocation()});
    ...
    for (int i = 0; i < pmList.length; i++) {
      if (pmList[i].exists(pid)) {
        ...
        log(
            LogService.LOG_DEBUG,
            "Found existing configuration {0} bound to {1}",
            new Object[] {pid, config.getBundleLocation()});
  ...
  /**
   * Creates a regular (non-factory) configuration for the given PID setting the bundle location
   * accordingly.
   *
   * <p>This method assumes the configuration to not exist yet and will create it without further
   * checking.
   *
   * @param pid The PID of the new configuration
   * @param bundleLocation The location to set on the new configuration. This may be <code>null
   *     </code> to not bind the configuration yet.
   * @return The new configuration persisted in the first persistence manager.
   * @throws IOException If an error occurrs writing the configuration to the persistence.
   */
  ...
  ConfigurationImpl[] listConfigurations(
      ConfigurationAdminImpl configurationAdmin, String filterString)
    ...
    log(LogService.LOG_DEBUG, "Listing configurations matching {0}", new Object[] {filterString});
    ...
    for (int i = 0; i < pmList.length; i++) {
      ...
      while (configs.hasMoreElements()) {
        ...
        if (!configurationAdmin.hasPermission(
            this, (String) config.get(ConfigurationAdmin.SERVICE_BUNDLELOCATION))) {
          log(
              LogService.LOG_DEBUG,
              "Omitting configuration {0}: No permission for bundle {1} on configuration bound to {2}",
              new Object[] {
                pid,
                configurationAdmin.getBundle().getLocation(),
        ...
        if (!cfg.isNew()) {
          log(LogService.LOG_DEBUG, "Adding configuration {0}", new Object[] {pid});
          configList.add(cfg);
        } else {
          log(LogService.LOG_DEBUG, "Omitting configuration {0}: Is new", new Object[] {pid});
        }
      }
    }
    ...
    return (ConfigurationImpl[]) configList.toArray(new ConfigurationImpl[configList.size()]);
  }
  ...
  void deleted(ConfigurationImpl config) {
    ...
    fireConfigurationEvent(
        ConfigurationEvent.CM_DELETED, config.getPidString(), config.getFactoryPidString());
    updateThread.schedule(new DeleteConfiguration(config));
    log(LogService.LOG_DEBUG, "DeleteConfiguration({0}) scheduled", new Object[] {config.getPid()});
  }

  void updated(ConfigurationImpl config, boolean fireEvent) {
    if (fireEvent) {
      fireConfigurationEvent(
          ConfigurationEvent.CM_UPDATED, config.getPidString(), config.getFactoryPidString());
    }
    updateThread.schedule(new UpdateConfiguration(config));
    log(LogService.LOG_DEBUG, "UpdateConfiguration({0}) scheduled", new Object[] {config.getPid()});
  }
  ...
  void locationChanged(ConfigurationImpl config, String oldLocation) {
    ...
    if (oldLocation != null && !config.isNew()) {
      ...
      log(
          LogService.LOG_DEBUG,
          "LocationChanged({0}, {1}=>{2}) scheduled",
          new Object[] {config.getPid(), oldLocation, config.getBundleLocation()});
    } else {
      log(
          LogService.LOG_DEBUG,
          "LocationChanged not scheduled for {0} (old location is null or configuration is new)",
          new Object[] {config.getPid()});
    }
  }

  void fireConfigurationEvent(int type, String pid, String factoryPid) {
    // prevent event senders
    FireConfigurationEvent asyncSender =
        new FireConfigurationEvent(this.configurationListenerTracker, type, pid, factoryPid);
    FireConfigurationEvent syncSender =
        new FireConfigurationEvent(this.syncConfigurationListenerTracker, type, pid, factoryPid);
    ...
    if (syncSender.hasConfigurationEventListeners()) {
      ...
      log(
          ...
          new Object[] {syncSender.getTypeName()});
    }
    ...
    if (asyncSender.hasConfigurationEventListeners()) {
      ...
      log(
          LogService.LOG_DEBUG,
          "No ConfigurationListeners to send {0} event to.",
          new Object[] {asyncSender.getTypeName()});
    }
  }
  ...
  private CachingPersistenceManagerProxy[] getPersistenceManagers() {
    ...
    if (persistenceManagers == null || currentPmtCount > pmtCount) {
      ...
      if (refs == null || refs.length == 0) {
        ...
        pm =
            (CachingPersistenceManagerProxy[])
                pmList.toArray(new CachingPersistenceManagerProxy[pmList.size()]);
      }
    ...
    if (context != null) {
      try {
        ServiceReference[] refs =
            context.getServiceReferences(ConfigurationAdmin.class.getName(), null);
        if (refs != null) {
          for (int i = 0; i < refs.length; i++) {
  ...
  /**
   * Configures the ManagedService and returns the service.pid service property as a String[], which
   * may be <code>null</code> if the ManagedService does not have such a property.
   */
  /**
   * Configures the ManagedServiceFactory and returns the service.pid service property as a
   * String[], which may be <code>null</code> if the ManagedServiceFactory does not have such a
   * property.
   */
  /**
   * Schedules the configuration of the referenced service with configuration for the given PID.
   *
   * @param pid The list of service PID of the configurations to be provided to the referenced
   *     service.
   * @param sr The <code>ServiceReference</code> to the service to be configured.
   * @param factory <code>true</code> If the service is considered to be a <code>
   *     ManagedServiceFactory</code>. Otherwise the service is considered to be a <code>
   *     ManagedService</code>.
   */
  public void configure(
      String[] pid, ServiceReference sr, final boolean factory, final ConfigurationMap<?> configs) {
    if (this.isLogEnabled(LogService.LOG_DEBUG)) {
      this.log(LogService.LOG_DEBUG, "configure(ManagedService {0})", new Object[] {toString(sr)});
    }
    ...
    log(LogService.LOG_DEBUG, "[{0}] scheduled", new Object[] {r});
  }

  /**
   * Factory method to create a new configuration object. The configuration object returned is not
   * stored in configuration cache and only persisted if the <code>factoryPid</code> parameter is
   * <code>null</code>.
   *
   * @param pid The PID of the new configuration object. Must not be <code>null</code>.
   * @param factoryPid The factory PID of the new configuration. Not <code>null</code> if the new
   *     configuration object belongs to a factory. The configuration object will not be persisted
   *     if this parameter is not <code>null</code>.
   * @param bundleLocation The bundle location of the bundle to which the configuration belongs or
   *     <code>null</code> if the configuration is not bound yet.
   * @return The new configuration object
   * @throws IOException May be thrown if an error occurrs persisting the new configuration object.
   */
  ConfigurationImpl createConfiguration(String pid, String factoryPid, String bundleLocation)
      throws IOException {
    log(
        LogService.LOG_DEBUG,
        "createConfiguration({0}, {1}, {2})",
        new Object[] {pid, factoryPid, bundleLocation});
    return new ConfigurationImpl(
        this, getPersistenceManagers()[0], pid, factoryPid, bundleLocation);
  }

  /**
   * Returns a list of {@link Factory} instances according to the Configuration Admin 1.5
   * specification for targeted PIDs (Section 104.3.2)
   *
   * @param rawFactoryPid The raw factory PID without any targetting.
   * @param target The <code>ServiceReference</code> of the service to be supplied with targeted
   *     configuration.
   * @return A list of {@link Factory} instances as listed above. This list will always at least
   *     include an instance for the <code>rawFactoryPid</code>. Other instances are only included
   *     if existing.
   * @throws IOException If an error occurrs reading any of the {@link Factory} instances from
   *     persistence
   */
  List<Factory> getTargetedFactories(final String rawFactoryPid, final ServiceReference target)
      throws IOException {
  ...
  /**
   * Gets the factory with the exact identifier from the cached or from the persistence managers. If
   * no factory exists already one is created and cached.
   *
   * @param factoryPid The PID of the {@link Factory} to return
   * @return The existing or newly created {@link Factory}
   * @throws IOException If an error occurrs reading the factory from a {@link PersistenceManager}
   */
  ...
  /**
   * Gets the factory with the exact identifier from the cached or from the persistence managers. If
   * no factory exists <code>null</code> is returned.
   *
   * @param factoryPid The PID of the {@link Factory} to return
   ...
   *
   * <p>The plugins to be called are selected as <code>ConfigurationPlugin</code> services
   * registered with a <code>cm.target</code> property set to <code>*</code> or the factory PID of
   * the configuration (for factory configurations) or the PID of the configuration (for non-factory
   * configurations).
   *
   * @param props The configuraiton properties run through the registered ConfigurationPlugin
   *     services. This must not be <code>null</code>.
   * @param sr The service reference of the managed service (factory) which is to be updated with
   *     configuration
   * @param configPid The PID of the configuration object whose properties are to be augmented
   * @param factoryPid the factory PID of the configuration object whose properties are to be
   *     augmented. This is non-<code>null</code> only for a factory configuration.
   */
  public void callPlugins(
    ...
    for (int i = 0; i < plugins.length; i++) {
      ...
      if (plugin != null) {
        ...
        try {
          plugin.modifyConfiguration(
              sr, ignore ? CaseInsensitiveDictionary.unmodifiable(props) : props);
        } catch (Throwable t) {
          log(
              LogService.LOG_ERROR,
              "Unexpected problem calling configuration plugin {0}",
              new Object[] {toString(pluginRef), t});
        } finally {
  ...
  /**
   * Checks whether the bundle is allowed to receive the configuration with the given location
   * binding.
   *
   * <p>This method implements the logic defined CM 1.4 / 104.4.1:
   *
   * <ul>
   *   <li>If the location is <code>null</code> (the configuration is not bound yet), assume the
   *       bundle is allowed
   *   <li>If the location is a single location (no leading "?"), require the bundle's location to
   *       match
   *   <li>If the location is a multi-location (leading "?"), assume the bundle is allowed if there
   *       is no security manager. If there is a security manager, check whether the bundle has
   *       "target" permission on this location.
   * </ul>
   */
  boolean canReceive(final Bundle bundle, final String location) {
    if (location == null) {
      log(
          LogService.LOG_DEBUG,
          "canReceive=true; bundle={0}; configuration=(unbound)",
      ...
      if (System.getSecurityManager() != null) {
        final boolean hasPermission =
            bundle.hasPermission(
                new ConfigurationPermission(location, ConfigurationPermission.TARGET));
        log(
            LogService.LOG_DEBUG,
            "canReceive={0}: bundle={1}; configuration={2} (SecurityManager check)",
            new Object[] {new Boolean(hasPermission), bundle.getLocation(), location});
        return hasPermission;
      }

      log(
          LogService.LOG_DEBUG,
          "canReceive=true; bundle={0}; configuration={1} (no SecurityManager)",
          new Object[] {bundle.getLocation(), location});
      return true;
    } else {
      ...
      log(
          LogService.LOG_DEBUG,
          "canReceive={0}: bundle={1}; configuration={2}",
          new Object[] {new Boolean(hasPermission), bundle.getLocation(), location});
      return hasPermission;
    }
  }
  ...
  /**
   * The <code>ManagedServiceUpdate</code> updates a freshly registered <code>ManagedService</code>
   * with a specific configuration. If a ManagedService is registered with multiple PIDs an instance
   * of this class is used for each registered PID.
   */
  private class ManagedServiceUpdate implements Runnable {
    ...
    public void run() {
      for (String pid : this.pids) {
        try {
          ...
          log(LogService.LOG_ERROR, "Error loading configuration for {0}", new Object[] {pid, ioe});
        } catch (Exception e) {
          log(
              LogService.LOG_ERROR,
              "Unexpected problem providing configuration {0} to service {1}",
              new Object[] {pid, ConfigurationManager.toString(this.sr), e});
        }
      }
    }
    ...
    private void provide(final String servicePid, final ConfigurationImpl config) {
      ...
      log(
          LogService.LOG_DEBUG,
          "Updating service {0} with configuration {1}@{2}",
          new Object[] {servicePid, configPid, new Long(revision)});

      managedServiceTracker.provideConfiguration(
          sr, configPid, null, properties, revision, this.configs);
    }
  ...
  /**
   * The <code>ManagedServiceFactoryUpdate</code> updates a freshly registered <code>
   * ManagedServiceFactory</code> with a specific configuration. If a ManagedServiceFactory is
   * registered with multiple PIDs an instance of this class is used for each registered PID.
   */
  private class ManagedServiceFactoryUpdate implements Runnable {
    ...
    ManagedServiceFactoryUpdate(
        String[] factoryPids, ServiceReference sr, final ConfigurationMap<?> configs) {
    ...
    public void run() {
      for (String factoryPid : this.factoryPids) {
        ...
        try {
            ...
            synchronized (factory) {
              for (Iterator pi = factory.getPIDs().iterator(); pi.hasNext(); ) {
                ...
                try {
                  ...
                  log(
                      LogService.LOG_ERROR,
                      "Error loading configuration for {0}",
                      new Object[] {pid, ioe});
                  continue;
                }
                ...
                if (cfg == null) {
                  log(
                      LogService.LOG_ERROR,
                      "Configuration {0} referred to by factory {1} does not exist",
                      new Object[] {pid, factoryPid});
                  ...
                  log(
                      LogService.LOG_ERROR,
                      "Ignoring new configuration pid={0}",
                      new Object[] {pid});
                  continue;
                }
          ...
          log(
              LogService.LOG_ERROR,
              "Cannot get factory mapping for factory PID {0}",
              new Object[] {factoryPid, ioe});
        }
      }
    }
    ...
    private void provide(final String factoryPid, final ConfigurationImpl config) {
      ...
      log(
          LogService.LOG_DEBUG,
          "Updating service {0} with configuration {1}/{2}@{3}",
          new Object[] {factoryPid, config.getFactoryPid(), config.getPid(), new Long(revision)});
      ...
      if (rawProperties != null) {
        log(
            LogService.LOG_DEBUG,
            "{0}: Updating configuration pid={1}",
            new Object[] {ConfigurationManager.toString(sr), config.getPid()});
        managedServiceFactoryTracker.provideConfiguration(
            sr, config.getPid(), config.getFactoryPid(), rawProperties, revision, this.configs);
      }
    }
  ...
  private abstract class ConfigurationProvider<T> implements Runnable {
    ...
    protected BaseTracker<T> getHelper() {
      if (this.helper == null) {
        this.helper =
            (BaseTracker<T>)
                ((this.config.getFactoryPid() == null)
                    ? ConfigurationManager.this.managedServiceTracker
      ...
      if (this.config.getFactoryPid() == null) {
        try {
          ...
          if (configPidString == null) {
            // The managed service is not registered anymore in the OSGi service registry.
            return false;
          }
          ...
          log(
              LogService.LOG_ERROR,
              "Error loading configuration for {0}",
              new Object[] {this.config.getPid(), ioe});
        } catch (Exception e) {
          log(
              LogService.LOG_ERROR,
              "Unexpected problem providing configuration {0} to service {1}",
              new Object[] {this.config.getPid(), ConfigurationManager.toString(sr), e});
        }
      }
  ...
  /**
   * The <code>UpdateConfiguration</code> is used to update <code>ManagedService[Factory]</code>
   * services with the configuration they are subscribed to. This may cause the configuration to be
  ...
  private class UpdateConfiguration extends ConfigurationProvider {
    ...
    public void run() {
      log(
          LogService.LOG_DEBUG,
          "Updating configuration {0} to revision #{1}",
          new Object[] {config.getPid(), new Long(revision)});
      ...
      if (!srList.isEmpty()) {
        ...
        if (bundle == null) {
          log(
              ...
              new Object[] {ConfigurationManager.toString(srList.get(0))});
          return;
        }
        ...
        for (ServiceReference<?> ref : srList) {
          ...
          if (refBundle == null) {
            log(
                LogService.LOG_DEBUG,
                "Service {0} seems to be unregistered concurrently (not providing configuration)",
                new Object[] {ConfigurationManager.toString(ref)});
        ...
        log(
            LogService.LOG_DEBUG,
            "No ManagedService[Factory] registered for updates to configuration {0}",
            new Object[] {config.getPid()});
      }
    }
  ...
  /**
   * The <code>DeleteConfiguration</code> class is used to inform <code>ManagedService[Factory]
   * </code> services of a configuration being deleted.
   */
    ...
    public void run() {
        ...
        for (ServiceReference<?> sr : srList) {
          ...
          if (srBundle == null) {
            ...
            log(
                ...
                new Object[] {config.getPid(), ConfigurationManager.toString(sr), configLocation});
          }
        }
      }
      ...
      if (factoryPid != null) {
        ...
        try {
          ...
          log(
              LogService.LOG_ERROR,
              "Failed removing {0} from the factory {1}",
              new Object[] {pid, factoryPid, ioe});
        }
      }
    }
      ...
      if (!srList.isEmpty()) {
        for (final ServiceReference<?> sr : srList) {
          ...
          if (srBundle == null) {
            log(
                LogService.LOG_DEBUG,
                "Service {0} seems to be unregistered concurrently (not processing)",
                new Object[] {ConfigurationManager.toString(sr)});
            continue;
          }
          ...
          if (wasVisible && !isVisible) {
            ...
            if (!this.provideReplacement(sr)) {
              ...
              log(
                  LogService.LOG_DEBUG,
                  "Configuration {0} revoked from {1} (no more visibility)",
                  new Object[] {config.getPid(), ConfigurationManager.toString(sr)});
            }
            ...
            log(
                LogService.LOG_DEBUG,
                "Configuration {0} provided to {1} (new visibility)",
                new Object[] {config.getPid(), ConfigurationManager.toString(sr)});
          } else {
            // same visibility as before
            log(
                LogService.LOG_DEBUG,
                "Unmodified visibility to configuration {0} for {1}",
                new Object[] {config.getPid(), ConfigurationManager.toString(sr)});
          }
        }
      }
  ...
  private class FireConfigurationEvent implements Runnable {
    ...
    private FireConfigurationEvent(
        final ServiceTracker listenerTracker,
        final int type,
        final String pid,
        final String factoryPid) {
      this.type = type;
    ...
    private void sendEvent(final int serviceIndex) {
      if ((listenerProvider[serviceIndex].getState() & (Bundle.ACTIVE | Bundle.STARTING)) > 0
        ...
        try {
          ...
          log(
              LogService.LOG_ERROR,
              "Unexpected problem delivering configuration event to {0}",
              new Object[] {ConfigurationManager.toString(listenerReferences[serviceIndex]), t});
        } finally {
          this.listeners[serviceIndex] = null;
        }
      }
