...
public class PasswordEncryptingPersistenceManager implements PersistenceManager {
  ...
  public PasswordEncryptingPersistenceManager(
  ...
  public Enumeration getDictionaries() throws IOException {
    ...
    while (dictionaries.hasMoreElements()) {
      ...
      String pid =
          (String)
              ((dictionary.get(SERVICE_FACTORYPID) == null)
                  ? dictionary.get(SERVICE_PID)
                  : dictionary.get(SERVICE_FACTORYPID));
      if (pid != null) {
        updatePasswords(pid, dictionary, encryptionService::decrypt);
      }
  ...
  private void updatePasswords(
      String pid, Dictionary dictionary, Function<String, String> passwordTransform) {
    ...
    ObjectClassDefinition objectClassDefinition =
        configurationAdmin.getObjectClassDefinition(trimmedPid);
