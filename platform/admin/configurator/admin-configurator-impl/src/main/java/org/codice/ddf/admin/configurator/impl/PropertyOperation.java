...
public abstract class PropertyOperation implements Operation<Void> {
  public static class Actions implements PropertyActions {
    ...
    @Override
    public PropertyOperation update(
        Path propFile, Map<String, String> configs, boolean keepIfNotPresent)
        throws ConfiguratorException {
      validatePropertiesPath(propFile);
      validateMap(configs, "Missing properties");
    ...
    public Map<String, String> getProperties(Path propFile) throws ConfiguratorException {
      validatePropertiesPath(propFile);
      return new PropertyOperation.UpdateHandler(propFile, Collections.emptyMap(), true)
          .readState();
    }
  }
  ...
  private PropertyOperation(
      Path configFile, Map<String, String> configs, boolean loadCurrentProps) {
