 ...
 *
 * <p>Sequentially processes {@link Operation}s, committing their changes. If a failure occurs
 ...
 * or with a failure to rollback - it returns a {@link OperationReport} of the outcome. In the case
 * of rollback failures, callers of this class should inform users of those failures so they may
 * manually intercede.
 *
 * <p>This class does not guarantee that it can reliably rollback changes in the case of failure. It
 * makes a best-effort to revert changes and reports the outcome.
 *
 * <p>To use this class, first instantiate then invoke the various methods for feature, bundle,
 * config, etc. updates in the order they should be applied. When all have been completed, call the
 * {@link #commit()} method to write the changes to the system. The resulting {@link
 * OperationReport} will have the outcome.
 */
public class ConfiguratorImpl implements Configurator {
   ...
   * committed.
   *
   * <p>In the case of a successful commit, the {@link SecurityLogger} will be invoked to log
   * changes.
   *
   * @param auditMessage In the case of a successful commit, the message to pass to the {@link
   *     SecurityLogger}
  ...
  private void rollback(
      UUID failedStep, OperationReport configReport, ConfiguratorException exception) {
    ...
    for (Map.Entry<UUID, Operation> row : undoStack) {
      try {
        ...
        if (operationData.isPresent()) {
          configReport.putResult(
              row.getKey(), ResultImpl.rollbackFailWithData(e, operationData.get()));
        } else {
