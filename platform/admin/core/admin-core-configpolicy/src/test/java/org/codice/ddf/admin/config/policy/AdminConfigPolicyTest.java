...
public class AdminConfigPolicyTest {
    ...
    public void testInvalidPolicyConfigs() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            KeyValuePermission featurePolicyPermission =
                    (KeyValuePermission) featurePolicyPermissions.get(0)
                            .getPermissionList()
                            .get(0);
            assertEquals(featurePolicyPermission.getKey(), TEST_ATTRIBUTE_NAME + i);
            assertEquals(featurePolicyPermission.getValues(),
                    Sets.newHashSet(TEST_ATTRIBUTE_VALUE + i));
        }
    }
    ...
    public void testValidateAllPermissions() {
        ...
        KeyValueCollectionPermission requestedFeaturePermissions = new KeyValueCollectionPermission(
        ...
        KeyValueCollectionPermission requestedServicePermissions = new KeyValueCollectionPermission(
                AdminConfigPolicy.VIEW_SERVICE_ACTION,
                getMatchOnePermissions(AdminConfigPolicy.SERVICE_PID).stream()
                        .toArray(KeyValuePermission[]::new));

        assertTrue(adminConfigPolicy.isPermittedMatchOne(getSubjectPermissions(),
                requestedFeaturePermissions)
                .isEmpty());

        assertTrue(adminConfigPolicy.isPermittedMatchOne(getSubjectPermissions(),
                requestedServicePermissions)
                .isEmpty());
    }
    ...
    public void testRemoveSomePermissions() {
        ...
        featurePolicyPermissions.add(
                TEST_PERMISSION_VALUE + UNAUTHORIZED + "=\"" + TEST_ATTRIBUTE_NAME + UNAUTHORIZED
                        + "=" + TEST_ATTRIBUTE_VALUE + UNAUTHORIZED + "\"");
        ...
        List<KeyValuePermission> matchOneFeaturePermissions = getMatchOnePermissions(
                AdminConfigPolicy.FEATURE_NAME);
        matchOneFeaturePermissions.add(new KeyValuePermission(AdminConfigPolicy.FEATURE_NAME,
                Sets.newHashSet(TEST_PERMISSION_VALUE + UNAUTHORIZED)));

        List<KeyValuePermission> matchOneServicePermissions = getMatchOnePermissions(AdminConfigPolicy.SERVICE_PID);
        matchOneServicePermissions.add(new KeyValuePermission(AdminConfigPolicy.SERVICE_PID,
                Sets.newHashSet(TEST_PERMISSION_VALUE + UNAUTHORIZED)));

        List<KeyValuePermission> matchOneInvalidActionPermission = new ArrayList<>();
        matchOneInvalidActionPermission.add(new KeyValuePermission("UNKNOWN_ACTION",
                Sets.newHashSet(TEST_PERMISSION_VALUE)));

        KeyValueCollectionPermission requestedFeaturePermissions = new KeyValueCollectionPermission(
                AdminConfigPolicy.VIEW_FEATURE_ACTION,
                matchOneFeaturePermissions.stream()
                        .toArray(KeyValuePermission[]::new));

        KeyValueCollectionPermission requestedServicePermissions = new KeyValueCollectionPermission(
                AdminConfigPolicy.VIEW_SERVICE_ACTION,
                matchOneServicePermissions.stream()
                        .toArray(KeyValuePermission[]::new));

        KeyValueCollectionPermission requestedInvalidActionPermissions =
                new KeyValueCollectionPermission("UNKNOWN_ACTION",
                        matchOneInvalidActionPermission.stream()
                                .toArray(KeyValuePermission[]::new));

        assertEquals(1, adminConfigPolicy.isPermittedMatchOne(getSubjectPermissions(),
                requestedFeaturePermissions)
                .getPermissionList()
                .size());
    ...
    public void testRemoveUnknownAttribute() {
        ...
        matchOneServicePermissions.add(new KeyValuePermission(AdminConfigPolicy.SERVICE_PID,
                Sets.newHashSet("UNKNOWN_ATTRIBUTE_NAME")));
        ...
        assertTrue(adminConfigPolicy.isPermittedMatchAll(getSubjectPermissions(),
                requestedServicePermissions)
                .isEmpty());
    }
    ...
    public List<String> getValidPolicyPermissions() {
        ...
        for (int i = 0; i < 3; i++) {
            policyPermissions.add(TEST_PERMISSION_VALUE + i + "=\"" + TEST_ATTRIBUTE_NAME + i + "="
                    + TEST_ATTRIBUTE_VALUE + i + "\"");
        }
    ...
    public List<KeyValuePermission> getMatchOnePermissions(String permissionKey) {
        List<KeyValuePermission> matchOneServicePermissions = new ArrayList<>();
        for (int i = 0; i < 3; i++) {
            matchOneServicePermissions.add(new KeyValuePermission(permissionKey,
                    Sets.newHashSet(TEST_PERMISSION_VALUE + i)));
        }
