...
public class InsecureDefaultsCollectorTest {
    ...
    public void validate() {
        ...
        Set<Serializable> details = (Set<Serializable>) captor.getValue()
                .getProperty(SystemNotice.SYSTEM_NOTICE_DETAILS_KEY);
        assertThat(details.size(), is(1));
        assertThat(details.iterator()
                .next(), is(BASE_ALERT_TEST_MESSAGE + 0));
    }
    ...
    private InsecureDefaultsCollector createInsecureDefaultsServiceBean(int validatorCount) {
        InsecureDefaultsCollector bean = new InsecureDefaultsCollector(eventAdmin) {
            @Override
            void addValidators() {
                return;
            }
