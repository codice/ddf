...
public class KeystoreValidator implements Validator {
    ...
    if (isInitialized()) {
      ...
      if (keystore != null) {
        ...
        if (!blacklistedCertificates.isEmpty()) {
          ...
          validateKeystoreCertificates(
              keystore, keystoreCertificateChains, blacklistedCertificates);
        }
      }
    }
  ...
  private boolean isInitialized() {
    ...
    if (StringUtils.isBlank(keystorePassword)) {
      ...
      alerts.add(
          new Alert(
              Level.WARN,
              String.format(GENERIC_INSECURE_DEFAULTS_MSG, keystorePath.toString())
  ...
  private List<Certificate> getBlackListedCertificates() {
    ...
    if (!new File(blacklistKeystorePath.toString()).canRead()) {
      alerts.add(
          new Alert(
              Level.WARN,
              String.format(
                  BLACKLIST_KEYSTORE_DOES_NOT_EXIST_MSG, keystorePath, blacklistKeystorePath)));
    ...
    try (FileInputStream fis = new FileInputStream(blacklistKeystorePath.toString())) {
      ...
      String msg =
          String.format(
              INVALID_BLACKLIST_KEYSTORE_PASSWORD_MSG,
              keystorePath,
  ...
  private void validateKeystoreCertificates(
      KeyStore keystore,
      List<Certificate[]> keystoreCertificateChains,
  ...
  private void validateCertificateChain(
      ...
      List<Certificate> blacklistedCertificates,
      KeyStore keystore) {
    ...
    for (Certificate certificate : certificateChain) {
      ...
      validateAgainstBlacklist(
          headCertificate, certificate, blacklistedCertificates, keystore, certificateChain.length);
    }
  }

  private void validateAgainstBlacklist(
      Certificate headCertificate,
