...
public abstract class CryptoPropertiesFileValidator extends PropertiesFileValidator {
  ...
  static final String KEYSTORE_PASSWORD_PROPERTY =
      "org.apache.ws.security.crypto.merlin.keystore.password";

  static final String KEYSTORE_ALIAS_PROPERTY =
      "org.apache.ws.security.crypto.merlin.keystore.alias";

  static final String PRIVATE_KEY_PASSWORD_PROPERTY =
  ...
  static final String COULD_NOT_FIND_PASSWORD_IN_PROPS_FILE_MSG =
      "Unable to determine if [%s] is using a default keystore password. Could not find password in [%s].";

  static final String COULD_NOT_FIND_ALIAS_IN_PROPS_FILE_MSG =
      "Unable to determine if [%s] is using a default keystore alias. Could not find keystore alias in [%s]";
  ...
  protected void validateKeystorePassword(Properties properties) {
    ...
    if (StringUtils.isBlank(defaultPassword)) {
      alerts.add(
          new Alert(
              Level.WARN, String.format(NO_DEFAULT_PASSWORD_PROVIDED_TO_VALIDATOR_MSG, path)));
    }
    ...
    if (StringUtils.isBlank(password)) {
      alerts.add(
          new Alert(
              Level.WARN,
              String.format(COULD_NOT_FIND_PASSWORD_IN_PROPS_FILE_MSG, path.toString(), path)));
    }
  ...
  protected void validateAlias(Properties properties) {
    ...
    if (StringUtils.isBlank(defaultAlias)) {
      alerts.add(
          new Alert(Level.WARN, String.format(NO_DEFAULT_ALIAS_PROVIDED_TO_VALIDATOR_MSG, path)));
    }
    ...
    if (StringUtils.equals(alias, defaultAlias)) {
      alerts.add(
          new Alert(
              Level.WARN,
              String.format(
                  DEFAULT_KEYSTORE_ALIAS_USED_MSG, KEYSTORE_ALIAS_PROPERTY, path, defaultAlias)));
    }
  }
