...
/** Tests the ApplicationStatusImpl class which retuns the stats of an application. */
public class ApplicationStatusImplTest {

  /** Tests that the app status is created properly and returns the correct values. */
  @Test
  public void testAppStatusCreation() {
    ...
    List<Feature> testFeatureList =
        new ArrayList<Feature>(Arrays.asList(testFeature1, testFeature2));
    ...
    ApplicationStatus testStatus =
        new ApplicationStatusImpl(testApp, testState, testFeatures, testBundles);
    ...
    assertTrue(
        "Sanity check for getErrorFeatures()",
        testStatus.getErrorFeatures().containsAll(testFeatureList));
    assertTrue(
        "Sanity check for getErrorBundles()",
