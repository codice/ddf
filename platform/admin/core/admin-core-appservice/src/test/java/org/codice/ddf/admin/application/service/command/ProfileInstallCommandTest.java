...
public class ProfileInstallCommandTest {
    ...
    public void setUp() throws Exception {
        ...
        Files.copy(this.getClass().getResourceAsStream("/profiles/invalidApp.json"), Paths.get(profilePath.toAbsolutePath().toString(), "invalidApp.json"));
        Files.copy(this.getClass().getResourceAsStream("/profiles/invalidFeatureInstall.json"), Paths.get(profilePath.toAbsolutePath().toString(), "invalidFeatureInstall.json"));
        Files.copy(this.getClass().getResourceAsStream("/profiles/invalidFeatureUninstall.json"), Paths.get(profilePath.toAbsolutePath().toString(), "invalidFeatureUninstall.json"));
        Files.copy(this.getClass().getResourceAsStream("/profiles/invalidStopBundles.json"), Paths.get(profilePath.toAbsolutePath().toString(), "invalidStopBundles.json"));
        ...
        when(featuresService.getFeature(anyString())).thenAnswer(invocation ->
                createMockFeature(invocation.getArguments()[0].toString()));
    ...
    public void testInstallPostInstallModuleFailure() throws Exception {
        ...
        doThrow(Exception.class).when(featuresService).installFeature("admin-post-install-modules", NO_AUTO_REFRESH);
        profileInstallCommand.profileName = "devProfile";
        profileInstallCommand.doExecute(applicationService, featuresService, bundleService);
    }
    ...
    private void verifyExtraProfileMocks() throws Exception {
        ...
        verify(featuresService, times(3)).installFeature(anyString(), (EnumSet<FeaturesService.Option>) any());
        ...
        inOrder.verify(featuresService).uninstallFeature(eq("feature3"), anyString(), eq(NO_AUTO_REFRESH));
        inOrder.verify(featuresService).uninstallFeature(eq("feature4"), anyString(), eq(NO_AUTO_REFRESH));
        ...
        inOrder.verify(featuresService).installFeature(eq("admin-post-install-modules"), eq(NO_AUTO_REFRESH));
    ...
    private void verifyMocksNoOp() throws Exception {
        ...
        verify(featuresService, times(0)).uninstallFeature(anyString(), anyString(), eq(NO_AUTO_REFRESH));
