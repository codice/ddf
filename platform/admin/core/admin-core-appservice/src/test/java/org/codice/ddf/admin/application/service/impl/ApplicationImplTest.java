...
/** Tests out the ApplicationImpl code to make sure it is following the interface specification. */
public class ApplicationImplTest {
  ...
  /**
   * Verify that the application is properly exposing the underlying repository.
   *
   * @throws Exception
   */
  @Test
  public void testAppGetters() throws Exception {
    RepositoryImpl repo =
        new RepositoryImpl(getClass().getClassLoader().getResource(FILE_NO_MAIN_FEATURES).toURI());
    repo.load();
  ...
  /**
   * Tests that if an application HAS a main feature that the properties in it are properly parsed
   * and set.
   *
   * @throws Exception
   */
  ...
  /**
   * Verifies if an app does NOT have a main feature that operations can still be performed.
   *
   * @throws Exception
   */
  @Test
