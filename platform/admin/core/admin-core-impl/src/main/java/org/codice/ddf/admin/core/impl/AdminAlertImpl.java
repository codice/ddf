...
public class AdminAlertImpl extends BasicMBean implements AdminAlertMBean {
    ...
    public List<Map<String, Object>> getAlerts() {
        ...
        alerts.sort((map1, map2) -> new Alert(map1).getPriority()
                .value() >= new Alert(map2).getPriority()
                .value() ? -1 : 1);
        return alerts;
    }
    ...
    private void escapeStrings(Map<String, Object> map) {
        for (Map.Entry<String, Object> entry : map.entrySet()) {
            if (entry.getValue() instanceof String) {
                entry.setValue(HtmlEscapers.htmlEscaper()
        ...
        for (String detail : details) {
            escapedStrings.add(HtmlEscapers.htmlEscaper()
                    .escape(detail));
        }
