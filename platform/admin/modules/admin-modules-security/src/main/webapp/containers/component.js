...
const Panel = ({disabled, id, path = [], label, description, children = []}) => (
  <div>
    <CardHeader style={{textAlign: 'center', padding: '0px 0px 0px 90px'}} title={label}
                subtitle={description} subtitleStyle={{marginTop: 7}}/>
    <CardActions>
...
const DescriptionIcon = ({description}) => {
  if (description) {
    return (<IconButton style={{width: '24px', height: '24px', display: 'inline-block', padding: '0px', marginLeft:'-24px', transform: 'translateY(7px)'}} tooltip={description} touch tooltipPosition='top-right'><InfoIcon /></IconButton>)
  } else {
    return null
  }
}
...
const ContextPanel = ({disabled, id, path = [], label, description, children = []}) => (
  <div>
    <Card>
      <CardHeader style={{textAlign: 'center', padding: '0px 0px 0px 90px'}} title={label}
                  subtitle={description} titleStyle={{marginTop: 7}}
                  subtitleStyle={{marginTop: 7}}/>
      <CardActions>
        <Flexbox style={{width: "100%"}} flexDirection='column' justifyContent='center'>{children.map((c, i) =>
...
const RadioButtons = ({ id, disabled, path, value = "undefined", options = {}, onEdit, defaultOption = {}}) => (
  <div style={{display: 'inline-block'}}>
    {Object.keys(options).map((itemKey, i) => (
      <SourceRadioButton key={i} value={itemKey} disabled={disabled} valueSelected={value.sourceType} {...options[itemKey]} onSelect={() => onEdit(path, options[itemKey])} />
      ))}
  </div>
)

const alertMessage = "This endpoint's SSL certificate is untrusted and possibly insecure."

const SourceRadioButton = ({ disabled, value, valueSelected, onSelect , certStatus }) => {
  if (certStatus == 'VERIFIED_CERT') {
    return (
      <div>
...
<div style={{textAlign: 'center', padding: 1}}>
    <div style={{ background: red500, color: 'white', margin: '0 auto', padding: 5, borderRadius: 3, textAlign: 'center', maxWidth: 300 }}>
    {label}
    </div>
</div>
)
...
<div style={{textAlign: 'center', padding: 1}}>
  <div style={{ background: yellow500, margin: '0 auto', padding: 5, borderRadius: 3, textAlign: 'center', maxWidth: 300 }}>
    {label}
  </div>
  </div>
)
...
<div style={{textAlign: 'center', padding: 1}}>
  <div style={{ background: green500, color: 'white', margin: '0 auto', padding: 5, borderRadius: 3, textAlign: 'center', maxWidth: 300 }}>
    {label}
  </div>
  </div>
)
...
const ListComponent = ({ disabled, path, id, label, description, children = [] }) => (
  <div style={{ height: 300, width: 525, overflow: 'scroll' }}>
    <List>
      {children.map((c, i) =>
...
const ListItemComponent = ({key, path, disabled, id, label, value, onEdit, attributes}) => (
  <Card key={key}>
    ...
    />
    <CardText expandable={true} style={{background: '#f2f2f2'}}>
