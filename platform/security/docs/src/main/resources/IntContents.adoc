...
The DDF Security IdP application provides service provider handling that satisfies the SAML 2.0 Web SSO profile in order to support external IdPs (Identity Providers).
...
These bundles are not installed by default, they can be stared by installing the `security-idp` feature.
...
. Under Authentication Types, set:
.. /search=SAML|IDP
