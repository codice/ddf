...
The DDF Security PDP module contains services that are able to perform authorization decisions based on configurations and policies.
...
* Match-All Mapping:  This mapping is used to guarantee that all values present in the attribute being asserted exist in the corresponding user attribute.

* Match-One Mapping:  This mapping is used to guarantee that at least one of the values present in the attribute being asserted exists in the corresponding user attribute.
