...
public class MetadataConfigurationParser {
    ...
    public static Map<String, EntityDescriptor> buildEntityDescriptors(
        ...
        for (String entityDescription : entityDescriptions) {
            ...
            if (entityDescription.startsWith(HTTPS) || entityDescription
                    .startsWith(HTTP)) {
                ...
                try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
                    ...
                    try {
                        ...
                        LOGGER.warn("Incorrectly configured URL for metadata: {}",
                                entityDescription, e);
                    }
                    if (httpResponse != null) {
                        ByteArrayInputStream byteStream = new ByteArrayInputStream(
                                httpResponse.getBytes());
                        addEntityDescriptor(entityDescriptors,
                                new InputStreamReader(byteStream, "UTF-8"));
                    }
                }
            } else if (entityDescription.startsWith(FILE + ddfHome)) {
                String pathStr = entityDescription.substring(FILE.length() - 1);
                ...
                addEntityDescriptor(entityDescriptors, new StringReader(entityDescription));
            } else {
    ...
    private static void addEntityDescriptor(Map<String, EntityDescriptor> entityDescriptors,
            Reader reader) {
        Document entityDoc;
        try {
        ...
        entityDescriptors.put(((EntityDescriptor) entityXmlObj).getEntityID(),
                (EntityDescriptor) entityXmlObj);
    }
}
