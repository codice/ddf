...
public abstract class ValidatorImpl implements Validator {
  ...
  public void validateAuthnRequest(
    ...
    if (authnRequest.getProtocolBinding() != null
        && !(authnRequest.getProtocolBinding().equals(Idp.HTTP_POST_BINDING)
            || authnRequest.getProtocolBinding().equals(Idp.HTTP_REDIRECT_BINDING)
            || authnRequest.getProtocolBinding().equals(Idp.SAML_SOAP_BINDING))) {
  ...
  protected void checkDestination(AuthnRequest authnRequest) throws ValidationException {
    if (authnRequest.getDestination() == null) {
      throw new ValidationException("Signed AuthnRequest must have a Destination attribute.");
        ...
        .getDestination()
        .equals(SystemBaseUrl.EXTERNAL.constructUrl("/idp/login", true))) {
