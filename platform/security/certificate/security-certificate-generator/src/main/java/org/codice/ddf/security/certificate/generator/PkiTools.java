...
public abstract class PkiTools {
   ...
   *
   * <ul>
   *   <li>email - email subject
   *   <li>URI - uniformed resource identifier
   *   <li>RID - registered id
   *   <li>DNS - hostname
   *   <li>IP - ip address (V4 or V6)
   *   <li>dirName - directory name
   * </ul>
   *
   ...
   * @throws IllegalArgumentException if the string is <code>null</code>, empty or cannot be parsed
   *     properly
   * @see <a href="https://www.openssl.org/docs/man1.0.2/apps/x509v3_config.html">OpenSSL X509 V3
   *     Configuration</a>
   * @see <a href="https://en.wikipedia.org/wiki/SubjectAltName">Subject Alternative Names on
   *     Wikipedia'</a>
   */
  public static GeneralName makeGeneralName(String name) {
    ...
    final String tag = name.substring(0, i).trim();
    final String gname = name.substring(i + 1).trim();
