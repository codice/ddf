...
public class WebSSOFilterTest {
    ...
    public void testDoFilterResolvingOnSecondCall() throws IOException, ServletException {
        ...
        verify(handler1, times(2)).getNormalizedToken(any(HttpServletRequest.class),
                any(HttpServletResponse.class),
                any(FilterChain.class),
                anyBoolean());
        // the next filter should NOT be called
        verify(filterChain, never()).doFilter(request, response);
        verify(request, never()).setAttribute(eq(DDF_AUTHENTICATION_TOKEN),
                any(HandlerResult.class));
    }
    ...
    public void testDoFilterWithRedirected() throws ServletException, IOException {
        ...
        when(handler1.getNormalizedToken(any(ServletRequest.class),
                any(ServletResponse.class),
                any(FilterChain.class),
                eq(true))).thenReturn(redirectedResult);
