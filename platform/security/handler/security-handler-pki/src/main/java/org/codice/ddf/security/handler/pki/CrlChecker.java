...
public class CrlChecker {
    ...
    private HandlerResult checkAgainstCrl(HttpServletResponse httpResponse,
            BaseAuthenticationToken token, X509Certificate[] certs, HandlerResult handlerResult) throws ServletException{
        if (passesCRL(certs)) {
            handlerResult.setToken(token);
            handlerResult.setStatus(HandlerResult.Status.COMPLETED);
        } else {
            ...
            try {
                ...
                throw new ServletException("Unable to send 401 response to client.", e);
            }
        }
        return handlerResult;
    }
