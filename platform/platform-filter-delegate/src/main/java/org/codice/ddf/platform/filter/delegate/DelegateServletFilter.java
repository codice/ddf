...
public class DelegateServletFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse,
        ...
        try {
            sortedSecurityFilterServiceReferences =
                    new TreeSet<>(bundleContext.getServiceReferences(SecurityFilter.class,
                            null));
        ...
        if (!CollectionUtils.isEmpty(sortedSecurityFilterServiceReferences)) {
            LOGGER.debug("Found {} filter(s), now filtering...",
            ...
            for (ServiceReference<SecurityFilter> securityFilterServiceReference : sortedSecurityFilterServiceReferences) {
                final SecurityFilter securityFilter = bundleContext.getService(
                        securityFilterServiceReference);

                if (!hasBeenInitialized(securityFilterServiceReference, bundleContext)) {
                    initializeSecurityFilter(bundleContext,
                            securityFilterServiceReference,
                            securityFilter);
                }
    ...
    private boolean hasBeenInitialized(
        ...
        return keysOfInitializedSecurityFilters.contains(getFilterKey(securityFilterServiceReference,
                bundleContext));
    }

    private void initializeSecurityFilter(BundleContext bundleContext,
            ServiceReference<SecurityFilter> securityFilterServiceReference,
            SecurityFilter securityFilter) throws ServletException {
        ...
        securityFilter.init(new SecurityFilterInitFilterConfig(securityFilterServiceReference,
                servletContext,
                bundleContext));
        keysOfInitializedSecurityFilters.add(getFilterKey(securityFilterServiceReference,
                bundleContext));
    ...
    /**
     * This logic to get the filter name from a {@link ServiceReference<Filter>} is copied from
     * {@link org.ops4j.pax.web.extender.whiteboard.internal.tracker.ServletTracker#createWebElement(ServiceReference, javax.servlet.Servlet)}.
     * See the pax-web Whiteboard documentation and
     * {@link org.osgi.service.http.whiteboard.HttpWhiteboardConstants#HTTP_WHITEBOARD_FILTER_NAME}
     * for how to configure {@link Filter} services with a filter name.
     */
    @Nonnull
    private static String getFilterName(
        ...
        final String filterNameFromTheServiceProperty = getStringProperty(
                securityFilterServiceReference,
        ...
        if (StringUtils.isBlank(filterNameFromTheServiceProperty)) {
            return bundleContext.getService(securityFilterServiceReference)
                    .getClass()
                    .getCanonicalName();
        } else {
    ...
    /**
     * This inner class is used to instantiate a {@link FilterConfig} from a
     * {@link SecurityFilter}'s service properties containing init params and the same
     * {@link ServletContext} as the {@link DelegateServletFilter}. The {@link FilterConfig} is used
     * to initialize the {@link SecurityFilter}.
     * <p>
     * The logic of this inner class is copied from the
     * {@link org.ops4j.pax.web.extender.whiteboard} feature.
     */
    private static class SecurityFilterInitFilterConfig implements FilterConfig {
        ...
        public String getFilterName() {
            return DelegateServletFilter.getFilterName(securityFilterServiceReference,
                    bundleContext);
        }
        ...
        /**
         * This code to create the init params from a {@link ServiceReference<Filter>} is copied
         * from
         * {@link org.ops4j.pax.web.extender.whiteboard.internal.tracker.ServletTracker#createWebElement(ServiceReference, javax.servlet.Servlet)}.
         * See the pax-web Whiteboard documentation and
         * {@link org.ops4j.pax.web.extender.whiteboard.ExtenderConstants#PROPERTY_INIT_PREFIX} for
         * how to configure {@link Filter} services with init params.
         */
        private Map<String, String> createInitParams() {
            ...
            String initPrefixProp = getStringProperty(securityFilterServiceReference,
            ...
            for (String key : initParamKeys) {
                String value = securityFilterServiceReference.getProperty(key) == null ?
                        "" :
                        securityFilterServiceReference.getProperty(key)
                                .toString();
                ...
                if (key.startsWith(initPrefixProp)) {
                    ...
                    initParameters.put(key.replaceFirst(HTTP_WHITEBOARD_SERVLET_INIT_PARAM_PREFIX, ""),
                            value);
                }
            }
