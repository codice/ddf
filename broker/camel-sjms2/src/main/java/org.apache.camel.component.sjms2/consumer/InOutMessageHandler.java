...
public class InOutMessageHandler extends AbstractMessageHandler {
    ...
    private final Map<String, MessageProducer> producerCache =
            new TreeMap<String, MessageProducer>();

    private final ReadWriteLock lock = new ReentrantReadWriteLock();

    public InOutMessageHandler(Sjms2Endpoint endpoint, ExecutorService executor) {
    ...
    public void handleMessage(Exchange exchange) {
        try {
            ...
            Object obj = exchange.getIn()
                    .getHeader(JmsConstants.JMS_REPLY_TO);
            if (obj != null) {
                Destination replyTo;
                if (isDestination(obj)) {
                    replyTo = (Destination) obj;
                } else if (obj instanceof String) {
                    replyTo = getEndpoint().getDestinationCreationStrategy()
                            .createDestination(getSession(), (String) obj, isTopic());
                } else {
                    throw new Exception(
            ...
            MessageHandlerAsyncCallback callback = new MessageHandlerAsyncCallback(exchange,
                    messageProducer);
            if (exchange.isFailed()) {
                ...
                if (isTransacted() || isSynchronous()) {
                    ...
                    // do so
                    log.debug("Synchronous processing: Message[{}], Destination[{}] ",
                            exchange.getIn()
                                    .getBody(),
                    ...
                    log.debug("Asynchronous processing: Message[{}], Destination[{}] ",
                            exchange.getIn()
                                    .getBody(),
                            getEndpoint().getEndpointUri());
    ...
    @Override
    public void close() {
        for (Map.Entry<String, MessageProducer> entry : producerCache.entrySet()) {
            try {
                entry.getValue()
                        .close();
            } catch (JMSException e) {
                log.debug("Cached MessageProducer with key:{} threw an unexpected exception",
                        entry.getKey(),
                        e);
            }
        }
    ...
    protected class MessageHandlerAsyncCallback implements AsyncCallback {
        ...
        public void done(boolean sync) {
            try {
                ...
                Message response = getEndpoint().getBinding()
                        .makeJmsMessage(exchange,
                                msg.getBody(),
                                msg.getHeaders(),
                                getSession(),
                                null);
                response.setJMSCorrelationID(exchange.getIn()
                        .getHeader(JmsConstants.JMS_CORRELATION_ID, String.class));
