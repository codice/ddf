...
import org.apache.camel.component.sjms2.Sjms2Producer;
 ...
 */
public class InOutProducer extends Sjms2Producer {

    private static final Map<String, Exchanger<Object>> EXCHANGERS =
            new ConcurrentHashMap<String, Exchanger<Object>>();
    ...
    public InOutProducer(Sjms2Endpoint endpoint) {
        super(endpoint);
    }
    ...
    protected void doStart() throws Exception {
        ...
        if (uuidGenerator == null) {
            ...
            uuidGenerator = getEndpoint().getCamelContext()
                    .getUuidGenerator();
        }
        if (consumers == null) {
            consumers =
    ...
    public MessageProducerResources doCreateProducerModel() throws Exception {
        ...
        try {
            ...
            MessageProducer messageProducer = JmsObjectFactory.createMessageProducer(session,
                    destination,
                    isPersistent(),
                    getTtl());
    ...
    public void sendMessage(Exchange exchange, AsyncCallback callback,
        ...
        Message request = getEndpoint().getBinding()
                .makeJmsMessage(exchange, producer.getSession());
        ...
        if (exchange.getException() == null) {
            if (responseObject instanceof Throwable) {
                ...
                SjmsMessage response = new SjmsMessage(message,
                        consumer.getSession(),
                ...
                exchange.setException(new CamelException(
                        "Unknown response type: " + responseObject));
            }
        }
    ...
    protected class MessageConsumerResourcesFactory
        ...
        public MessageConsumerResources makeObject() throws Exception {
            ...
            try {
                ...
                if (ObjectHelper.isEmpty(getNamedReplyTo())) {
                    replyToDestination = getEndpoint().getDestinationCreationStrategy()
