 ...
 */
package org.apache.camel.component.sjms2.threadpool;
...
public class ThreadPoolTest extends JmsTestSupport {
    ...
    protected RouteBuilder createRouteBuilder() throws Exception {
        return new RouteBuilder() {
            ...
            public void configure() throws Exception {
                from("direct:start").to("sjms:queue:foo")
                        .routeId(FROM_ROUTE);
                from("sjms:queue:foo").to("log:test.log.1?showBody=true")
                        .routeId(TO_ROUTE);
            }
        };
    }
    ...
    private void assertProducerThreadPoolCount(int count) throws Exception {
        Assert.assertEquals(count, getMbeanCount("\"InOnlyProducer"));
    }

    private void assertConsumerThreadPoolCount(int count) throws Exception {
        Assert.assertEquals(count, getMbeanCount("\"Sjms2Consumer"));
    }

    private int getMbeanCount(String name) throws MalformedObjectNameException {
        MBeanServer mbs = ManagementFactory.getPlatformMBeanServer();
        Set<ObjectInstance> mbeans = mbs.queryMBeans(new ObjectName(
                "org.apache.camel:type=threadpools,*"), null);
        LOGGER.debug("mbeans size: " + mbeans.size());
        int count = 0;
        for (ObjectInstance mbean : mbeans) {
            LOGGER.debug("mbean: {}", mbean);
            if (mbean.getObjectName()
                    .getKeyProperty("name")
                    .startsWith(name)) {
                count++;
            }
