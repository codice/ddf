...
public class SjmsDestinationCreationStrategyTest extends JmsTestSupport {
    ...
    public void testSjmsComponentUsesCustomDestinationCreationStrategy() throws Exception {
        Assert.assertFalse(createDestinationCalled);
        template.sendBody("sjms:queue:inonly?prefillPool=false", "hello world");
        Assert.assertTrue(createDestinationCalled);

        Assert.assertFalse(createTemporaryDestination);
        String response = (String) template.sendBody(
                "sjms:queue:inout?prefillPool=false&exchangePattern=InOut",
                ExchangePattern.InOut,
                "hello world 2");
        Assert.assertTrue(createTemporaryDestination);
