 ...
 */
package org.apache.camel.component.sjms2.tx;
...
public abstract class TransactedConsumerSupport extends CamelTestSupport {
    ...
    protected void runTest(String destinationName, int routeCount, int messageCount,
            int totalRedeliverdFalse, int totalRedeliveredTrue, int batchCount, int consumerCount,
            int maxAttemptsCount) throws Exception {
        ...
        CountDownLatch latch = new CountDownLatch(
                totalRedeliverdFalse * routeCount + totalRedeliveredTrue * routeCount);
    ...
    protected CamelContext createCamelContext() throws Exception {
        ...
        connectionFactory.getRedeliveryPolicy()
                .setInitialRedeliveryDelay(100);
        connectionFactory.getRedeliveryPolicy()
                .setRedeliveryDelay(100);
        connectionFactory.getRedeliveryPolicy()
                .setUseCollisionAvoidance(false);
        connectionFactory.getRedeliveryPolicy()
    ...
    protected void addRoute(String destinationName, int routeNumber, int batchCount,
            int consumerCount, int maxAttemptsCount, CountDownLatch latch) throws Exception {
        context.addRoutes(new RouteBuilder() {
            @Override
            public void configure() {

                if (context.getRoute("direct.route") == null) {
                    from("direct:start").id("direct.route")
                            .to(destinationName);
                }

                // Our test consumer route
                from(destinationName + "?transacted=true&transactionBatchCount=" + batchCount
                        + "&consumerCount=" + consumerCount).id("consumer.route." + routeNumber)
