    ...
    public void testConsumerTemplate() throws Exception {
        ...
        while (true) {
            Exchange ex = consumer.receiveNoWait(
                    "sjms:queue:consumer?messageSelector=SIZE_NUMBER<1500");
            if (ex != null) {
                Message message = ex.getIn();
                int size = message.getHeader("SIZE_NUMBER", int.class);
                Assert.assertTrue("The message header SIZE_NUMBER should be less than 1500",
                        size < 1500);
                Assert.assertEquals("The message body is wrong", "Message3", message.getBody());
            } else {
                break;
            }
        }
