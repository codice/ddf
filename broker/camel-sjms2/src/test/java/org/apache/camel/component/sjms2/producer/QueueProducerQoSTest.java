...
public class QueueProducerQoSTest extends JmsTestSupport {
    ...
    public void testInOutQueueProducerTTL() throws Exception {
        ...
        String endpoint = String.format(
                "sjms:queue:%s?ttl=1000&exchangePattern=InOut&responseTimeOut=500",
                TEST_INOUT_DESTINATION_NAME);

        try {
            template.requestBody(endpoint, "test message");
            Assert.fail("we aren't expecting any consumers, so should not succeed");
        } catch (Exception e) {
    ...
    protected RouteBuilder createRouteBuilder() throws Exception {
        return new RouteBuilder() {
            ...
            public void configure() throws Exception {
                from("sjms:topic:ActiveMQ.Advisory.Expired.Queue.>").routeId(
                        EXPIRED_MESSAGE_ROUTE_ID)
