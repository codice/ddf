...
public abstract class BatchTransactedProducerSupport extends CamelTestSupport {
    ...
    protected void runTest(String destinationName, int consumerRouteCount, int messageCount,
            int totalAttempts) throws Exception {
        ...
        CountDownLatch latch = new CountDownLatch(
                totalAttempts + messageCount * consumerRouteCount);
    ...
    protected void addRoute(String destinationName, int consumerRouteCount, CountDownLatch latch)
            throws Exception {
        context.addRoutes(new RouteBuilder() {
            @Override
            public void configure() {

                from("direct:start").id("producer.route")
                ...
                for (int i = 1; i <= consumerRouteCount; i++) {
                    from(destinationName).id("consumer.route." + i)
