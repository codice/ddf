...
const messages = (messages = [], {type, checked = false, expanded = false, id, message, ids}) => {
    switch (type) {
        case 'ADD_MESSAGE':
...
export const displayNumSelected = (data) => getSelectedIds(data).length + ' of ' + data.length + ' selected.'

export const getIds = (messages) => messages.map((message) => message.id)

export const getSelectedMessages = (messages) => messages.filter((message) => message.checked)

export const getSelectedIds = (messages) => getIds(getSelectedMessages(messages))

export const getExpandedIds = (messages) => getIds(messages.filter((message) => message.expanded))
