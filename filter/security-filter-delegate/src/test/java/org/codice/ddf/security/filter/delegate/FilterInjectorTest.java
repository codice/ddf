...
public class FilterInjectorTest {
    ...
    public void testInjectFilter() {
        ...
        // check that the filter was only registered once
        verify(curReference.getBundle().getBundleContext()).registerService(eq(Filter.class),
                eq(filter), Matchers.<Dictionary<String, Object>> any());

    }

    /**
     * Tests that the filter is unregistered when the removeFilter method is
     * called.
     */
    @Test
    public void testRemoveFilter() {
        ...
        injector.removeFilter(curReference);

        // duplicate call
        injector.removeFilter(curReference);

        // check that the filter was only unregistered once
        verify(curRegistration).unregister();

    }

    /**
     * Tests that all filters that were not previously unregistered are
