...
/**
 * This class tries to resolve all user given field names to their corresponding
 * dynamic Solr index field name. This class takes most of its logic directly
 * from the configured Solr schema.xml. For instance, the suffixes enumerated in
 * this class are directly copied from the schema.xml.
 * 
...
//			xmlInputFactory.setProperty(XMLInputFactory.IS_VALIDATING, Boolean.FALSE);
	...
	public DynamicSchemaResolver(SolrServer server) {
		...
		/*
		 * Adding this request handler allows us to query the schema
		 * dynamically. The name of the request handler is provided by the
		 * schema.xml. If the name is changed in the schema.xml, then this value
	...
	public void addFields(Metacard metacard, SolrInputDocument solrInputDocument) throws MetacardCreationException {
		...
		/*
		 * Lastly the metacardType must be added to the solr document. These are
		 * internal fields
		 */
		...
		if (metacardTypeBytes == null) {
			MetacardType coreMetacardType = new MetacardTypeImpl(schema.getName(), schema.getAttributeDescriptors());
	...
	/**
	 * Returns the best approximation as to what {@link AttributeFormat} this
	 * Solr Field is.
	 * 
	 * @param solrFieldName
	...
	public Serializable getDocValue(String solrFieldName, Object docValue) {
		...
		if (AttributeFormat.SHORT.equals(format)) {
			/*
			 * We have inside knowledge that user-given short objects are stored
			 * as Integers in Solr. You cannot cast an int to a short, so the
			 * workaround is to parse it. This should not lead to any loss of
			 * information because the value was originally a short.
			 */
	 ...
	 * 
	 * The convention is that we add a suffix starting with an underscore, so if
	 * we find the last underscore, then we can return the original field name.
	 * 
	 * @param solrFieldName
	 ...
	 * {@link AttributeFormat}
	 * 
	 * @param field
	 *            user given field name
	 * @return a list of possible Solr field names that match the given field.
	 ...
	 * @param format
	 *            {@link AttributeFormat} that describes the type of
	 *            {@link Attribute} the field is
	 * @param isSearchedAsExactValue
	 *            specifies if any special index suffixes need to be added to
	 *            the field
	 * @return the proper schema field name. If a schema name cannot be found in
	 *         cache, returns a schema field name that matches the dynamic field
	 *         type formatting.
	 */
	public String getField(String propertyName, AttributeFormat format, boolean isSearchedAsExactValue) {
		...
		LOGGER.info("Could not find exact schema field name for [" + propertyName + "], attempting to search with ["
				+ fieldName + "]");
	...
	/**
	 * Given xml as a string, this method will parse out element text and CDATA
	 * text. It separates each by one space character.
	 * 
	 * @param xmlData
