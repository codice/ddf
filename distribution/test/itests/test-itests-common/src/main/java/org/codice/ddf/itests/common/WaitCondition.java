 ...
 * using the {@link #within(long, TimeUnit)} and {@link #checkEvery(long, TimeUnit)} methods
 * respectively.
 *
 * <p>Usage examples (using Java 8 lambda expressions):
 *
 * <pre>
...
public class WaitCondition {
   ...
   * @param matchCondition matcher used to determine whether the condition has been met or not
   */
    ...
    try {
      ...
      while (!done) {
        ...
        if (System.currentTimeMillis() > timeoutLimit) {
          fail(
              String.format(
                  "%s: expectation not met after %d seconds",
                  description, TimeUnit.MILLISECONDS.toSeconds(timeoutMs)));
        }
      ...
      String message =
          String.format(
              "%s: Unexpected exception [%s] caught while waiting for expectation to be met",
              description, e);
  ...
  /**
   * Waits until the value returned by the condition is {@code true}, or the timeout expires. The
   * wait condition will be re-evaluated every polling interval. If the timeout expires or an
   * exception occurs while waiting, the current test will be failed.
   *
