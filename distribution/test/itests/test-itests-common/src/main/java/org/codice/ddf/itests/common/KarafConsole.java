 ...
 *
 * <p>Note: This class is needed to expose the protected methods provided by {@link
 * KarafTestSupport}. Since we already extend from our own base class, out test classes cannot
...
public class KarafConsole extends KarafTestSupport {
   ...
   */
  public KarafConsole(
      BundleContext bundleContext, FeaturesService featuresService, SessionFactory sessionFactory) {
    super();
  ...
  /**
   * Runs a shell command and returns output as a String. Commands have a default timeout of 10
   * seconds.
   *
   * @param command command to execute. Cannot be {@code null}.
   * @param timeout command timeout in milliseconds
   * @param principals principals (e.g. RolePrincipal objects) to run the command under (optional)
  ...
  /**
   * Runs a shell command and returns output as a String. Commands have a default timeout of 10
   * seconds. Uses the DEFAULT_ROLES to execute the command as an administrator.
   *
   * @param command command to execute. Cannot be {@code null}.
   * @return command output. Can be empty but not {@code null}.
   */
  ...
  public String runCommand(String command) {
    String response = executeCommand(command, 60000L, false, DEFAULT_ROLES);
