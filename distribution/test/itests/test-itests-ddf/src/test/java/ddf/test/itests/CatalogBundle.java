...
public class CatalogBundle {
    ...
    private <T extends Source> T waitForSource(String id, Class<T> type)
        ...
        while (!available) {
            if (source == null) {
                Collection<ServiceReference<T>> srcRefs = serviceManager.getServiceReferences(type,
                        null);
                for (ServiceReference<T> srcRef : srcRefs) {
                    T src = serviceManager.getService(srcRef);
