  ...
  public void bTestAppStatus() throws ApplicationServiceException, InterruptedException {
    ...
    systemSubject.execute(
        () -> {
          ...
          assertThat(
              "Application [" + REGISTRY_APP + "] should be INACTIVE",
              status.getState(),
              is(INACTIVE));
  ...
  public void cTestAppStartStop() throws ApplicationServiceException {
    ...
    systemSubject.execute(
        () -> {
          ...
          status = applicationService.getApplicationStatus(registry);
          assertThat(
              REGISTRY_APP + " should be ACTIVE after start, but was [" + status.getState() + "]",
              status.getState(),
              is(ACTIVE));
          try {
          ...
          status = applicationService.getApplicationStatus(registry);
          assertThat(
              REGISTRY_APP + " should be INACTIVE after stop", status.getState(), is(INACTIVE));
          ...
          assertThat(
              REGISTRY_APP + " should be empty response after " + START_COMMAND,
              response,
              isEmptyString());
          response = console.runCommand(STATUS_COMMAND + REGISTRY_APP);
          assertThat(
              REGISTRY_APP + " should be ACTIVE after " + START_COMMAND,
              response,
          ...
          response = console.runCommand(STATUS_COMMAND + REGISTRY_APP);
          assertThat(
              REGISTRY_APP + " should be INACTIVE after " + STOP_COMMAND,
              response,
