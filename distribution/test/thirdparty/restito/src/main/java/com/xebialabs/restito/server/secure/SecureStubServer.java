...
/**
 * This class can be removed if pull request https://github.com/mkotsur/restito/pull/46 is merged into master
 * this just extends StubServer to adds the functionality present in that pull request
 */
...
public class SecureStubServer extends StubServer {
    ...
    public SecureStubServer(int portRangeStart, int portRangeEnd, Stub... stubs) {
        ...
        simpleServer = HttpServer.createSimpleServer(null,
                new PortRange(portRangeStart, portRangeEnd));
    }
    ...
    public SecureStubServer(Behavior behavior) {
        this(behavior.getStubs()
                .toArray(new Stub[behavior.getStubs()
                        .size()]));
    }
    ...
    public SecureStubServer run() {
        simpleServer.getServerConfiguration()
                .addHttpHandler(stubsToHandler(), "/");
        try {
            if (secured) {
     ...
     */
    public int getPort() {
        return simpleServer.getListeners()
                .iterator()
                .next()
                .getPort();
    }
