...
==== Attribute Mixin Definition

Global attribute mixins are simply existing attributes that will be mixed (injected) into every metacard type. They allow you to extend metacard types with additional attributes even after those metacard types have been defined.

To define attribute mixins, your definition file must have a `mixins` key in the root object.
...
The value of `mixins` is simply a list of the attributes that should be injected into every metacard type.
