...
|\http://<DISTRIBUTION_HOST>:<DISTRIBUTION_PORT>/services/catalog?transform=<input transformer>

where <input transformer> is the name of the transformer to use when parsing metadata (optional).
...
|\http://<DISTRIBUTION_HOST>:<DISTRIBUTION_PORT>/services/catalog/<metacardId>?transform=<input transformer>

where `<metacardId>` is the `Metacard.ID` of the metacard to be updated
and <input transformer> is the name of the transformer to use when parsing an override metadata attribute (optional).
...
The FTP endpoint supports the PUT, MPUT, MKD, DELE, RETR, RMD, APPE, RNTO, STOU, and SITE operations.

The FTP endpoint supports files being upload as a dot-file (e.g., .foo) and then being renamed to
the final filename (e.g., some-file.pdf). The endpoint will complete the ingest process when
the rename command is sent. The endpoint also supports clients that create a directory,
change to the new directory and then upload a file.
 ...
 * `arguments`: (unlimited) [list of strings: each argument is one case-sensitive, valid enumeration value]
 ...
 - `iso3_country` (validates that a String is a valid ISO 3166-1 alpha-3 country code)
  * `arguments`: (0) [NO ARGUMENTS]
 - `iso3_countryignorecase` (validates that a String is a valid case-insensitive ISO 3166-1 alpha-3 country code)
  * `arguments`: (0) [NO ARGUMENTS]
  * NOTE: This is installed for the `location.country-code` attribute by default. In order to validate the `location.country-code` attribute with case sensitivity, the `iso3_country` validator must be manually installed.
