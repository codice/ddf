...
The Metacard Validity Marker Pre-Ingest plugin makes modifications to `CreateRequest` and `UpdateRequest` metacards.

The plugin runs the metacards contained in the `CreateRequest` and `UpdateRequest` with each service registered as a `MetacardValidator`.

[NOTE]
====
This plugin can make it seem like ingested products are not successfully ingested.
If after ingesting, the ingest did not fail but the expected results does not show up after a query,
