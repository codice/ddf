...
The KML Style Mapper is install by default with a standard installation in the <<_managing_ddf_spatial,${ddf-spatial} Application>>.
It is included in the `spatial-kml-transformer` feature and is installed by default with the Spatial App.
...
.[[spatial_kml_style_map_entry]]KML Style Mapper Configurable Properties
...
.KML Style Mapper Implementation Details
...
===== Known Issues with the KML Style Mapper
