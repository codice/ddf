...
Then change the LDAP Bind User Password in the <<Ldap_Login_Config, Security STS LDAP Login>> configurations to use the encrypted password.
...
All caching properties are part of the <<ddf.catalog.resource.download.ReliableResourceDownloadManager, Resource Download Settings>> configuration in the ${admin-console}.
