...
=== ${branding} Installation Prerequisites
...
// add windows version per https://codice.atlassian.net/browse/DDF-2481

* Change the current directory to the location of the zip file (${branding-lowercase}-X.Y.zip).
...
** `chown -R NON_ROOT_USER <${branding}_HOME>`
...
For example the `NON_ROOT_USER` should only have read access to `<${branding}_HOME.
====
