...
The default certificates will need to be replaced with certificates that use the fully-qualified hostname of the server running the ${branding} instance.
...
It also includes an entry for the DDF Demo Root CA.
...
The keys and certificates for the DDF demo root CA are included as part of the ${branding} distribution.
This entry must be removed from the truststore before ${branding} can operate securely.
See for more information about securely configuring ${branding}.
...
The above command creates a new entry in the server keystore.
It uses the hostname as the fully qualified domain name (FQDN) when it creates the certificate.
The Demo Certificate Authority signs the certificate so that it will be trusted by the default configuration.
