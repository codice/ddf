...
To set multi-valued attributes, use a comma separated list to list all values for a specified attribute. For example, to add the keywords _PPI_ and _radiology_ to each metacard, add the custom attribute mappings:
