...
${branding} is written in Java and requires a moderate amount of experience with the Java programming language, along with Java terminology, such as _packages_, _methods_, _classes_, and _interfaces_.
...
//TODO: This section is empty? add content or delete. DDF-1867

=== Building ${ddf-branding}
include::building-ddf-contents.adoc[]
...
|                      /-=------------------------------------------------------------------------\                     |
|                      | /----------------------\  /-------------------\ /----------------------\ |/-------------------\|
|   ${ddf-branding} Applications   | |cDEF  ${ddf-branding} Catalog     |  |cDEF${ddf-branding} Security   | |cDEF  ${ddf-branding} Content     | ||  New Application  ||
|                      | |                      |  |                   | |                      | |\-------------------/|
|                      | \----------------------/  \-------------------/ \----------------------/ \-------------------\ |
| /--------------------/ /------------------------------------------------------------------------------------------\ | |
| |       ${ddf-branding}            |cDEF                              ${ddf-branding} Framework                                           | | |
| \--------------------\ |                                                                                          | | |
...
Similar to its peers, ${branding} incorporates <<additional upstream dependencies>>.
...
${command-console}:: Provides command line administration of the OSGi container. All functionality in the {admin-console} can also be performed via this command line console.
...
Deployer:: Provides hot deployment of new bundles dropped into the `<INSTALL_DIR>/deploy` directory.
...
More information can be found at http://aries.apache.org/modules/blueprint.htm[Apache Aries].
...
Apache CXF:: Apache CXF is an open source services framework. CXF helps build and develop services using front end programming APIs, such as JAX-WS and JAX-RS. More information can be found at http://cxf.apache.org[CXF].
Apache Commons:: Provides a set of reusable Java components that extends functionality beyond that provided by the standard JDK  (More info available at http://commons.apache.org[Apache Commons]).
OSGeo GeoTools:: Provides spatial object model and fundamental geometric functions, which are used by ${branding} spatial criteria searches. More information can be found at http://geotools.org/[GeoTools].
Joda Time:: Provides an enhanced, easier to use version of Java date and time classes.
More information can be found at http://joda-time.sourceforge.net[Joda Time].
...
It is highly recommended to use a dependency injection framework for non-advanced OSGi tasks.
...
* The `<VERSION>` represents the version of the project.

.Export Example
[source,xml,linenums]
----
<Export-Package>
  <PROJECT/ARTIFACTID>*;version=<VERSION>
</Export-Package>
----
...
The ${branding} Security Framework uses http://shiro.apache.org/[Apache Shiro] as the underlying security framework.
...
|org.codice.${ddf-branding-lowercase}.security.handler.anonymous.AnonymousHandler
...
| |   ${ddf-branding}    |--------->| Servlet  |--------->|     Filter     |                             /---->|       |-->|->|Validators|->|  Claims  |->|  Token   | | |
...
The decision to filter the metacard eventually relies on the PDP (`features:install security-pdp-authz`).
