...
=== Configuring 1-way and 2-way SSL

1-way SSL is enabled out of the box. You can see this reflected in the org.ops4j.pax.web.cfg file at the line that reads:
...
2-way SSL is also supported out of the box if the client initiates a 2-way SSL connection with the server, however this 2-way SSL connection is not required by the server until a property is added to the org.ops4j.pax.web.cfg file that specifies that client authorization is needed instead of wanted. This can be done by adding the following line to org.ops4j.pax.web.cfg:
...
----
are specified, 2-way SSL will still be required by the server since the "clientauthneeded" property takes precedence.

==== SSL Port

The port for connecting over HTTPS can be changed by changing the port number assigned to the org.osgi.service.http.port.secure property in the org.ops4j.pax.web.cfg file. When changing this port number, be sure to verify that the chosen port does not conflict with any other secure ports being used in your network. For example, JBoss and other application servers use port 8443 by default.
...
Behind the scenes, the {branding} Standard UI invokes the REST endpoint to retrieve a resource.  In this request, it adds the query parameter "user" with the CometD session ID or the unique User ID as the value.  This allows the CometD server to know which subscriber is interested in the notification.  For example,  http://{branding}_HOST:8181/services/catalog/sources/ddf.distribution/2f5db9e5131444279a1293c541c106cd?transform=resource&user=1w1qlo79j6tscii19jszwp9s2i55 notifications contain the following information:
