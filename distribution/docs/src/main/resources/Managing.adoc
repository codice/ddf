...
. Add or modify a Web Context to use PKI in authentication. For example, enabling CRL for the search ui endpoint would require adding an authorization policy of `/search=SAML|PKI` or "/search=SAML|ANON" to allow anonymous access, as well as adding "/search=karaf" to the realms section and "/search=" to the required attributes section.
