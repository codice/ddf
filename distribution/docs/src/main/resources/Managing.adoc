...
Expansion defines the rules for mapping between the attributes found in records and the attributes assigned to users. The configuration files are found in the dib-enterprise-suite-$<version>/security directory. Specifically, edit the following files:

* `ddf-user-attribute-ruleset.cfg`
* `ddf-metacard-attribute-ruleset.cfg`

Add any desired mappings to the ruleset so they will be expanded correctly.
