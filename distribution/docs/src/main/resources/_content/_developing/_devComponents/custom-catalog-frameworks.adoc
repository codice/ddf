...
CUD operations process `PreIngestPlugin` instances before execution and `PostIngestPlugin` instances after execution.
...
----
Query requests process `PreQueryPlugin` instances before execution and `PostQueryPlugin` instances after execution.
