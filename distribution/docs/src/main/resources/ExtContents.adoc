...
First, the interceptor reads the configured policy, builds a security header, and gets a guest SAML assertion. Using this, it makes a `getSubject` call which is sent through Shiro to the STS realm. Upon success, the STS realm returns the subject and the call is made to the endpoint.
