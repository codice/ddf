...
${branding} supports ingest of many datatypes and commonly used file formats, such as Microsoft office products: Word documents, Excel spreadsheets, and PowerPoint presentations as well as .pdf files, GeoJson and others. See <<_all_formats_supported,complete list>>.
Many of these file types support additional <<_format_specific_attribute_mappings,format-specific attribute mappings>> from which additional metadata can be extracted.
