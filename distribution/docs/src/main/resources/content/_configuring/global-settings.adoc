...
|http://localhost:${solr.http.port}/solr
|Yes

|${branding} manages starting/stopping Solr server
|solr.managed
|boolean
|true
|Set to true if ${branding} manages starting and stopping of Solr server. Set to `false` if Solr is externally managed.
|Yes
