...
. Under *Authentication Types*, set the IdP authentication type to context paths as necessary.
Note that it should only be used on context paths that will be accessed by users via web browsers. For example:
* `/search=IdP`
...
When using the included IdP, ${branding} can be configured to use the <<{managing-prefix}configuring_included_sts,included Security Token Service(STS)>> or an <<{managing-prefix}connecting_to_external_sts,external STS>>.
