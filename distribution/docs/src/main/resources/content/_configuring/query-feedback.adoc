...
. First, configure the *Email Service* to point to a mail server. See <<{application-prefix}org.codice.ddf.platform.email.impl.SmtpClientImpl,Email Service Configurations>>.
...
See <<{application-prefix}org.codice.ddf.catalog.ui.config,Catalog UI Search Configurations>> for default Query Feedback configurations.