...
. First, configure the *Email Service* to point to a mail server. See <<org.codice.ddf.platform.email.impl.SmtpClientImpl,Email Service Configurations>>.
...
|`{{auth_username}}`
|Username of the security subsystem (see <<_security_framework,Security Framework>>)
...
See <<org.codice.ddf.catalog.ui.config,Catalog UI Search Configurations>> for default Query Feedback configurations.