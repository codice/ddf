...
Equivalent addition and deletion of a map layer can be found in <<_map_configuration_for_intrigue,Map Configuration for ${catalog-ui}>>.
...
See <<org.codice.ddf.catalog.ui.config,Catalog UI Search Configurations>> for examples of map layer configurations.
