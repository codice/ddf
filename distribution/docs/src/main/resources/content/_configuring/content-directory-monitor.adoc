...
Attributes will only be overridden if they are part of the <<_metacard_type, metacard type>>, however the attributes do not have to be present on the metacard to be set.
...
To override attributes that are not part of the metacard type, <<_attribute_injection,attribute injection>> can be used.
