...
.File Size Limit
CDM will not ingest files beyond a certain size. By default, the maximum file size is 1 GB.
[TIP]
====
The size limit can be adjusted by setting the following in *etc/custom.system.properties*
`org.codice.ddf.catalog.content.monitor.maxFileSizeBytes=[maximum]`

Set the maximum file size as a number of bytes.
====
