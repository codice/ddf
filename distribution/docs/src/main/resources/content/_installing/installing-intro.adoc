...
3+^|Minimum and Recommended Requirements for ${branding} Systems^&dagger;^
...
|Operating System
|Windows 10, Linux CentOS 5
|Windows Server 2012, Linux CentOS 7.3

|Additional Software
|JDK 8 x64, IE 10
|JDK 8 x64, Edge/FireFox/Chrome
...
^*^If installing a test, demo, or development installation, see the <<_quick_install_prerequisites,Quick Start Installation Prerequisites>>.

^&dagger;^These requirements assume the use of a remote catalog. Extra resources will need to be allocated if the system will also be hosting a local catalog.

^&Dagger;^The amount of RAM can be increased to support memory-intensive applications. See <<jvm-memory-configuration, Memory Considerations>>
