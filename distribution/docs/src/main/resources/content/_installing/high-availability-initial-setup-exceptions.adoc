...
In order to integrate with an failover proxy, the ${branding} node's system properties (in `${home_directory}/etc/system.properties`) must be changed to publish the correct port to external systems and users.
