...
The Catalog Framework then invokes <<_catalog_plugins,Catalog Plugins>>, <<_transformers,Transformers>>, and <<_resources,Resource Components>> as needed before sending requests to the intended destination, such as one or more <<_connecting_to_sources,Sources>>. 
