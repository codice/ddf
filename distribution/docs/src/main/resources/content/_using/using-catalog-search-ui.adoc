...
Panels are different ways to view search results.

. Click the *Add Panel* (image:add-visual-icon.png[add visual icon]) icon in the top right corner of ${catalog-ui}.
...
The *Inspector* panel provides in depth information and actions for each search result.
...
Results can be edited from the *Summary* or *Details* tabs in the *Inspector* panel.

==== Viewing Text Previews

If a preview for a result is available, an extra tab will appear in the *Inspector* panel that allows you to see a preview of the resource.
