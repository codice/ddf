...
<<_core_attributes,Core Attributes>>
...
[WARNING]
====
Every <<_introduction_to_federation_and_sources,Source>> should at the very least return an ID attribute according to Catalog API.
...
It maintains a list of all supported `MetacardTypes` in the `CatalogFramework`, so that other components such as <<_endpoints,Endpoints>>, <<_types_of_plugins,Plugins>>, and <<_transformers,Transformers>> can make use of those `MetacardTypes`. 
