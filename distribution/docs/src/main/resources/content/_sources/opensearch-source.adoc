...
See <<{application-prefix}OpenSearchSource,OpenSearch Source configurations>> for all possible configurations.
...
|Pulled from the query if it is a bounding-box query. +
 +
Or else, calculated from the query if it is a single geometry or polygon query and the <<{application-prefix}shouldConvertToBBox,`shouldConvertToBBox`>> configuration option is `true`.
NOTE: Converting a polygon that crosses the antimeridian to a bounding box will produce an incorrect bounding box. +
//TODO DDF-3742
 +
Or else, calculated from the query if it is a geometry collection and the <<{application-prefix}shouldConvertToBBox,`shouldConvertToBBox`>> configuration option is `true`.
