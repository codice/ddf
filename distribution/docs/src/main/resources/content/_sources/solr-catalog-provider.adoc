...
.[[_internal_solr]]Internally Managed Solr Server (default)::
${branding} is bundled with a customized distribution
...
.[[_external_solr]]Externally Managed Solr Server::
In this configuration, ${branding} communicates with a
...
* The externally managed Solr server can be located on an external host. ${branding} uses
