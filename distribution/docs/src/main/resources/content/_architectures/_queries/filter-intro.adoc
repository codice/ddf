...
That package name was changed to `org.geotools.api.filter` in version 30.0 and is currently used by ${branding}.
...
Note that the `ddf.catalog.operation.Query` interface extends the `org.geotools.api.filter.Filter` interface, which means that a Query object is an OGC Java Filter with Query Options.
