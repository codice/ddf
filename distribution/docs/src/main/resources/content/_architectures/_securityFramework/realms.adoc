...
:children: Claims Handlers, Security JAAS LDAP
:order: 11
:summary: Security realms.
...
Realms are implementations of http://shiro.apache.org/[Apache Shiro]'s AuthenticatingRealm class.
They can be used to add additional authentication mechanisms to ${branding}.
Many realms already exist, such as a UsernamePasswordRealm, OidcRealm, SamlRealm, and others to handle common authentication and Single Sign On (SSO) protocols.

Additionally, multiple realms can be used to authenticate a user.
For example, guest permissions are implemented via a Realm and will run alongside any other realms to add guest user attributes to the current Subject being authenticated.
...
|This realm validates SAML assertions found on the incoming Authentication Token.
|===
