...
|[[_solr_autopassword]]Allow ${branding} to change the Solr server password if it detects the default password is in use
|solr.attemptAutoPasswordChange
|Boolean
|If true, ${branding} attempts to change the default Solr server password to a randomly
generated UUID. This property is only used if the `solr.client` property is `HttpSolrClient`
and the `solrBasicAuth` property is `true`.
|true
|Yes
