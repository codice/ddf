...
Use the <<{integrating-prefix}encryption_service,Encryption Service>>, from the ${command-console} to set passwords for your LDAP server.
Then change the LDAP Bind User Password in the <<{application-prefix}Claims_Handler_Manager, Security STS LDAP and Roles Claims Handler>> configurations to use the encrypted password.
...
See the <<{application-prefix}Claims_Handler_Manager,Security STS LDAP and Roles Claims Handler>> for all possible configurations.
