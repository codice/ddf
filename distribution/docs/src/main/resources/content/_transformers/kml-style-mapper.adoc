...
The KML Style Mapper is installed by default with a standard installation in the <<_spatial_application_reference,${ddf-spatial} Application>> in the `spatial-kml-transformer` feature.
...
See <<org.codice.ddf.spatial.kml.style,KML Style Mapper configurations>> for all possible configurations.
