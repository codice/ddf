...
It is installed with the <<{application-prefix}installing_registry,${ddf-registry}>> application.
...
See <<{application-prefix}org.codice.ddf.registry.policy.RegistryPolicyPlugin,Registry Policy Plugin configurations>> for all possible configurations.
