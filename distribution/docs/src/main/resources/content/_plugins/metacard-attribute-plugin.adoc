...
The Metacard Attribute Security Policy Plugin combines existing metacard attributes to make new
 attributes and add them to the metacard. For example, if a metacard has two attributes,
`sci-tokens` and `codewords`, the values of the two attributes could be combined into a new
 attribute, `compartments`. The `sci-tokens` and `codewords` are the _source attributes_ and `compartments` is the _destination attribute_.

There are two way to combine the values of source attributes. The first, and most common, is to take of all the attributess values and put them together. This is called the union. For example, if the source attributes `sci-tokens` and `codewords` had the values:
...
The other way to combine attributes is use on the values common to all of the attributes. This is called the intersection. Using our previous example, the **intersection** of `sci-tokens` and `codewords` would create the new attribute `compartments`
...
The policy plugin could also be used to rename attributes. If there is only one source attribute,
 and the combination policy is union, then the attributes values are effectively renamed to the
...
attribute, `compartments` using the union, enter these two lines under the title **Metacard Union Attributes**
