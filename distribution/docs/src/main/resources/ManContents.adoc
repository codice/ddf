...
* Configure `<DDF_HOME>/etc/ws-security/ser/signature.properties`
[source,bash]
----
#Property descriptions at http://ws.apache.org/wss4j/config.html
org.apache.ws.security.crypto.provider=org.apache.ws.security.components.crypto.Merlin
...
[NOTE]
====
Enabling CRL revocation will require a restart of {BRANDING}.
====
...
With anonymous access, a user with a revoced cert will be given a 401 error, but users without a certificate will be able to access the web context as the anonymous user.

[NOTE]
======
Restarting {BRANDING} is not required is the CRL is edited. The only time restarting is required is if CRL property in `encryption.properties` is modified. For example, when enabling or disabling CRL revocation or if moving or renaming the CRL file.
======
