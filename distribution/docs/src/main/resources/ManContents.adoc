...
==== Configuring Manage Service Factory bundles
...
Managed Service Factory PID, be followed by a sequential number and have a `.config` extension. For instance,
assuming that the Managed Service Factory PID is `org.codice.ddf.factory.pid` and two instances of the
service need to be configured, files `org.codice.ddf.factory.pid.1.config` and `org.codice.ddf.factory.pid.2.config`
need to be created and added to `<DDF_HOME>/etc`.
