...
The default constraints are ".*", which allows any connection with a trusted certificate access to the STS. This opens up the vulnerability for another client to "pretend" to be another client.
Provide constraints by modifying this property to be a list of regular expressions that map to the fully qualified hostnames of systems that should be allowed to contact the STS.
