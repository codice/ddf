...
[IMPORTANT]
====
Certain bundles can only be configured using the new `.config` file format. Please refer to the Configuring {branding} using Configuration (.config) Files section for more details.
====
...
The {branding} can also be configured using configuration `.config` files. Like the Karaf `.cfg` files, these configuration files must be located in the `<DDF_HOME>/etc/` directory and have a name that matches the configuration persistence ID (PID) they represent.

This type of configuration file supports lists within configuration values and should be used for any configuration that makes use of lists (metatype `cardinality` attribute greater than 1).

[IMPORTANT]
====
This new configuration file format *must* be used to configure the following bundles:

* Web Context Policy Manager (PID: `org.codice.ddf.security.policy.context.impl.PolicyManager`)
* Security STS Anonymous Claims Handler (PID: `ddf.security.sts.anonymousclaims`)
====

[WARNING]
====
Currently the `.config` configuration files do not support configuration of ManagedServiceFactories. To prevent any configuration issues, it is advised that this method be used mainly to configure services that are not created by a ManagedServiceFactory and that need to contain lists of values.
====
...
The syntax of the `.config` configuration files is similar to the older `.cfg` files but introduces support for lists and types other than simple strings. The type associated with a property must match the `type` attribute used in the corresponding `metatype.xml` file when applicable.
...
|Lists of Strings
|`name = [ "value1", "value2", ... ]`
|`authenticationTypes = [ "/\=SAML\|ANON", "/admin\=SAML\|basic", "/jolokia\=SAML\|basic", "/system\=basic", "/solr\=SAML\|PKI\|basic", "/sources\=SAML\|basic", "/security-config\=SAML\|basic" ]`
...
authenticationTypes=["/\=SAML|ANON","/admin\=SAML|basic","/jolokia\=SAML|basic","/system\=basic","/solr\=SAML|PKI|basic","/sources\=SAML|basic","/security-config\=SAML|basic","/search\=basic"]
realms=["/\=karaf"]
requiredAttributes=["/\=","/admin\={http://schemas.xmlsoap.org/ws/2005/05/identity/claims/role\=admin}","/solr\={http://schemas.xmlsoap.org/ws/2005/05/identity/claims/role\=admin}","/jolokia\={http://schemas.xmlsoap.org/ws/2005/05/identity/claims/role\=admin}","/system\={http://schemas.xmlsoap.org/ws/2005/05/identity/claims/role\=admin}","/sources\={http://schemas.xmlsoap.org/ws/2005/05/identity/claims/role\=admin}","/security-config\={http://schemas.xmlsoap.org/ws/2005/05/identity/claims/role\=admin}"]
whiteListContexts=["/services/SecurityTokenService","/services/internal/metrics","/services/saml","/proxy","/services/csw"]
----

==== Pre-Configuring {branding}

The main purpose of the configuration files is to allow administrators to pre-configure {branding} without having to use the admin console. In order to do so, simply extract the {branding} zip file, copy the configuration files in the `<DDF_HOME>/etc` directory and start the application. The system will then configure itself using the values set in the properties files.

Once {branding} is up and running, the `.config` configuration files that existed in `<DDF_HOME>/etc` at startup will be automatically updated to reflect any changes made using the admin console. This allows administrators to capture any configuration changes made at runtime.

==== Dynamic Configuration File Updates

{branding} will also monitor any changes made to the configuration files and update the itself accordingly. Those changes will be automatically reflected in the admin console.

Creating a new configuration file in the `<DDF_HOME>/etc` directory will automatically create the corresponding configuration in the admin console if it did not already exist, or update the existing one. From that point on, the configuration file will be tracked and any update reflected in the admin console.

[IMPORTANT]
====
Even though file updates and deletions are monitored and handled, it is still strongly recommended to make configuration changes when the application is running using the admin console. Changes made directly to the configuration files while the system is running could conflict with changes made through the admin console and corrupt the system' configuration.
====
...
. If anonymous access is required, add "ANON" to the policy. Ex, `/search=PKI|ANON`.
