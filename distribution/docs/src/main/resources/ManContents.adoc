...
=== Configure {branding} with New Certificates
{branding} ships with a default security certificate configured to identify the {branding} instance machine as "localhost." This allows the {branding} distribution to be unzipped and run immediately in a secure manner. However, in order to access the {branding} instance from another machine over HTTPS (now the default for many services) the default certificates need to be replaced with a certificate that uses the fully qualified hostname of the server running the {branding} instance.
...
|JKS
|Java *keystore*. A dictionary of cryptographic objects (e.g. private keys, certificates) references by an *alias*. The JKS format is specific Java.
|
...
By default Solr, STS server, STS client and platform global config settings point to *localhost* and not to the fully qualified domain name of the {branding} instance. Assuming the {branding} instance is providing these servcies, the configuration must be updated to use the {branding} instance's *fully qualified domain name*  as the service provider.

This can be done throught the *Admin UI* application, the *Felix console*, or from the {branding} console. The most common method is to use the *Admin UI*.

[IMPORTANT]
====
The process of changing the configuration can cause users to loose access to the {branding} instance via the Web. This includes losing access to the *Admin UI* and the *Felix console*. Without access to these bundles, it is not possible to configure the {branding} instance through a Web browser.
====
...
[NOTE]
====
The Security STS Server Signature Username and Encryption Username properties are the *alias* of the server's private key entry in the Java keystore. _The server's FQDN is used as the alias_. This is repeated in other palces where an alias is configured.
====
...
* Configure `<DDF_HOME>/etc/ws-security/issuer/encryption.properties`
[source,bash]
----
...
* Configure `<DDF_HOME>/etc/ws-security/issuer/signature.properties`
[source,bash]
----
#Property descriptions at http://ws.apache.org/wss4j/config.html
org.apache.ws.security.crypto.provider=org.apache.ws.security.components.crypto.Merlin
...
* Configure `<{DDF_HOME>/etc/ws-security/server/encryption.properties`
[source,bash]
----
#Property descriptions at http://ws.apache.org/wss4j/config.html
org.apache.ws.security.crypto.provider=org.apache.ws.security.components.crypto.Merlin
...
* Configure `<DDFHOME>/etc/ws-security/ser/signature.properties`
[source,bash]
----
#Property descriptions at http://ws.apache.org/wss4j/config.html
org.apache.ws.security.crypto.provider=org.apache.ws.security.components.crypto.Merlin
...
A new `.crt` file is writen to the current directory. y default, the certificate is valid for one year. This period can be changed using `openssl` command line options or by editing the `openssl` configuration file, `openssl-demo.cnf`.
 ...
 * Concatenate the certificate from the issuer, from all intermediate certificate authorities, and the root authority into a text file.
