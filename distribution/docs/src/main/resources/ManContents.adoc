...
The {branding} can also be configured with configuration (.config) files.

[NOTE]
====
This type of configuration file supports lists within configuration values and should be used for any configuration that makes use of lists
====

The syntax of the (.config) configuration files is similar to the older (.cfg) files but introduces support for lists and types other than simple strings. The type associated with a property should match the _type_ attribute used in the corresponding _metatype.xml_ file when applicable.
...
[NOTE]
====
* Lists of values can be of any of the supported types, namely integers (prefixed with _I_ as in the example above), booleans (prefixed with _B_), floats (prefixed with _F_)or  doubles (prefixed with _D_)
* The backslashes before the equals signs are generally a good practice in order to avoid running into configuration issues
====
...
property.int = I"10"
property.float = F"10.0"
property.double = D"10.01"
property.long = L"10"
...
[WARNING]
====
Currently the configuration (.config) file method does not support configuration of ManagedServiceFactories, for safety it is advised that this method is used mainly for configurations that need to contain lists of values
====
