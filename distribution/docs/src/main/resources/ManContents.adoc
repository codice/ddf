...
Using absolute paths in those cases will cause the `platform:config-export` command to display warnings about those files and exclude them from the export process.
