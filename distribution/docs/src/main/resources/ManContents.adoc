...
[NOTE]
======
Restarting {BRANDING} is not required if the CRL is edited. The only time restarting is required is if CRL property in `encryption.properties` is modified. For example, when enabling or disabling CRL revocation or if moving or renaming the CRL file.
======
