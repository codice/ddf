...
* All configurations editable in the Web Admin Console will be exported to `<DDF_HOME>/etc/exported/etc`.
...
Using absolute paths in those cases will cause the `platform:config-export` command to throw warnings about those files and exclude them from the export process.
The export process itself is not aborted. 
