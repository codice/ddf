...
{branding} uses public key infrastructure known as X.509 or PKIX. It is documented in the IEFT RFC 5280.
...
|keytool
|The Java keytool is a key and certificate management utility.
|
...
[IMPORTANT]
====
When updating the Solr URL's be sure to include "/solr" at the end of the URL.
====
+
[cols="3" options="header"]
|===
...
[NOTE]
====
The Security STS Server Signature Username and Encryption Username are the keystore alias that points to the {branding} instance's private key entry. The alias used for this key is the instance's FQDN. That is why the <FQDN> is used as the configured value.
====
...
org.apache.ws.security.crypto.merlin.keystore.alias=<FQDN>
...
The command encrypts the private key to protect it. The command prompts for a pass phrase that will decrypt the private key:

----
writing new private key to '....key'
Enter PEM pass phrase:
----
Enter a pass phrase (the default is `changeit`).

The command prompts for identity information:
...
* The passphrase must match the passphrase configured in the `.properties` files.  The default passphrase is `changeit`.
...
//TODO:
//If you want to create your own CA and sign your own request see the section on <<Certificate Management>>
...
A new `.crt` file is writen to the current directory. y default, the certificate is valid for one year. This period can be changed using `openssl` command line options or by editing the opensll configuration file, `openssl-demo.cnf`.

==== Create PKCS#12 File
The private key and certificate need to be combined into a new structure that can be imported into the keystore. A file that conforms to the PKCS#12 specification is created as an intermediary step in the process.

=====Creating the Chain of Trust

 * If the certificate was signed by the Demo CA, skip this step. It is necessary because `openssl` automatically adds the issuer's (signer's) certificate to the PKCS12 file to create the complete chain of trust.

 //TODO: Ask Scott under what circumstances this step is not necessary

 * Concatenate all the issuer's certificate, all intermediate certificates, and the root authority's certificate into a text file.
...
[NOTE]
====
The argument for `-passin` is the password that was used to encrypt the private key. The argument for `-passout` sets the password used to decrypt the PKCS#12 file.
====
...
==== Remove localhost Private Key Entry
...
==== Import CA Certificates into Key Store
...
==== Import CA Certificates into Trust Store
...
//TODO: I have a question out to Scott if this is still necessary.
//==== Configure hostname mapping
//In `/etc/hosts` add the following mapping:
//
//* 127.0.0.1       <FQDN>

==== Restart Server Porcesses
Rstart the {branding} instance:
[source,bash]
----
$> cd <{branding}_HOME>/bin
$> ./ddf
----
