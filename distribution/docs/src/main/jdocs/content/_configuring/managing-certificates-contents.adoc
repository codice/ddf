...
. If provided with a server keystore, start at <<_adding_an_existing_server_keystore,Adding an Existing Server Keystore>>.
. If provided with a signed certificate and private key, start at <<_creating_a_new_keystore_truststore_with_an_existing_certificate_and_private_key,Creating a New Keystore/Truststore with an Existing Certificate and Private Key>>.
