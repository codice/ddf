...
Metrics are also collected on a per source basis for each configured <<_federated_sources,Federated Source>> and <<_catalog_providers,Catalog Provider>>.
...
Therefore, it is possible to access metrics data for sources renamed months ago, i.e., until ${branding} is reinstalled or the metrics data is deleted from the `<${branding}_HOME>/data/metrics` directory.
