...
An <<_endpoints, endpoint>> invokes the active Catalog Framework, which calls any configured <<_pre_query_plugins, Pre-query>> or <<_pre_ingest_plugins, Pre-ingest plug-ins>>.
The selected <<_federation_strategy, federation strategy>> calls the active <<_catalog_providers, Catalog Provider>> and any connected or federated sources.
...
The Catalog Framework then invokes <<_catalog_plugins,Catalog Plugins>>, <<_transformers,Transformers>>, and <<_resources,Resource Components>> as needed before sending requests to the intended destination, such as one or more <<_sources,Sources>>. 
...
If caching is enabled, each time a retrieve product request is received, the Catalog Framework will look in its cache (default location `<${branding}_HOME>/data/product-cache`) to see if the product has been cached locally.
