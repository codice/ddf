...
Additionally, for more details about the health of the cluster, the following command can be run using curl or a https://localhost:8993/admin/jolokia/read/org.apache.activemq.artemis:broker=%22localhost%22,component=cluster-connections,name=%22my-cluster%22/Topology[browser].
...
Once the STS LDAP Login and Claims Handlers are configured, update `<${branding}_HOME>/etc/org.apache.activemq.artemis.cfg` to use the `ldap` realm (just change `domain=karaf` to `domain=ldap`):
...
but ${branding} is still using the default security settings.
The Web Context Policy Manager can be configured to use the same `ldap` realm to secure ${branding}:
