# Creating A Sample Search Form

In order to create a sample search form, you must place a `forms.json` file within the `/etc/forms` directory.

## Sample Forms JSON blob

```$xslt
[
  {
    "title": "Contact Name",
    "description": "A convenient single-field form for searching across all types of contact names.",
    "filterTemplateFile": "contact-name.xml"
  }
]
```

Simply add a title, description and filepath to the actual XML representing the query structure associated with the form. The associated XML
would look something like the following:

```$xslt
<?xml version="1.0" encoding="UTF-8"?>
<fes:Filter xmlns:fes="http://www.opengis.net/fes/2.0"
            xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
            xsi:schemaLocation="http://www.opengis.net/fes/2.0 http://schemas.opengis.net/filter/2.0/filterAll.xsd">
    <fes:Or>
        <fes:PropertyIsEqualTo matchAction="ANY" matchCase="false">
            <fes:ValueReference>contact.creator-name</fes:ValueReference>
            <fes:Function name="template.value.v1">
                <fes:Literal/>
                <fes:Literal>my-id-same-source</fes:Literal>
                <fes:Literal>true</fes:Literal>
                <fes:Literal>false</fes:Literal>
            </fes:Function>
        </fes:PropertyIsEqualTo>
        <fes:PropertyIsEqualTo matchAction="ANY" matchCase="false">
            <fes:ValueReference>contact.publisher-name</fes:ValueReference>
            <fes:Function name="template.value.v1">
                <fes:Literal/>
                <fes:Literal>my-id-same-source</fes:Literal>
                <fes:Literal>true</fes:Literal>
                <fes:Literal>false</fes:Literal>
            </fes:Function>
        </fes:PropertyIsEqualTo>
        <fes:PropertyIsEqualTo matchAction="ANY" matchCase="false">
            <fes:ValueReference>contact.point-of-contact-name</fes:ValueReference>
            <fes:Function name="template.value.v1">
                <fes:Literal/>
                <fes:Literal>my-id-same-source</fes:Literal>
                <fes:Literal>true</fes:Literal>
                <fes:Literal>false</fes:Literal>
            </fes:Function>
        </fes:PropertyIsEqualTo>
        <fes:PropertyIsEqualTo matchAction="ANY" matchCase="false">
            <fes:ValueReference>contact.contributor-name</fes:ValueReference>
            <fes:Function name="template.value.v1">
                <fes:Literal/>
                <fes:Literal>my-id-same-source</fes:Literal>
                <fes:Literal>true</fes:Literal>
                <fes:Literal>false</fes:Literal>
            </fes:Function>
        </fes:PropertyIsEqualTo>
    </fes:Or>
</fes:Filter>
```

## Sample Results JSON blob (results.json)

```$xslt
[
  {
    "title": "Common Fields",
    "description": "A set of minimalist fields needed for skimming results",
    "descriptors": [
      "title",
      "description",
      "created",
      "resource-download-url",
      "thumbnail"
    ]
  }
]
```