...
function getViewSwitcher() {
    if (!viewSwitcher) {
        viewSwitcher = new ViewSwitcher("resultsView", "recordView", "cesiumContainer");
    }
    return viewSwitcher;
}
...
function setUpPaging(hits, start, pageSize) {
    ...
    if (hits > pageSize) {
        ...
        if (startPage === currentPage) {
            ...
            $(
                "<li><a href='javascript:getResults("
                    + ((start - pageSize) > 1 ? (start - pageSize) : 1)
                    + ")'>Prev</a></li>").appendTo(pages);
        }
        for (p = startPage; p <= endPage; p++) {
            ...
            $(
                "<li" + (p === currentPage ? " class='active'" : "")
                    + "><a href='" + javascript + ":getResults("
                    + navTo + ")'>" + p + "</a></li>").appendTo(pages);
        }
        if (endPage === currentPage) {
            $("<li class='disabled'><a>Next</a></li>").appendTo(pages);
        } else {
            $(
                "<li><a href='javascript:getResults(" + (start + pageSize)
                    + ")'>Next</a></li>").appendTo(pages);
        }
    }
}
...
function createRow(index) {
    var metacard, props, row, title, link, source, location, date, thumbnail, product, productLink, productIcon, thumbnailLink, javascript;
    ...
    if (props.thumbnail) {
        ...
        thumbnailLink.attr("src", "data:image/jpeg;charset=utf-8;base64, "
            + props.thumbnail);
    }
    ...
    productLink.attr("href", getDDFServer() + "/services/catalog/sources/"
        + props["source-id"] + "/" + props.id + "?transform=resource");
    productIcon = $("<i>");
    ...
    if (props["resource-size"]) {
        product
            .append("<div style=\"visibility: hidden;\" class=\"resourceSize\">"
                + props["resource-size"] + "</div>");
    }
    row.append(title);
    row.append(source);
    row.append(location);
...
function loadPageForItem(index) {
    if (resultsMapping[index]) {
        ...
        setUpPaging(getMaxResults(), getPageStartIndex(index),
            getItemsPerPage());
        buildRows(getPageStartIndex(index), getPageEndIndex(index));
    }
}

function getResults(startVal, showList, displayOnMap) {
...
function hideLoading(startVal, showResults, displayOnMap) {
    $("#loadingView").hide();
    if ((showResults
        || getViewSwitcher().getCurrentView() === getViewSwitcher().RESULTS_VIEW) && !displayOnMap) {
        getViewSwitcher().showResultsView();
    } else if (displayOnMap) {
        getViewSwitcher().showMapView();
    } else {
...
function createResultsOnMap(startAt, finishAt) {
...
//	if(results.displayOnMap) {
    createResultsOnMap(results.start, results.start + results.results.length - 1);
//	} else {
    buildRows(results.start, results.start + results.results.length - 1);
//	}
    hideLoading(results.start, results.showList, results.displayOnMap);
}
...
function restoreFederationSelections(src) {
    if (src) {
        ...
        $.each(sources, function () {
            $("select[name=federationSources] option:contains('" + this + "')")
...
function restoreTypeSelection(type) {
    $("select[name=typeList] option:contains('" + type + "')").attr("selected",
        "selected");
}

function restoreCountSelection(count) {
    $("select[name=count] option").filter(function () {
...
function validatePositiveInteger(posIntElement, revertValue) {
    var val = validateNumberInRange(0, Number.MAX_VALUE, $.trim(posIntElement
        .val()), revertValue, true);
    if (Number(val) === 0) {
        val = "";
    }
...
function validateNumber(numberElement, revertValue) {
    var val = validateNumberInRange(numberElement.attr("min"), numberElement
...
var pointRadiusLatitude = validateNumberInRange(-90, 90, urlVals
    .param('latitude'), "");
$('input[name=latitude]').val(pointRadiusLatitude);
var pointRadiusLongitude = validateNumberInRange(-180, 180, urlVals
    .param('longitude'), "");
...
function updateOffset() {
    relOffsetEntry = validatePositiveInteger($('input[name=offsetTime]'),
        relOffsetEntry);
    if (relOffsetEntry) {
        $('input[name=dtoffset]').val(
            getTimeInMillis(relOffsetEntry, $(
...
function updatePointRadius() {
    pointRadiusLatitude = validateNumber($('input[name=latitude]'),
        pointRadiusLatitude);
    pointRadiusLongitude = validateNumber($('input[name=longitude]'),
        pointRadiusLongitude);
    radiusValue = validatePositiveInteger($('input[name=radiusValue]'),
        radiusValue);
    ...
    if (pointRadiusLatitude && pointRadiusLongitude && radiusValue) {
        ...
        $('input[name=radius]').val(
            getDistanceInMeters(radiusValue, $('select[name=radiusUnits]')
                .val()));
...
$('button[name=resultsListBtn]').on('click', function (e) {
    mapDisplay = false;
    getViewSwitcher().showResultsView();

});

$('button[name=resultsMapBtn]').on('click', function (e) {
    mapDisplay = true;
    getViewSwitcher().showMapView();
