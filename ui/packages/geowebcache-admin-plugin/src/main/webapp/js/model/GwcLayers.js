    ...
    function (Backbone, $) {
        ...
        GwcLayers.LayerModel = Backbone.Model.extend({
            ...
            parseLayerXml: function(name, data) {
                var xmlUrl = $(data).find("wmsUrl").find("string").text();
                var layerUrl = "../../geowebcache/rest/layers/" + name + ".xml";
                var mimes = $(data).find("mimeFormats").find("string");
                var mimeTypes = [];
                $.each(mimes, function(index, value) {
                    mimeTypes.push($(value).text());
                });

                var xmlLayers = $(data).find("wmsLayers").text();
                var xmlLayersArray = xmlLayers.trim().split(",");

                if(!this.nameExistsInArray(name)) {
                    var layer = { name : name, xmlUrl : xmlUrl, layerUrl : layerUrl, mimeTypes : mimeTypes, xmlLayers : xmlLayersArray };
