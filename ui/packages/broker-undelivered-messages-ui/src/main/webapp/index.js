...
const Header = ({data}) => {
  ...
  return (
    <div>
      <button onClick={() => retrieveData(data, getMessagesURI)}>RETRIEVE</button>
      <button onClick={() => operateOnData(data, 'resendMessages')}>RESEND</button>
      <button onClick={() => operateOnData(data, 'deleteMessages')}>DELETE</button>
      <button onClick={() => poll()}>
        {store.getState().polling.polling ? 'STOP POLLING' : 'START POLLING'}
      </button>
      <div className='selected'>
        {getSelectedIds(data).length} of {data.length} selected.
        <span className='clickable' onClick={() => store.dispatch(checkAllMessages(true))}>Select All</span>
        <span className='clickable' onClick={() => store.dispatch(checkAllMessages(false))}>Deselect All</span>
        <span className='clickable' onClick={() => store.dispatch(expandAllMessages(true))}>Expand All</span>
        <span className='clickable' onClick={() => store.dispatch(expandAllMessages(false))}>Collapse All</span>
