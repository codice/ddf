...
define([
  ...
  ModalRegistry.View = Marionette.Layout.extend({
    ...
    onRender: function() {
      ...
      if (this.mode !== 'edit') {
        this.showError(
          'Registry must have a valid url (non-blank/non-duplicate)'
        )
      }
    ...
    urlChanged: function(evt) {
      ...
      if (hostnamePort && this.urlIsValid(url)) {
        ...
        const newNameInParens =
          '(' + hostnamePort + ')' + ' (' + this.type + ')'
        ...
        this.registry.get('collection').models.forEach(function(registry) {
          if (registry.id) {
            if (
              registry.id.endsWith(newNameToSet) ||
              registry.id.endsWith(newNameInParens)
            ) {
        ...
        if (
          _.isUndefined(registryWithDupId) ||
          (this.mode === 'edit' &&
            (oldName.endsWith(newNameToSet) ||
              oldName.endsWith(newNameInParens)))
        ) {
    ...
    checkName: function(newName) {
      ...
      if (newName === '') {
        ...
        if (
          view.nameIsValid(newName, config.get('properties').get('registry-id'))
        ) {
          ...
          view.showError(
            'A registry configuration with the name "' +
              newName +
    ...
    nameIsValid: function(name, registryId) {
      ...
      configs.forEach(function(registryConfig) {
        if (
          ...
          registryConfig.get('attributes')['registry-id'] !== registryId
        ) {
          match = registryConfig
          return
        }
