...
define(['backbone.marionette', 'jquery', 'backbone'], function(
  Marionette,
  $,
  Backbone
) {
  ...
  Ingest.DetailModel = Backbone.Model.extend({
    ...
    updateGeoIndexWithUrl: function(checked) {
      var url = this.get('url')
        ...
        .join('!/')
      url =
        '../jolokia/exec/org.codice.ddf.spatial.admin.module.service.Geocoding:service=geocoding/updateGeoIndexWithUrl/' +
        url +
        '/' +
        checked
    ...
    updateGeoIndexWithFilePath: function(resourceUri, id, model, checked) {
      var url =
        '../jolokia/exec/org.codice.ddf.spatial.admin.module.service.Geocoding:service=geocoding/updateGeoIndexWithFilePath/' +
        id +
        '/' +
        resourceUri +
        '/' +
    ...
    startPollingModel: function(model) {
      ;(function poll() {
        ...
        if (
          currentProgress !== 100 &&
          model.get('state') !== 'failed' &&
          model.get('state') !== 'failed-geonames'
        ) {
          setTimeout(function() {
            $.ajax({
              url:
                '../jolokia/exec/org.codice.ddf.spatial.admin.module.service.Geocoding:service=geocoding/progressCallback/',
