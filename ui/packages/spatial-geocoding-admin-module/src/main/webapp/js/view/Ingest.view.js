...
define([
  ...
  IngestView.Details = Marionette.LayoutView.extend({
    ...
    onRender: function() {
      ...
      view.fileUploadListRegion.show(
        new UploadList({ collection: view.model.collection })
      )
      ...
      this.$('.fileupload').fileupload({
        ...
        add: function(e, data) {
          ...
          var uploadsInProgress = view.model.collection.findWhere({
            state: 'uploading',
          })
          // Allow no duplicate entries
          var duplicates = view.model.collection.findWhere({
            name: model.attributes.name,
          })

          if (
            typeof duplicates === 'undefined' &&
            typeof uploadsInProgress === 'undefined'
          ) {
            ...
            if (view.isValidGeoNamesFile(data.files[0].name)) {
              ...
              $.blueimp.fileupload.prototype.options.add.call(
                model.fileuploadObject.ref,
                model.fileuploadObject.e,
                model.fileuploadObject.data
              )
            } else {
          ...
          var contentFolder = data._response.jqXHR.getResponseHeader(
            'Content-ID'
          )
      ...
      this.setupPopOver(
        '[data-toggle="url-ingest-popover"]',
        'Download a GeoNames file from the provided Geonames URL and import it into DDF.'
      )
      this.setupPopOver(
        '[data-toggle="url-popover"]',
        'The GeoNames URL to download a GeoNames file from.'
      )
      this.setupPopOver(
        '[data-toggle="local-ingest-popover"]',
        'Upload a GeoNames file into DDF.'
      )
      this.setupPopOver(
        '[data-toggle="create-index-popover"]',
        'Check to overwrite all previous GeoName entries.'
      )
    },
    onClose: function() {
