...
org.cometd.RequestTransport = function() {
  ...
  function _metaConnectSend(envelope) {
    if (_metaConnectRequest !== null) {
      throw 'Concurrent metaConnect requests not allowed, request id=' +
...
org.cometd.WebSocketTransport = function() {
  ...
  function _websocketConnect() {
    ...
    var webSocket = protocol
      ? new org.cometd.WebSocket(url, protocol)
      : new org.cometd.WebSocket(url)
    var onopen = function() {
...
org.cometd.Cometd = function(name) {
  ...
  function _send(sync, messages, longpoll, extraPath) {
    ...
    for (var i = 0; i < messages.length; ++i) {
      ...
      if (message !== undefined && message !== null) {
        messages[i] = message
        if (callback) _publishCallbacks[message.id] = callback
      } else {
        messages.splice(i--, 1)
      }
    }
