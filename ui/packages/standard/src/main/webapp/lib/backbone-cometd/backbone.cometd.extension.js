...
;(function(undefined) {
  ...
  Backbone.Collection.prototype.origDestroy =
    ...
    if (options.useAjaxSync || model.useAjaxSync) {
      ...
      if (method === 'read') {
        ...
        if (options.data) {
          ...
          //just listen for a response from the service endpoint
          model.subscription = Cometd.Comet.subscribe(
            model.url,
            options.success
          )
        }
        ...
        //for now we are assuming that create, patch, and update all do the same thing
        var success = options.success
        options.success = function(resp) {
          var retVal = success(resp)
          if (retVal === false) {
