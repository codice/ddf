...
define(['backbone', 'underscore', 'wreqr', 'backboneassociations'], function(
  Backbone,
  _,
  wreqr
) {
  ...
  MetaCard.Geometry = Backbone.AssociatedModel.extend({
    ...
    getAllPoints: function() {
      ...
      if (this.isGeometryCollection()) {
        ...
        return _.flatten(
          _.map(geometries, function(geometry) {
      ...
      if (this.lastResponse) {
        ...
        if (update && this.get('results') && this.get('results').length > 0) {
          var selectedForSave = this.get('results').filter(function(result) {
            return (
              result.get('metacard').has('selectedForSave') &&
          ...
          _.forEach(update.results, function(result) {
            if (
              _.some(selectedForSave, function(saved) {
                return (
                  saved
                    .get('metacard')
                    .get('properties')
                    .get('id') === result.metacard.properties.id
                )
              })
            ) {
