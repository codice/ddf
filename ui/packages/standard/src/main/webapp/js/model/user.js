...
define([
  ...
  User.updateMapLayers = function(layerPrefs, layersToRemove, index) {
    layerPrefs.each(function(layer) {
      ...
      if (layer) {
        for (var i = 0; i < properties.imageryProviders.length; i++) {
          var layerObj = _.omit(layer.toJSON(), [
            'id',
            'show',
            'label',
            'alpha',
    ...
    for (var i = 0; i < properties.imageryProviders.length; i++) {
      ...
      for (var j = 0; j < layerPrefs.models.length; j++) {
        var layerObj = _.omit(layerPrefs.at(j).toJSON(), [
          'id',
          'show',
          'label',
          'alpha',
      ...
      if (!found) {
        layerPrefs.add(
          new User.MapLayer(properties.imageryProviders[i], { parse: true })
        )
      }
    }
  }
    ...
    savePreferences: function() {
      if (this.parents[0].parents[0].isGuestUser()) {
        window.localStorage.setItem(
          'org.codice.ddf.search.preferences.mapColors',
  ...
  User.MapLayers = Backbone.Collection.extend({
    ...
    savePreferences: function() {
      if (this.parents[0].parents[0].isGuestUser()) {
        ...
        window.localStorage.setItem(
          'org.codice.ddf.search.preferences.mapLayers',
          mapLayersJson
        )
      } else {
  ...
  User.Response = Backbone.AssociatedModel.extend({
    ...
    getFallbackMapColors: function() {
      var jsonString = window.localStorage.getItem(
        'org.codice.ddf.search.preferences.mapColors'
      )
