 ...
 *
 **/
define(['underscore', 'moment', 'properties'], function(_, moment, Properties) {
  ...
  Filter.CQLFactory = {
    toCQL: function(filter) {
      ...
      if (
        Filter.CQLFactory[fieldType] &&
        Filter.CQLFactory[fieldType][fieldOperator]
      ) {
    ...
    buildStringFilter: function(filter, likeOperator) {
      if (
        filter.get('fieldName') === Properties.filters.METADATA_CONTENT_TYPE
      ) {
        ...
        _.each(split, function(subContentType) {
          if (subContentType === 'no-value') {
            joinArray.push(
              Filter.CQLFactory.formatFieldName(filter.get('fieldName')) +
                ' IS NULL '
            )
          } else {
            // lets check if its a mapped value
            if (
              Properties.typeNameMapping &&
              Properties.typeNameMapping[subContentType]
            ) {
              // for each content type mapped to this value, create a expression for it.
              _.each(Properties.typeNameMapping[subContentType], function(
                customContentType
              ) {
                joinArray.push(
                  Filter.CQLFactory.formatFieldName(filter.get('fieldName')) +
                    ' = ' +
                    Filter.CQLFactory.getValue(customContentType)
                )
              })
            } else {
              ...
              joinArray.push(
                Filter.CQLFactory.formatFieldName(filter.get('fieldName')) +
                  ' = ' +
                  Filter.CQLFactory.getValue(subContentType)
              )
            }
          }
        })
    ...
    string: {
      equals: function(filter) {
        return (
          Filter.CQLFactory.formatFieldName(filter.get('fieldName')) +
          ' = ' +
          Filter.CQLFactory.getValue(filter.get('stringValue1'))
        )
      },
    ...
    date: {
      before: function(filter) {
        return (
          Filter.CQLFactory.formatFieldName(filter.get('fieldName')) +
          ' BEFORE ' +
          Filter.CQLFactory.getDateValue(filter.get('dateValue1'))
        )
      },
    ...
    geometry: {
      intersects: function(filter) {
        ...
        if (geoType === 'bbox') {
          ...
          if (
            _.first(polygon)[0] !== _.last(polygon)[0] &&
            _.first(polygon)[1] !== _.last(polygon)[1]
          ) {
