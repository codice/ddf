...
define(['backbone', 'moment', 'underscore', 'jquery'], function(
  Backbone,
  moment,
  _,
  $
  ...
  Notification.Notification = Backbone.Model.extend({
    ...
    validate: function(attrs) {
      if (!attrs.application) return 'Notification must have application.'
      if (!attrs.title) return 'Notification must have title.'
      if (!attrs.message) return 'Notification must have message.'
      if (!attrs.timestamp) return 'Notification must have timestamp.'
    },
  })
    ...
    policies: {
      latest: function(timeCutOff) {
        ...
        this.each(function(notification) {
          if (
            moment(notification.get('timestamp')).valueOf() + timeCutOff <
            coll.now
          ) {
