 ...
 **/
/* global define */
define(['marionette', 'cesium', 'underscore'], function(Marionette, Cesium, _) {
  var DEFAULT_PIXEL_DISTANCE = 100

  var MAX_RADIUS_METER_DISTANCE = 2500000

  var MIN_CAMERA_METER_HEIGHT = 5

  var MARKER_COLOR = Cesium.Color.RED
  ...
  MapClustering = Marionette.Controller.extend({
    ...
    getEllipsoidAt40PercentOfMapView: function(width, height) {
      return this.viewer.camera.pickEllipsoid(
        new Cesium.Cartesian2((width * 40) / 100, height / 2)
      )
    },
    getEllipsoidAt60PercentOfMapView: function(width, height) {
      return this.viewer.camera.pickEllipsoid(
        new Cesium.Cartesian2((width * 60) / 100, height / 2)
      )
    },
