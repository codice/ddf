...
define([
  ...
  FilterController = Marionette.Controller.extend({
    ...
    processSearch: function(searchToProcess) {
      ...
      wreqr.vent.trigger(
        'mapfilter:showFilters',
        searchToProcess.parents[0].filters
      )
      // default all field
      var array = [
      ...
      if (searchToProcess.has('metacard-types')) {
        ...
        _.each(_.keys(types), function(type) {
          ...
          _.each(pairs, function(pair) {
            if (
              !_.contains(currentFieldNames, pair[0]) &&
              pair[1].indexed === true
            ) {
    ...
    registerFacetCounts: function(facetCounts, numberOfResults) {
      ...
      controller.facetCounts = _.pick(facetCounts, [
        Properties.filters.METADATA_CONTENT_TYPE,
      ])
      var defaults = {}
      ...
      if (
        !controller.facetCounts[Properties.filters.METADATA_CONTENT_TYPE][
          'no-value'
        ]
      ) {
        var values = _.values(
        ...
        controller.facetCounts[Properties.filters.METADATA_CONTENT_TYPE][
          'no-value'
        ] = numberOfResults - totalFacetCount
      }
    },
  })
