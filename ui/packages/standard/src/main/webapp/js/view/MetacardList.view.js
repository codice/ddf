...
define([
  ...
  function toggleSourceCheckbox(evt, sourceId) {
    if (
      $('.source-toggle:checked, .excluded-source-toggle:checked').length < 1
    ) {
  ...
  List.MetacardRow = Marionette.ItemView.extend({
    ...
    initialize: function() {
      if (
        this.model.collection.parents &&
        this.model.collection.parents[0] &&
        this.model.collection.parents[0].get('selecting')
      ) {
      ...
      if (collectionView.isBuffering) {
        ...
        if (index === 0) {
          ...
          if (collectionView.elBuffer.firstChild) {
            return collectionView.elBuffer.insertBefore(
              childView.el,
  ...
  List.StatusTable = Marionette.CompositeView.extend({
    ...
    serializeData: function() {
      ...
      var excludedSources = new Backbone.Collection(
        this.options.sources.filter(function(source) {
  ...
  List.MetacardListView = Marionette.LayoutView.extend({
    ...
    onRender: function() {
      var view = this
      view.listRegion.show(
        new List.MetacardTable({
          collection: view.model.get('results'),
        })
      )
      if (view.model.get('status')) {
        view.statusRegion.show(
          new List.StatusTable({
            collection: view.model.get('status'),
            sources: wreqr.reqres.request('workspace:getsources'),
          })
        )
      }
      view.countRegion.show(
        new List.CountView({
          model: view.model,
        })
      )
      view.filterRegion.show(
        new FilterLayoutView({
          model: view.model,
        })
      )
    },
    ...
    updateSpinner: function() {
      if (
        !_.isUndefined(this.model.get('hits')) ||
    ...
    saveSelectedRecords: function() {
      ...
      var records = this.model
        .get('results')
        .where({ 'metacard>selectedForSave': true })
