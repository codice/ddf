...
define([
  ...
  WorkspaceView.WorkspaceAdd = Marionette.ItemView.extend({
    ...
    addWorkspace: function() {
      var workspace = new Workspace.Model({
        name: this.$('#workspaceName').val(),
      })
    ...
    showSearch: function(event) {
      if (['edit', 'remove'].indexOf(event.target.id) === -1) {
        ...
        if (
          ...
          this.model.get('result').get('status')
        ) {
          this.model
            .get('result')
            .get('status')
  ...
  WorkspaceView.Workspace = Marionette.LayoutView.extend({
    ...
    onRender: function() {
      ...
      this.workspaceSearchPanelRegion.show(
        new WorkspaceView.SearchList({ collection: this.model.get('searches') })
      )
    },
    editMode: function() {
  ...
  WorkspaceView.WorkspaceItem = Marionette.ItemView.extend({
    ...
    serializeData: function() {
      var working = false,
        hits,
        searches = this.model.get('searches')
      if (searches) {
        searches.forEach(function(search) {
          var result = search.get('result')
          if (result) {
  ...
  WorkspaceView.WorkspacesLayoutView = Marionette.LayoutView.extend({
    ...
    initialize: function() {
      ...
      this.listenTo(
        wreqr.vent,
        'workspace:searcheditcancel',
        this.workspaceCancelEdit
      )
    ...
    setupEvents: function(tabHash) {
      ...
      if (tabHash === '#workspaces') {
        ...
        this.listenTo(
          wreqr.vent,
          'workspace:results',
          this.showWorkspaceResults
        )
        this.listenTo(
          wreqr.vent,
          'metacard:selected',
          this.showWorkspaceMetacard
        )
        this.listenTo(wreqr.vent, 'workspace:new', this.showWorkspaceAdd)
        this.listenTo(
          wreqr.vent,
          'workspace:searchedit',
          this.showWorkspaceSearchEdit
        )
        this.listenTo(
          wreqr.vent,
          'workspace:editcancel',
          this.cancelEditWorkspaceList
        )
        ...
        this.listenTo(
          ...
          this.saveResultsToWorkspace
        )
        this.listenTo(
          wreqr.vent,
          'workspace:resultssavecancel',
          this.cancelResultsToWorkspace
        )
    ...
    saveResultsToWorkspace: function(search, records) {
      this.workspaceRegion.show(
        new WorkspaceSaveResults({
          model: this.model,
          search: search,
          records: records,
        }),
    ...
    workspaceSave: function(search) {
      ...
      if (view.currentWorkspace) {
        if (search) {
          if (
            _.isUndefined(this.currentSearchIndex) ||
            _.isNull(this.currentSearchIndex) ||
            _.isNaN(this.currentSearchIndex) ||
            this.currentSearchIndex < 0
          ) {
            view.currentWorkspace.get('searches').add(search)
          } else {
            view.currentWorkspace.get('searches').models[
    ...
    showWorkspace: function(direction, model) {
      ...
      this.workspaceRegion.show(
        new WorkspaceView.Workspace({ model: this.currentWorkspace }),
        direction
      )
    },
    ...
    showWorkspaceList: function(direction) {
      this.workspaceRegion.show(
        new WorkspaceView.WorkspaceList({
          collection: this.model.get('workspaces'),
        }),
        direction
      )
    },
    ...
    showWorkspaceSearchEdit: function(direction, model, index) {
      ...
      this.workspaceRegion.show(
        new QueryView.QueryView({ isWorkspace: true, model: queryModel }),
        direction
      )
    },
    ...
    showWorkspaceAdd: function(direction) {
      this.workspaceRegion.show(
        new WorkspaceView.WorkspaceAdd({
          collection: this.model.get('workspaces'),
        }),
        direction
      )
    ...
    showWorkspaceResults: function(direction, model) {
      ...
      this.workspaceRegion.show(
        new MetacardList.MetacardListView({
          model: this.currentSearch.get('result'),
        }),
        direction
      )
      wreqr.vent.trigger('map:clear')
    ...
    showWorkspaceMetacard: function(direction, model) {
      this.workspaceRegion.show(
        new MetacardDetail.MetacardDetailView({ model: model }),
        direction
      )
    },
    ...
    updateMapPrimitive: function() {
      ...
      if (
        this.currentSearch.get('north') &&
        this.currentSearch.get('south') &&
        this.currentSearch.get('east') &&
        this.currentSearch.get('west')
      ) {
        wreqr.vent.trigger('search:bboxdisplay', this.currentSearch)
        this.currentSearch.trigger('EndExtent')
      } else if (
        this.currentSearch.get('lat') &&
        this.currentSearch.get('lon') &&
        this.currentSearch.get('radius')
      ) {
    ...
    onRender: function() {
      this.workspaceControlRegion.show(
        new WorkspaceControl.WorkspaceControlView()
      )
      this.workspaceRegion.show(
        new WorkspaceView.WorkspaceList({
          collection: this.model.get('workspaces'),
  ...
  WorkspaceView.WorkspaceLayout = Marionette.LayoutView.extend({
    ...
    className:
      'partialaffix span3 row-fluid nav well well-small search-controls',
    regions: {
    ...
    onRender: function() {
      this.workspacesRegion.show(
        new WorkspaceView.WorkspacesLayoutView({ model: this.model })
      )
      ...
      wreqr.vent.trigger(
        'workspace:tabshown',
        this.$('.nav-tabs > .active a').attr('href')
      )
    },
  })
  ...
  WorkspaceView.PanelLayout = Marionette.LayoutView.extend({
    ...
    onRender: function() {
      this.panelRegion.show(
        new WorkspaceView.WorkspaceLayout({ model: this.model })
      )
