 ...
 **/
/*global define*/
define(['marionette', 'underscore', 'properties', 'q', 'direction'], function(
  Marionette,
  _,
  Properties,
  Q,
  ...
  var Region = Marionette.Region.extend({
    ...
    show: function(view, direction) {
      var region = this
      direction = _.isUndefined(direction, dir.forward)
        ? dir.forward
        : direction
      this._ensureElement()
      ...
      if (
        !_.isUndefined(this.currentPromise) &&
        Q.isPromise(this.currentPromise) &&
        this.currentPromise.isPending()
      ) {
      ...
      this.currentPromise = destroyPromise
        .then(function() {
          if (isViewClosed) {
    ...
    open: function(view, direction, isViewClosed) {
      ...
      return this.slide(view, direction, flyIn).then(function() {
    ...
    destroy: function(direction) {
      ...
      return this.slide(view, direction, flyOut).then(function() {
    ...
    slide: function(view, forwardorBackward, flyInOrOut) {
      ...
      if (flyInOrOut === flyIn && forwardorBackward === dir.forward) {
        ...
        animationProps.left =
          parseInt(view.$el.css('left'), 10) === 0 ? -view.$el.outerWidth() : 0
