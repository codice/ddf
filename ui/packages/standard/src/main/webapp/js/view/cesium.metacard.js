  ...
  Views.PointView = Marionette.ItemView.extend({
    ...
    initialize: function(options) {
      ...
      this.color =
        ...
        Cesium.Color.fromCssColorString(
          Application.UserModel.get('user>preferences>mapColors>pointColor')
        )
      this.buildBillboard()
    },
    ...
    isThisPrimitive: function(event) {
      ...
      if (event.object) {
        ...
        if (
          _.contains(
            _.map(this.geometries, function(view) {
    ...
    buildBillboard: function() {
      ...
      if (this.geoController.scene.terrainProvider && !point.altitude) {
        var promise = Cesium.sampleTerrain(
          this.geoController.scene.terrainProvider,
          5,
          [cartographic]
        )
        Cesium.when(promise, function(updatedCartographic) {
          if (updatedCartographic[0].height && !view.isDestroyed) {
            view.billboard.position = view.geoController.ellipsoid.cartographicToCartesian(
  ...
  Views.MultiPointView = Views.PointView.extend({
    initialize: function(options) {
      options.color =
        options.color ||
        Cesium.Color.fromCssColorString(
          Application.UserModel.get(
            'user>preferences>mapColors>multiPointColor'
          )
        )
    ...
    buildBillboard: function() {
      ...
      view.points = _.map(cartPoints, function(point) {
        ...
        if (view.geoController.scene.terrainProvider && !point.height) {
          var promise = Cesium.sampleTerrain(
            view.geoController.scene.terrainProvider,
            5,
            [point]
          )
          Cesium.when(promise, function(updatedCartographic) {
            if (updatedCartographic[0].height && !view.isDestroyed) {
              billboard.position = view.geoController.ellipsoid.cartographicToCartesian(
                updatedCartographic[0]
              )
            }
          })
  ...
  Views.LineView = Views.PointView.extend({
    initialize: function(options) {
      options.color =
        options.color ||
        Cesium.Color.fromCssColorString(
          Application.UserModel.get('user>preferences>mapColors>lineColor')
        )
    ...
    buildLine: function(points) {
      ...
      if (view.geoController.scene.terrainProvider) {
        ...
        Cesium.when(promise, function(updatedCartographic) {
          var positions = view.geoController.ellipsoid.cartographicArrayToCartesianArray(
            updatedCartographic
          )
          if (updatedCartographic[0].height && !view.isDestroyed) {
      ...
      var cartesian = view.geoController.ellipsoid.cartographicArrayToCartesianArray(
        cartPoints
      )
  ...
  Views.MultiLineView = Views.LineView.extend({
    initialize: function(options) {
      options.color =
        options.color ||
        Cesium.Color.fromCssColorString(
          Application.UserModel.get('user>preferences>mapColors>multiLineColor')
        )
      this.lineMap = {}
      Views.PointView.prototype.initialize.call(this, options)
    },
  ...
  Views.RegionView = Views.PointView.extend({
    initialize: function(options) {
      this.color =
        options.color ||
        Cesium.Color.fromCssColorString(
          Application.UserModel.get('user>preferences>mapColors>polygonColor')
        )
    ...
    getOutlineColor: function() {
      return new Cesium.PerInstanceColorAppearance({
        ...
        renderState: {
          ...
          lineWidth: Math.min(
            2.0,
            this.geoController.scene.maximumAliasedLineWidth
          ),
        },
      })
    },
    ...
    setPolygonSelected: function() {
      var view = this
      var attributes = view.polygons[0].outline.getGeometryInstanceAttributes(
        'outline'
      )
      ...
      var fillAttributes = view.polygons[0].fill.getGeometryInstanceAttributes(
        'polyfill'
      )
      fillAttributes.show = Cesium.ShowGeometryInstanceAttribute.toValue(
        true,
        fillAttributes.show
      )
    },
  ...
  Views.MultiRegionView = Views.RegionView.extend({
    initialize: function(options) {
      this.color =
        ...
        Cesium.Color.fromCssColorString(
          Application.UserModel.get(
            'user>preferences>mapColors>multiPolygonColor'
          )
        )
      options.color = this.color
    ...
    setPolygonSelected: function() {
      ...
      for (var i = 0; i < view.polygons.length; i++) {
        attributes = view.polygons[i].outline.getGeometryInstanceAttributes(
          'outline'
        )
        attributes.color = Cesium.ColorGeometryInstanceAttribute.toValue(
          Cesium.Color.BLACK
        ...
        fillAttributes = view.polygons[i].fill.getGeometryInstanceAttributes(
          'polyfill'
        )
        fillAttributes.show = Cesium.ShowGeometryInstanceAttribute.toValue(
          true,
  ...
  Views.GeometryCollectionView = Views.PointView.extend({
    initialize: function(options) {
      options.color =
        ...
        Cesium.Color.fromCssColorString(
          Application.UserModel.get(
            'user>preferences>mapColors>geometryCollectionColor'
          )
        )
      options.polygonColor =
        options.polygonColor ||
      ...
      options.polygonColor = Cesium.Color.fromRgba(
        options.polygonColor.toRgba()
      )
    ...
    buildGeometryCollection: function(options) {
      ...
      this.geometries = _.map(collection.getGeometryCollection(), function(
        geo
      ) {
