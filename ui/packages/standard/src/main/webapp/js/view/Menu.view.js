...
define([
        ...
        if (id !== 'close' && id !== 'cancelRemove') {
          if (id === 'remove') {
            ...
            Cometd.Comet.publish(this.model.url, {
              data: [{ id: this.model.get('id'), action: id }],
            })
          }
        } else {
          if (id === 'cancelRemove') {
    ...
    onRender: function() {
      if (parseInt(this.model.get('progress'), 10) <= 100) {
        if (parseInt(this.model.get('progress'), 10) !== -1) {
          this.$('.task-progressbar').progressbar({
            value: parseInt(this.model.get('progress'), 10),
          })
        }
      }
    },
  ...
  Menu.TaskCategory = Marionette.ItemView.extend({
    ...
    removeAll: function(e) {
      ...
      currentActivities = this.model
        .get('collection')
        .where({ category: this.model.get('category') })
  ...
  Menu.NotificationCategory = Marionette.ItemView.extend({
    ...
    removeAll: function(e) {
      ...
      var notifications = []
      notificationsInCategory = this.model
        .get('collection')
        .where({ application: this.model.get('category') })
        ...
        var Welcome = Menu.Item.extend({
          ...
          initialize: function() {
            if (this.isNotGuestUser()) {
              this.model.set({
                name: Application.UserModel.get('user').get('username'),
              })
            } else if (
              !this.isNotGuestUser() &&
              properties.externalAuthentication
            ) {
              this.model.set({
                name:
                  typeof Application.UserModel.get('user').get('username') ===
                  'undefined'
                    ? 'ERROR'
                    : Application.UserModel.get('user')
                        .get('username')
                        .split('@')[0],
              })
            }
          ...
          isNotGuestUser: function() {
            return (
              ...
              Application.UserModel.get('user') &&
              Application.UserModel.get('user').get('username') &&
              !Application.UserModel.get('user').isGuestUser()
            )
          },
          onRender: function() {
