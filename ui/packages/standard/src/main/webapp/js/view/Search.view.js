...
define([
  ...
  Search.SearchLayout = Marionette.LayoutView.extend({
    ...
    updateMapPrimitive: function() {
      ...
      if (
        this.query.get('north') &&
        this.query.get('south') &&
        this.query.get('east') &&
        this.query.get('west')
      ) {
    ...
    showResults: function(direction, result) {
      if (result) {
        this.result = result
        this.searchRegion.show(
          new MetacardList.MetacardListView({ model: this.result }),
          direction
        )
      } else {
        if (!this.result) {
    ...
    showMetacardDetail: function(direction, metacard) {
      this.searchRegion.show(
        new MetacardDetail.MetacardDetailView({ model: metacard }),
        direction
      )
    },
  })
