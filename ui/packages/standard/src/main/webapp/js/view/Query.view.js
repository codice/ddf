  ...
  Query.QueryView = Marionette.ItemView.extend({
    ...
    onRender: function() {
      ...
      var radiusConverter = function(direction, value) {
          ...
          switch (direction) {
            case 'ViewToModel':
              var distanceInMeters = view.getDistanceInMeters(
                value,
                radiusUnitVal
              )
            ...
            case 'ModelToView':
              var distanceFromMeters = view.getDistanceFromMeters(
                view.model.get('radius'),
                radiusUnitVal
              )
              ...
              return Math.abs(currentValue - distanceFromMeters) >
                deltaThreshold
                ? distanceFromMeters
                : currentValue
          }
        ...
        offsetConverter = function(direction, value) {
          if (direction !== 'ModelToView') {
            ...
            view.model.set(
              'dtoffset',
              view.getTimeInMillis(
                view.$('input[name=offsetTime]').val(),
                view.$('select[name=offsetTimeUnits]').val()
              )
            )
          }
          return value
      ...
      var queryModelBindings = Backbone.ModelBinder.createDefaultBindings(
        this.el,
        'name'
      )
      ...
      this.sourcesCollectionBinder.bind(
        this.sources,
        this.$('#federationSources')
      )
      ...
      this.setupPopOver(
        '[data-toggle="keyword-popover"]',
        'Search by free text using the grammar of the underlying source. For wildcard searches, use % or * after partial keywords (e.g. earth%).'
      )
      this.setupPopOver(
      ...
      this.setupPopOver(
        '[data-toggle="location-popover"]',
        'Search by latitude/longitude or the USNG using a polygon, point-radius, or bounding box.'
      )
      this.setupPopOver(
        '[data-toggle="type-popover"]',
        'Search for specific content types.'
      )
      this.setupPopOver(
        '[data-toggle="sorting-popover"]',
        'Sort results by relevance, distance, created time, modified time or effective time.'
      )
      this.setupPopOver(
        '[data-toggle="additional-sources-popover"]',
        'Perform an enterprise search (all federations are queried) or search specific sources.'
      )
    ...
    onRadiusUnitsChanged: function() {
      this.$('#radiusValue').val(
        this.getDistanceFromMeters(
          this.model.get('radius'),
          this.$('#radiusUnits').val()
        )
      )
    },
    ...
    onTimeUnitsChanged: function() {
      var timeInMillis = this.getTimeInMillis(
        this.model.get('dtoffset'),
        this.$('#offsetTimeUnits').val()
      )
