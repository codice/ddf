...
define([
  ...
  CesiumLayersController,
  maptype,
  Modal,
  preferencesModalTemplate,
  colorPrefsTabTemplate,
  layerPrefsTabTemplate,
  layerListTemplate,
  layerPickerTemplate,
  preferenceButtonsTemplate,
  User,
  Cesium,
  wreqr
) {
  ...
  var PrefsModalView = Modal.extend({
    ...
    initialize: function() {
      ...
      this.colorTabView = new PrefsModalView.ColorTabView({
        model: prefs.get('mapColors'),
      })
      this.layerTabView = new PrefsModalView.LayerTabView({
        model: prefs.get('mapLayers'),
      })
  ...
  PrefsModalView.ColorTabView = Marionette.LayoutView.extend({
    ...
    onRender: function() {
      this.colorBindings = Backbone.ModelBinder.createDefaultBindings(
        this.el,
        'name'
      )
  ...
  PrefsModalView.LayerTabView = Marionette.LayoutView.extend({
    ...
    resetDefaults: function() {
      ...
      this.viewMapLayers.each(function(viewLayer) {
        ...
        var defaultConfig = _.find(properties.imageryProviders, function(
          layerObj
        ) {
  ...
  PrefsModalView.LayerPicker = Marionette.ItemView.extend({
    ...
    onRender: function() {
      var layerBindings = Backbone.ModelBinder.createDefaultBindings(
        this.el,
        'name'
      )
