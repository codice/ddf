  ...
  var SaveResults = Marionette.ItemView.extend({
    ...
    serializeData: function() {
      return _.extend(this.model.toJSON(), {
        hasSearch: this.search ? true : false,
      })
    },
    onRender: function() {
      var modelBindings = Backbone.ModelBinder.createDefaultBindings(
        this.el,
        'name'
      )
    ...
    saveResults: function() {
      ...
      if (!this.viewModel.get('workspaceType')) {
        ...
        workspace = this.model
          .get('workspaces')
          .findWhere({ name: this.viewModel.get('workspaceType') })
      }
      if (this.viewModel.get('workspaceType') === 'create_new_workspace') {
        if (
          !this.viewModel.get('workspaceName') ||
          this.viewModel.get('workspaceName') === ''
        ) {
          this.$('input[name=workspaceName]').focus()
          return
        }
        workspace = new Workspace.Model({
          name: this.viewModel.get('workspaceName'),
        })
