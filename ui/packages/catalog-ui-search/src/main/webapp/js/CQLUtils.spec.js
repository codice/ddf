...
describe('CQL Utils', () => {
  ...
  describe('filter to CQL and CQL to filter conversions', () => {
    it('transform filter to CQL', () => {
      const cql = CQLUtils.transformFilterToCQL({type: "INTERSECTS", property: "anyGeo", value: "POLYGON((1 2,3 4,5 6,1 2))"});
      expect(cql).to.equal('(INTERSECTS(anyGeo, POLYGON((1 2,3 4,5 6,1 2))))');
    });

    it('transform compound AND filter to CQL', () => {
      const cql = CQLUtils.transformFilterToCQL({type: "AND", filters:[
          {type: "INTERSECTS", property: "anyGeo", value: "LINESTRING((1 2,3 4))"},
          {type: "INTERSECTS", property: "anyGeo", value: "POLYGON((5 6,7 8,9 10,5 6))"}]});
      expect(cql).to.equal('((INTERSECTS(anyGeo, LINESTRING((1 2,3 4)))) AND (INTERSECTS(anyGeo, POLYGON((5 6,7 8,9 10,5 6)))))');
    });

    it('transform CQL to filter', () => {
      const cql = CQLUtils.transformCQLToFilter('(INTERSECTS(anyGeo, POLYGON((1 2,3 4,5 6,1 2))))');
      expect(cql).to.deep.equal({type: "INTERSECTS", property: "anyGeo", value: {type: "GEOMETRY", value: "POLYGON((1 2,3 4,5 6,1 2))"}});
    });

    it('transform compound AND CQL to filter', () => {
      const cql = CQLUtils.transformCQLToFilter('((INTERSECTS(anyGeo, LINESTRING((1 2,3 4)))) AND (INTERSECTS(anyGeo, POLYGON((5 6,7 8,9 10,5 6)))))');
      expect(cql).to.deep.equal({type: "AND", filters:[
          {type: "INTERSECTS", property: "anyGeo", value: {type: "GEOMETRY", value: "LINESTRING((1 2,3 4))"}},
          {type: "INTERSECTS", property: "anyGeo", value: {type: "GEOMETRY", value: "POLYGON((5 6,7 8,9 10,5 6))"}}]});
    });
  });

  describe('transforms CQL', () => {
    it('removes single quotes from POLYGON WKTs in CQL', () => {
      const cql = CQLUtils.sanitizeGeometryCql('(INTERSECTS(anyGeo, \'POLYGON((-112.2 43.6,-102.1 48.3,-90.7 35.6,-112.2 43.6))\'))');
    ...
    it('removes single quotes from MULTIPOLYGON WKTs in CQL', () => {
      const cql = CQLUtils.sanitizeGeometryCql('(INTERSECTS(anyGeo, \'MULTIPOLYGON(((-112.2 43.6,-102.1 48.3,-90.7 35.6,-112.2 43.6)))\'))');
    ...
    it('removes single quotes from POINT WKTs in CQL', () => {
      const cql = CQLUtils.sanitizeGeometryCql('(DWITHIN(anyGeo, \'POINT(-110.4 30.4)\', 100, meters))');
      expect(cql).to.equal('(DWITHIN(anyGeo, POINT(-110.4 30.4), 100, meters))');
    });

    it('removes single quotes from LINESTRING WKTs in CQL', () => {
      const cql = CQLUtils.sanitizeGeometryCql('(DWITHIN(anyGeo, \'LINESTRING(-106.7 36.2,-87.5 46.5)\', 1, meters))');
    ...
    it('builds CQL for LINESTRING location', () => {
      const cql = CQLUtils.buildIntersectCQL(wkx.Geometry.parse('LINESTRING(1 2, 3 4)'));
      expect(cql).to.equal('(DWITHIN(anyGeo, LINESTRING(1 2,3 4), 1, meters))');
    });

    it('builds CQL for POLYGON location', () => {
      const cql = CQLUtils.buildIntersectCQL(wkx.Geometry.parse('POLYGON((1 2, 3 4, 5 6, 1 2))'));
      expect(cql).to.equal('(INTERSECTS(anyGeo, POLYGON((1 2,3 4,5 6,1 2))))');
    });

    it('builds CQL for MULTIPOINT location', () => {
      const cql = CQLUtils.buildIntersectCQL(wkx.Geometry.parse('MULTIPOINT((1 2), (3 4))'));
    ...
    it('builds CQL for MULTILINESTRING location', () => {
      const cql = CQLUtils.buildIntersectCQL(wkx.Geometry.parse('MULTILINESTRING((1 2, 3 4), (5 6, 7 8))'));
    ...
    it('builds CQL for MULTIPOLYGON location', () => {
      const cql = CQLUtils.buildIntersectCQL(wkx.Geometry.parse('MULTIPOLYGON(((1 2, 3 4, 5 6, 1 2)), ((10 20, 30 40, 50 60, 10 20)))'));
    ...
    it('builds CQL for GEOMETRYCOLLECTION location', () => {
      const cql = CQLUtils.buildIntersectCQL(wkx.Geometry.parse('GEOMETRYCOLLECTION(POINT(1 2), LINESTRING(1 2, 3 4))'));
  ...
  describe('checks filter types', () => {
    ...
    it('filter with a POINT is a point radius', () => {
      const isPointRadiusFilter = CQLUtils.isPointRadiusFilter({value: {value: 'POINT(1 1)'}});
  ...
  describe('parses WKTs into arrays', () => {
    it('correctly parses a POLYGON into an array', () => {
      const polygon = CQLUtils.arrayFromPolygonWkt('POLYGON((3 50, 4 49, 4 50, 3 50))');
      assertPolygon(polygon, [[3.0, 50.0], [4.0, 49.0], [4.0, 50.0], [3.0, 50.0]]);
    });

    it('correctly parses a MULTIPOLYGON with one POLYGON into an array', () => {
      const multipolygon = CQLUtils.arrayFromPolygonWkt('MULTIPOLYGON(((3 50, 4 49, 4 50, 3 50)))');
      assertMultiPolygon(multipolygon, [[[3.0, 50.0], [4.0, 49.0], [4.0, 50.0], [3.0, 50.0]]]);
    });

    it('correctly parses a MULTIPOLYGON with multiple POLYGONs into an array', () => {
      const multipolygon = CQLUtils.arrayFromPolygonWkt('MULTIPOLYGON(((3 50, 4 49, 4 50, 3 50)), ((8 55, 9 54, 9 55, 8 55)))');
      assertMultiPolygon(multipolygon, [[[3.0, 50.0], [4.0, 49.0], [4.0, 50.0], [3.0, 50.0]], [[8.0, 55.0], [9.0, 54.0], [9.0, 55.0], [8.0, 55.0]]]);
    });
  });
