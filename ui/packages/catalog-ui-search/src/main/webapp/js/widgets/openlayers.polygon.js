...
const translateFromOpenlayersCoordinates = (coords) => {
    return coords.map(value => value.map(point => {
        const mappedPoint = ol.proj.transform([point[0], point[1]], properties.projection, 'EPSG:4326');
        if (mappedPoint[1] > 90) {
            mappedPoint[1] = 89.9;
        }
...
Draw.PolygonView = Marionette.View.extend({
    initialize: function(options) {
        ...
        this.listenTo(this.model, 'change:polygon change:polygonBufferWidth change:polygonBufferUnits', this.updateGeometry);
    ...
    drawBorderedPolygon: function(rectangle) {
        ...
        const bufferWidth = DistanceUtils.getDistanceInMeters(this.model.get('polygonBufferWidth'), this.model.get('polygonBufferUnits')) || 1;
        const line = Turf.lineString(translateFromOpenlayersCoordinates(rectangle.getCoordinates()));
        const bufferedLine = Turf.buffer(line, bufferWidth, 'meters');
        const geometryRepresentation = (bufferedLine && new ol.geom.Polygon(bufferedLine.geometry.coordinates)) || rectangle;
