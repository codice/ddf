...
const needsRedraw = (view) => {
    ...
    if (view.cameraMagnitude < CAMERA_MAGNITUDE_THRESHOLD && currentMagnitude > CAMERA_MAGNITUDE_THRESHOLD) {
        return true;
    }
    if (view.cameraMagnitude > CAMERA_MAGNITUDE_THRESHOLD && currentMagnitude < CAMERA_MAGNITUDE_THRESHOLD) {
        return true;
...
class GeometryRenderView extends Marionette.View {
  ...
  listenForCameraChange = () => {
    this.map.scene.camera.moveStart.addEventListener(this.handleCameraMoveStart, this);
    this.map.scene.camera.moveEnd.addEventListener(this.handleCameraMoveEnd, this);
  }
  ...
  destroy = () => {
    ...
    this.map.scene.camera.moveStart.removeEventListener(this.handleCameraMoveStart, this);
    this.map.scene.camera.moveEnd.removeEventListener(this.handleCameraMoveEnd, this);
    window.cancelAnimationFrame(this.animationFrameId);
    if (this.primitive) {
    ...
    this.drawHelper[`startDrawing${capitalize(this.geometry)}`]({
        callback: (positions) => {
            ...
            const latLonRadPoints = _.map(positions, function (cartPos) {
                const latLon = controller.map.scene.globe.ellipsoid.cartesianToCartographic(cartPos);
