...
module.exports = Marionette.LayoutView.extend({
    ...
    onRender: function () {
        this.layers.show(new LayerItemCollectionView({
    ...
    listenToModel: function(){
        ...
        this.listenTo(this.model.get('mapLayers'), 'change:alpha change:show', this.save);
    },
    stopListeningToModel: function(){
        this.stopListening(this.model.get('mapLayers'), 'change:alpha change:show', this.save);
    },
    resetDefaults: function () {
        ...
        this.model.get('mapLayers').forEach(function(viewLayer){
            ...
            const defaultConfig = _.find(properties.imageryProviders, function (layerObj) {
    ...
    updateOrdering: function() {
        _.forEach(this.$el.find(`${CustomElements.getNamespace()}layer-item`), (element, index) => {
            this.model.get('mapLayers').get(element.getAttribute('data-id')).set('order', index);
        });
        this.model.get('mapLayers').sort();
        this.save();
    },
