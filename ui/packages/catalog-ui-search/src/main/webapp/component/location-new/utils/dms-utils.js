...
function dmsToWkt(dms) {
    ...
    switch(dms.shape) {
        ...
        case 'polygon':
            ...
            break;
        case 'boundingbox':
            const nw = { latitude: dms.boundingbox.north, longitude: dms.boundingbox.west };
            const ne = { latitude: dms.boundingbox.north, longitude: dms.boundingbox.east };
            const se = { latitude: dms.boundingbox.south, longitude: dms.boundingbox.east };
            const sw = { latitude: dms.boundingbox.south, longitude: dms.boundingbox.west };
...
function validateDmsPoint(point) {
    return validateDmsLatitude(point.latitude) && validateDmsLongitude(point.longitude);
}
...
function validateDmsBoundingBox(boundingbox) {
    ...
    if (!validateLatitudeRange(north)
    ...
    if (ddNorth - ddSouth < minimumDifference || ddEast - ddWest < minimumDifference) {
        return false;
    }
...
function validateDms(dms) {
    ...
    switch(dms.shape) {
        ...
        case 'circle':
            ...
            if (isNaN(radius) || radius <= 0 || toKilometers(radius, dms.circle.units) > 10000) {
                valid = false;
        ...
        case 'polygon':
            ...
            break;
        case 'boundingbox':
            if (!validateDmsLatitude(dms.boundingbox.north)
