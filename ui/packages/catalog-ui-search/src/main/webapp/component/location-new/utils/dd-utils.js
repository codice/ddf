...
function validateDdPoint(point) {
    return validateDdLatitude(point.latitude) && validateDdLongitude(point.longitude);
}
...
function validateDd(dd) {
    ...
    switch(dd.shape) {
        ...
        case 'circle':
            ...
            if (isNaN(radius) || radius <= 0 || toKilometers(radius, dd.circle.units) > 10000) {
                valid = false;
        ...
        case 'polygon':
            ...
            break;
        case 'boundingbox':
            if (!validateDdLatitude(dd.boundingbox.north)
