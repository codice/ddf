...
define([
    ...
    return Marionette.LayoutView.extend({
        ...
        startListeningToRegions: function(){
            this.listenTo(this.propertyPersistence.currentView.model, 'change:value', this.handlePersistence);
            this.listenTo(this.propertyExpiration.currentView.model, 'change:value', this.save);
            this.listenTo(this.propertyPersistence.currentView.model, 'change:value', this.save);
        },
        ...
        setupPersistence: function () {
            const persistence = user.get('user').get('preferences').get('alertPersistence');
            this.propertyPersistence.show(new PropertyView({
                model: new Property({
        ...
        setupExpiration: function () {
            const expiration = user.get('user').get('preferences').get('alertExpiration');
        ...
        save: function () {
            ...
            preferences.set({
                alertPersistence: this.propertyPersistence.currentView.model.get('value')[0],
