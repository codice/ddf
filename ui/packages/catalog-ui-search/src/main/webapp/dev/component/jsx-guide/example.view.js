...
const List = (props) => {
    ...
    const listItems =  Object.keys(user).filter((key) => key !== 'password').map((key, i) => {
        return (
            <ListItem key={key /* key is recommended, but not necessary since react doesn't control our rendering yet*/} label={key} value={user[key]}>
                <h1>{i}</h1>
            </ListItem>
        )
    });
