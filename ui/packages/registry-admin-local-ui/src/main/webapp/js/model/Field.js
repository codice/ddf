...
define([
  ...
  Field.FormField = Backbone.AssociatedModel.extend({
    ...
    isEmpty: function() {
      return (
        !this.get('value') ||
        (_.isArray(this.get('value')) && this.get('value').length === 0)
      )
    },
    ...
    validate: function() {
      ...
      if (this.get('regex')) {
        ...
        var pattern = this.get('regex').replace(
          new RegExp('^/(.*?)/' + flags + '$'),
          '$1'
        )
      ...
      if (!this.get('multiValued')) {
        if (type === 'number') {
          ...
          if (!regex.test(this.get('value'))) {
            error = this.get('regexMessage')
              ? this.get('regexMessage')
              : 'Invalid input value. Must match ' + this.get('regex')
          }
        } else if (type === 'point') {
          if (
            this.get('valueLat') &&
            (this.get('valueLat') > 90 || this.get('valueLat') < -90)
          ) {
            error = 'Invalid decimal latitude value. Must be -90 < lat < 90'
          }
          if (
            this.get('valueLon') &&
            (this.get('valueLon') > 180 || this.get('valueLon') < -180)
          ) {
            error = 'Invalid decimal longitude value. Must be -180 < lat < 180'
          }
        } else if (type === 'bounds') {
          if (
            this.get('valueLowerLat') &&
            (this.get('valueLowerLat') > 90 || this.get('valueLowerLat') < -90)
          ) {
            error = 'Invalid decimal latitude value. Must be -90 < lat < 90'
          }
          if (
            this.get('valueUpperLat') &&
            (this.get('valueUpperLat') > 90 || this.get('valueUpperLat') < -90)
          ) {
            error = 'Invalid decimal latitude value. Must be -90 < lat < 90'
          }
          if (
            this.get('valueLowerLon') &&
            (this.get('valueLowerLon') > 180 ||
              this.get('valueLowerLon') < -180)
          ) {
            error = 'Invalid decimal longitude value. Must be -180 < lat < 180'
          }
          if (
            this.get('valueUpperLon') &&
            (this.get('valueUpperLon') > 180 ||
              this.get('valueUpperLon') < -180)
          ) {
            error = 'Invalid decimal longitude value. Must be -180 < lat < 180'
          }
          ...
          if (!moment(this.get('valueDate')).isValid()) {
            ...
            this.get('valueTime') &&
            this.get('valueTime').length > 0 &&
            !moment(
              this.get('valueDate') + 'T' + this.get('valueTime')
            ).isValid()
          ) {
            error =
              'Invalid date/time. Must be of the format MM-DD-YYYY HH:MM AM/PM'
          }
        }
      } else {
          ...
          if (
            ...
            slot.value = {
              Envelope: {
                srsName: 'urn:ogc:def:crs:EPSG:4326',
                lowerCorner:
                  this.get('valueLowerLon') + ' ' + this.get('valueLowerLat'),
                upperCorner:
                  this.get('valueUpperLon') + ' ' + this.get('valueUpperLat'),
              },
            }
          }
