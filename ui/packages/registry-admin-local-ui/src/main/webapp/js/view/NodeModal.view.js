...
define([
  ...
  NodeModal.View = Marionette.Layout.extend({
    ...
    refreshData: function() {
      ...
      this.listenTo(
        wreqr.vent,
        'fieldErrorChange:' + this.model.generalInfo.get('segmentId'),
        this.updateGeneralTabError
      )
      this.listenTo(
        wreqr.vent,
        'fieldErrorChange:' + this.model.organizationInfo.get('segmentId'),
        this.updateOrgTabError
      )
      this.listenTo(
        wreqr.vent,
        'fieldErrorChange:' + this.model.contactInfo.get('segmentId'),
        this.updateContactTabError
      )
      this.listenTo(
        wreqr.vent,
        'fieldErrorChange:' + this.model.serviceInfo.get('segmentId'),
        this.updateServiceTabError
      )
      this.listenTo(
        wreqr.vent,
        'fieldErrorChange:' + this.model.contentInfo.get('segmentId'),
        this.updateContentTabError
      )

      this.model.generalInfo.get('segments').forEach(
        function(segment) {
          if (this.model.get('summary').constructor === Object) {
            ...
            segment.set(
              'identityNode',
              this.model.get('summary').get('identityNode')
            )
          }
        }.bind(this)
      )
