...
define([
  ...
  Segment.SegmentView = Marionette.Layout.extend({
    ...
    initialize: function(options) {
      ...
      this.listenTo(
        wreqr.vent,
        'fieldChange:' + this.model.get('segmentId'),
        this.updateTitle
      )
      this.listenTo(
        wreqr.vent,
        'fieldErrorChange:' + this.model.get('segmentId'),
        this.updateTitleError
      )
      this.listenTo(
        wreqr.vent,
        'valueAdded:' + this.model.get('segmentId'),
        this.setupPopOvers
      )
      this.listenTo(
        wreqr.vent,
        'valueRemoved:' + this.model.get('segmentId'),
        this.setupPopOvers
      )
      this.addRegions({
        ...
        formAdvancedFields:
          '.form-fields-advanced-' + this.model.get('simpleId'),
      })
      this.events['click .' + this.model.get('simpleId')] = this.toggleCheveron
      this.events[
        'click .advanced-button-' + this.model.get('simpleId')
      ] = this.toggleAdvanced
      this.delegateEvents()
      ...
      this.formFieldsView = new Field.FieldCollectionView({
        collection: new Backbone.Collection(standardFields),
      ...
      this.formAdvancedFieldsView = new Field.FieldCollectionView({
        collection: new Backbone.Collection(advancedFields),
        readOnly: this.readOnly,
      })

      if (
        FieldDescriptors.isCustomizableSegment(this.model.get('segmentType'))
      ) {
        ...
        if (this.model.get('fields').models.length > 0) {
          ...
          this.associationsView = new Association.AssociationCollectionView({
            ...
            collection: new Backbone.Collection(
              associationModel.getAssociationsForId(this.model.get('segmentId'))
            ),
            readOnly: this.readOnly,
          })
        }
    ...
    removeSegment: function() {
      wreqr.vent.trigger(
        'removeSegment:' + this.model.get('parentId'),
        this.model.get('segmentId')
      )
    },
    ...
    toggleAdvanced: function(evt) {
      evt.stopPropagation()
      var advancedSections = this.$(
        '.advanced-section-' + this.model.get('simpleId')
      )
      var advancedButton = this.$(
    ...
    serializeData: function() {
      ...
      data.associationsAreAdvanced =
        FieldDescriptors.configurations.associationsAreAdvanced
      data.customFieldsAreAdvanced =
        FieldDescriptors.configurations.customFieldsAreAdvanced
      data.advancedFields = this.advancedFields
      data.errors = this.model.validationError
      data.customizable =
        FieldDescriptors.isCustomizableSegment(this.model.get('segmentType')) &&
        !this.model.get('multiValued')
      data.showFields = this.model.get('fields').models.length > 0
      data.showAssociations =
        FieldDescriptors.isCustomizableSegment(this.model.get('segmentType')) &&
        this.model.get('fields').models.length > 0
      data.showAdvanced =
        (data.associationsAreAdvanced ||
          data.customFieldsAreAdvanced ||
          this.addvancedFields) &&
        data.customizable
    ...
    toggleCheveron: function() {
      this.$el
        .find('.chevron-' + this.model.get('simpleId'))
  ...
  Segment.SegmentCollectionView = Marionette.CompositeView.extend({
    ...
    initialize: function(options) {
      ...
      this.listenTo(
        wreqr.vent,
        'removeSegment:' + this.model.get('segmentId'),
        this.removeSegment
      )
    },
