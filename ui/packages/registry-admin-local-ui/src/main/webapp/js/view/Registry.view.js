...
define([
  ...
  RegistryView.RegistryPage = Marionette.Layout.extend({
    ...
    onRender: function() {
      this.identityRegion.show(
        new RegistryView.NodeTable({
          collection: new NodeCollection(this.model.getIdentityNode()),
        })
      )
      this.additionalRegion.show(
        new RegistryView.NodeTable({
          collection: new NodeCollection(this.model.getSecondaryNodes()),
          multiValued: true,
          region: 'additional',
          filterInverted: this.model.filterInverted,
        })
      )
      this.remoteNodeRegion.show(
        new RegistryView.NodeTable({
          collection: new NodeCollection(this.model.getRemoteNodes()),
          multiValued: true,
          region: 'remote',
          readOnly: true,
          filterInverted: this.model.filterInverted,
    ...
    deleteAllNodes: function(table) {
      ...
      modal = new RegistryView.DeleteAllModal({
        model: this.model,
        remote: remote,
        nodes: nodes,
      })
    ...
    clientServerMode: function() {
      wreqr.vent.trigger(
        'showModal',
        new RegistryView.ClientServerModal({
          clientMode: this.model.clientMode,
        })
      )
    },
  ...
  RegistryView.NodeRow = Marionette.ItemView.extend({
    ...
    serializeData: function() {
      ...
      if (this.model) {
        data = this.model.toJSON()
        data.filtered =
          (!data.filtered && this.options.filterInverted) ||
          (data.filtered && !this.options.filterInverted)
      }
      data.lastUpdated =
        moment
          .parseZone(this.model.get('modified'))
          .utc()
          .format('MMM DD, YYYY HH:mm') + 'Z'
      data.liveDate =
        moment
          .parseZone(this.model.get('created'))
          .utc()
          .format('MMM DD, YYYY HH:mm') + 'Z'
