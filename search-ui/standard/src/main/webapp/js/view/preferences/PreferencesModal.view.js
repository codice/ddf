/**
 * Copyright (c) Codice Foundation
 *
 * This is free software: you can redistribute it and/or modify it under the terms of the GNU Lesser
 * General Public License as published by the Free Software Foundation, either version 3 of the
 * License, or any later version.
 *
 * This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without
 * even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU
 * Lesser General Public License for more details. A copy of the GNU Lesser General Public License
 * is distributed along with this program and can be found at
 * <http://www.gnu.org/licenses/lgpl.html>.
 *
 **/
/* global define, require */
define([
        'icanhaz',
        'underscore',
        'marionette',
        'backbone',
        'jquery',
        'js/view/Modal',
        'text!templates/preferences/preferences.modal.handlebars',
        'modelbinder'
    ],
    function (ich, _, Marionette, Backbone, $, Modal, preferencesModalTemplate) {

        ich.addTemplate('preferencesModalTemplate',preferencesModalTemplate);

        return Modal.extend({
            template: 'preferencesModalTemplate',
            events: {
                'hidden.bs.modal': 'saveChanges'
            },
            initialize: function () {
                this.modelBinder = new Backbone.ModelBinder();
            },
            onRender: function () {
                var preferences = this.model.get('user').get('user>preferences');
                var preferencesBindings = Backbone.ModelBinder.createDefaultBindings(this.el, 'name');
                this.modelBinder.bind(preferences, this.$el, preferencesBindings);
            },
            onShow: function () {
                require(['spectrum']);
            },
            saveChanges: function () {
                this.model.get('user').get('user>preferences').savePreferences();
            }
        });
    });