 ...
 * 
 **/
package org.codice.ddf.ui.searchui.proxy;
...
public class ProxyServlet extends HttpServlet {
    ...
    protected void readConfigParam(HttpParams hcParams, String hcParamName, Class type) {
        String valStr = getServletConfig().getInitParameter(hcParamName);
        if (valStr == null)
            return;
        Object valObj;
        if (type == String.class) {
            ...
            try {
                ...
                valObj = type.getMethod("valueOf", String.class).invoke(type, valStr);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        }
        hcParams.setParameter(hcParamName, valObj);
    }
    ...
    static {
        ...
        String[] headers = new String[] {"Connection", "Keep-Alive", "Proxy-Authenticate",
        ...
        while (enumerationOfHeaderNames.hasMoreElements()) {
            ...
            while (headers.hasMoreElements()) { // sometimes more than one value
                ...
                if (headerName.equalsIgnoreCase(HttpHeaders.HOST)) {
                    HttpHost host = URIUtils.extractHost(getTargetUriObj());
     ...
     * {@link #targetUriObj}. It's used to make the new request.
     */
    ...
    protected String rewriteUrlFromResponse(HttpServletRequest servletRequest, String theUrl) {
        ...
        if (theUrl.startsWith(getTargetUri())) {
            ...
            theUrl = curUrl + theUrl.substring(getTargetUri().length());
        }
        return theUrl;
    }
    ...
    public String getTargetUri() {
        return  getTargetUriObj().toString();
    }
    
    public URI getTargetUriObj() {
        URI target = null;
        try {
            target = new URI(ConfigurationStore.getInstance().getWmsServer());
        } catch (Exception e) {
            throw new RuntimeException("Trying to process targetUri from wmsServer configuration value: " + e, e);
        }
        return target;
    }
    ...
    static {
        char[] unreserved = "_-!.~'()*".toCharArray(); // plus alphanum
        char[] punct = ",;:$&+=".toCharArray();
        char[] reserved = "?/[]@".toCharArray(); // plus punct
