    ...
    private void verifyExtractedBytes(final List<byte[]> metadataPackets) {
        ...
        for (int packetNum = 0; packetNum < metadataPackets.size(); ++packetNum) {
            ...
            if (packetNum == 10) {
                ...
                assertThat(metadataPacketBytes.length,
                        is(0xA6 + expectedHeaderMetadataBytesWithPayload.length));
                assertThat(Arrays.copyOf(metadataPacketBytes, 6),
                        is(expectedHeaderMetadataBytesWithPayload));
            } else {
