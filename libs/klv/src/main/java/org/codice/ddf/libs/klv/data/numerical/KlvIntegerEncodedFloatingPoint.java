...
public class KlvIntegerEncodedFloatingPoint extends KlvNumericalDataElement<Double> {
   ...
   *
   * <p>The value returned by this {@code KlvIntegerEncodedFloatingPoint} is calculated as follows:
   * <li>The value returned by {@code klvRawDataValue} is converted to a <strong>long</strong>
   * <li>Scale the value by calculating: (value - {@code encodedRangeMin}) / ({@code
   *     encodedRangeMax} - {@code encodedRangeMin})
   * <li>Apply the scaled value to the actual range: scaled value * ({@code actualRangeMax} - {@code
   *     actualRangeMin}) + {@code actualRangeMin}
   *
   * @param klvRawDataValue the {@link KlvNumericalDataElement} that retrieves the raw numerical
   *     value
  ...
  public KlvIntegerEncodedFloatingPoint(
      final KlvNumericalDataElement<?> klvRawDataValue,
      final long encodedRangeMin,
      final long encodedRangeMax,
      final double actualRangeMin,
  ...
  public KlvIntegerEncodedFloatingPoint(
    ...
    Preconditions.checkArgument(
        encodedRangeMax > encodedRangeMin, "The encoded value range is incorrect.");
    Preconditions.checkArgument(
        actualRangeMax > actualRangeMin, "The actual value range is incorrect.");
  ...
  protected void decodeValue(final Klv klv) {
    ...
    value =
        convert(
            klvRawDataValue.getValue().longValue(),
            encodedRangeMin,
            encodedRangeMax,
            actualRangeMin,
            actualRangeMax);
  ...
  protected KlvIntegerEncodedFloatingPoint copy() {
    return new KlvIntegerEncodedFloatingPoint(
        klvRawDataValue.copy(),
        encodedRangeMin,
        encodedRangeMax,
        actualRangeMin,
        actualRangeMax,
  ...
  /**
   * If there is a floating point error indicator, then use it. Otherwise, check the raw data for an
   * error indicator.
   *
