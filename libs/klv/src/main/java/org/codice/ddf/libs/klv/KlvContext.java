 ...
 * <p>
 * Basically, this class makes it possible to decode KLV-encoded data with an arbitrary structure.
 * The <strong>value</strong> portion of each KLV set can contain either a raw data value (such as a
 * number or string) or another chain of data elements (sometimes called a <i>local set</i>). So,
 * whatever is decoding it needs to know how to interpret the values of the data elements it finds.
 * This class allows you to provide that information so your KLV can be decoded.
 * <p>
 * In essence, a {@code KlvContext} represents the structure of the encoded KLV data you wish to
 * decode.
 */
public class KlvContext {
    ...
    public KlvContext(final KeyLength keyLength, final LengthEncoding lengthEncoding,
        ...
        Preconditions.checkArgument(dataElements != null,
                "The collection of KLVDataElements cannot be null");
    ...
    public void addDataElements(final Collection<? extends KlvDataElement> dataElements) {
        Preconditions.checkArgument(dataElements != null,
                "The collection of data elements cannot be null.");
    ...
    /**
     * Returns the {@link KlvDataElement}s inside this {@code KlvContext} as a {@link Map}, where
     * the keys are the data elements' names and the values are the data elements.
     *
     ...
     * null if this {@code KlvContext} does not have a {@code KlvDataElement} with the name
     * {@code name}
     */
