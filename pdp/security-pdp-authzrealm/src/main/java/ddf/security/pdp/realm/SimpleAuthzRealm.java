...
public class SimpleAuthzRealm extends AbstractAuthorizingRealm
{
    ...
    //This method is for testing purposes only, Mockito was not able to mock the getAuthorizationInfo method
    ...
    /**
     * Checks if the corresponding Subject/user contained within the AuthorizationInfo object implies the given
     * Permission.
    ...
    private boolean isPermitted(Permission permission, AuthorizationInfo info)
    {
        ...
        if (perms != null && !perms.isEmpty())
        {
            ...
            {
                List<KeyValuePermission> metacardPermissions = ((KeyValueCollectionPermission) permission).getKeyValuePermissionList();
                ...
                for (KeyValuePermission metacardPermission : metacardPermissions)
                {
                    ...
                    {
                        SecurityLogger.logInfo("Mapping metacard key " + metacardKey + " to " + matchAllMap.get(metacardKey));
                        KeyValuePermission kvp = new KeyValuePermission(matchAllMap.get(metacardKey), metacardPermission.getValues());
                    ...
                    {
                        SecurityLogger.logInfo("Mapping metacard key " + metacardKey + " to " + matchOneMap.get(metacardKey));
                        KeyValuePermission kvp = new KeyValuePermission(matchOneMap.get(metacardKey), metacardPermission.getValues());
                        matchOnePermissions.add(kvp);
                    }
                    // this metacard key was not specified in either - default to match all with the same key value
                    else
                    {
                ...
                KeyValueCollectionPermission matchAllCollection = new KeyValueCollectionPermission(matchAllPermissions);
                KeyValueCollectionPermission matchOneCollection = new KeyValueCollectionPermission(matchOnePermissions);
    ...
    /**
     * Returns a collection of {@link Permission} objects that the {@link AuthorizationInfo} object of a {@link ddf.security.Subject}
     * is asserting.
     * @param info      the application-specific subject/user identifier.
    ...
    /**
     * Returns a collection of {@link Permission} objects that are built from the associated collection of Strings.
    ...
    /**
     * Returns a collection of {@link Permission} objects that are built from the associated collection of Strings that
     * represent the roles that a user possesses.
