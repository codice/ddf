...
[ditaa, content_framework_architecture, png]
....
+---------------------------------------------------------------------------+             
|                 /-=-----------------\                                     |
|                 |      Clients      |                                     |
...
[plantuml, content_framework_design, png]
....
title <b>Content Framework Design</b>

class ContentRestEndpoint {
    Response create(InputStream, String contentType, String directive, String contentUri)
    Response read(String id)
    Response update(InputStream, String id, String contentType, String directive)
    Response updateCatalogOnly(InputStream, String contentType, String contentUri) 
    Response delete(String id, String directive)
    Response deleteCatalogOnly(String contentUri)
}
...
interface ContentFramework {
    ...
    DeleteResponse delete(DeleteRequest)
}
...
interface MimeTypeMapper {
    String getMimeTypeForFileExtension(String)
    String getFileExtensionForMimeype(String)
}
...
interface ContentPlugin {
    CreateResponse processCreate(CreateResponse)
    UpdateResponse processUpdate(UpdateResponse)
    DeleteResponse processDelete(DeleteResponse)
}

interface InputTransformer {
    Metacard transform(InputStream)
    Metacard transform(InputStream, String id)
}
...
Client -> ContentRestEndpoint
ContentRestEndpoint --> IncomingContentItem : creates
ContentItem <|.. IncomingContentItem
ContentRestEndpoint -> ContentFrameworkImpl : invokes
ContentFramework <|.. ContentFrameworkImpl
ContentFrameworkImpl -> FileSystemProvider : invokes
StorageProvider <|.. FileSystemProvider
FileSystemProvider --> MimeTypeMapperImpl : uses
MimeTypeMapper <|.. MimeTypeMapperImpl
MimeTypeMapperImpl o-- MimeTypeResolver : looks up in OSGi registry
MimeTypeResolver <|.. TikaMimeTypeResolver
MimeTypeResolver <|.. CustomMimeTypeResolver
FileSystemProvider --> ContentFile : creates
ContentItem <|.. ContentFile
ContentFrameworkImpl o-- ContentPlugin : invokes
ContentPlugin <|.. CatalogContentPlugin
CatalogContentPlugin o-- InputTransformer : looks up in OSGi registry
InputTransformer <|.. GeoJsonInputTransformer
CatalogContentPlugin -> CatalogFramework : invokes

note "Adds support for application/json;id=geojson,\nand image/nitf mime types" as Note1
Note1 .. CustomMimeTypeResolver

note "Only component in Content Framework that\ninterfaces with Catalog Framework" as Note3
CatalogContentPlugin .. Note3

note "Assigns GUID for ContentItem on create requests" as Note4
ContentFrameworkImpl .. Note4

note top of ContentRestEndpoint : directive STORE = only store content in content repository\nSTORE_AND_PROCESS = store content in content repository and parse content\nto create Metacard and create catalog entry in MDC
....
...
[plantuml, content_api_packaging, png]
....
...
[plantuml, content_api_reference_impl, png]
....
package "DDF Content API Impl" <<Folder>> #DDDDDD {
package ddf.content.impl <<Node>> {
class ContentFrameworkImpl
}
package ddf.content.operation.impl <<Node>> {
class OperationImpl
class RequestImpl
class ResponseImpl
...
class ReadResponseImpl
class UpdateRequestImpl
class UpdateResponseImpl
class DeleteRequestImpl
class DeleteResponseImpl 
}
package ddf.content.provider.file <<Node>> {
class FileSystemProvider
}
package ddf.content.plugin.catalog <<Node>> {
class CatalogContentPlugin
class Cataloger
}
}
...
[ditaa, content_data_components, png]
....
+---------------------------------------------------------------------------+             
|                 /-=-----------------\                                     |
|                 |      Clients      |                                     |
...
[plantuml, content_framework_execution_flow, png]
....
autonumber 

title Content File Creation 
hide footbox 
...
actor Client 
box "DDF Content Framework" #DDEEFF 
  participant "Content REST Service Endpoint" as ContentRestEndpoint <<Endpoint>>
  participant "Incoming Content Item" as IncomingContentItem <<ContentItem>>
  participant "Content Framework" as ContentFramework <<ContentFramework>>
  participant "File System Provider" as FileSystemProvider <<StorageProvider>>
  participant "Content File" as ContentFile <<ContentItem>>
  participant "Mime Type Mapper" as MimeTypeMapper <<MimeTypeMapper>>
  participant "DDF Custom Mime Type Resolver" as CustomMimeTypeResolver <<MimeTypeResolver>>
  participant "Catalog Content Plugin" as CatalogContentPlugin <<ContentPlugin>>
  participant "GeoJson Input Transformer" as GeoJsonInputTransformer <<InputTransformer>>
end box 
participant "Catalog Framework" as CatalogFramework <<CatalogFramework>>

 

Client -> ContentRestEndpoint : create content
ContentRestEndpoint -> IncomingContentItem: create(stream,mimeType)
ContentRestEndpoint -> ContentFramework: create(CreateRequest) 
ContentFramework -> FileSystemProvider: create(CreateRequest)
FileSystemProvider -> MimeTypeMapper: getFileExtensionForMimeType(mimeType)
note right
Searches all registered MimeTypeResolvers until file extension found
The mime type will be application/json;id=geojson
end note
MimeTypeMapper -> CustomMimeTypeResolver: getFileExtensionForMimeType(mimeType)
CustomMimeTypeResolver --> MimeTypeMapper: file extension
MimeTypeMapper --> FileSystemProvider: file extension 
FileSystemProvider -> FileSystemProvider: create GUID for content item to be stored
FileSystemProvider -> ContentFile: create(stream,id,mimeType)
FileSystemProvider -> FileSystemProvider: store content to file system
FileSystemProvider --> ContentFramework: CreateResponse
ContentFramework -> CatalogContentPlugin: processCreate(CreateResponse)
CatalogContentPlugin -> CatalogContentPlugin: look up registered InputTransformer for mime type
CatalogContentPlugin -> GeoJsonInputTransformer : InputStream
GeoJsonInputTransformer -> GeoJsonInputTransformer : parse GeoJson and create Metacard
GeoJsonInputTransformer --> CatalogContentPlugin: Metacard
CatalogContentPlugin -> CatalogFramework: createRequest(CreateRequest) with GeoJson-parsed Metacard
CatalogFramework --> CatalogContentPlugin: CreateResponse with new catalog ID
CatalogContentPlugin -> CatalogContentPlugin: add new catalog ID to properties of CreateResponse
CatalogContentPlugin --> ContentFramework: CreateResponse
ContentFramework --> ContentRestEndpoint: CreateResponse
ContentRestEndpoint -> Client: Response with contentID, catalogID, and Location URI
....
