...
box "Catalog" #DDEEFF
   ...
   participant "Standard Catalog Framework" as StandardCatalogFramework <<CatalogFramework>>
   participant "PreIngestPlugin" as PreIngestPlugin <<PreIngestPlugin>> 
   participant "PostIngestPlugin" as PostIngestPlugin <<PostIngestPlugin>> 
   participant "Solr Catalog Provider" as CatalogProvider <<CatalogProvider>> 
end box 
participant "Solr Search Server" as DataSource <<External>>

Client -> IngestServiceEndpoint: Web Service Ingest Request
IngestServiceEndpoint -> StandardCatalogFramework: create(CreateRequest)
StandardCatalogFramework -> PreIngestPlugin: process(CreateRequest)
PreIngestPlugin --> StandardCatalogFramework: CreateRequest
StandardCatalogFramework -> CatalogProvider: create(CreateRequest)
CatalogProvider -> DataSource: create
 
DataSource --> CatalogProvider: response
CatalogProvider --> StandardCatalogFramework: CreateResponse
StandardCatalogFramework -> PostIngestPlugin: process(CreateResponse)
PostIngestPlugin --> StandardCatalogFramework: CreateResponse
StandardCatalogFramework --> IngestServiceEndpoint: CreateResponse
IngestServiceEndpoint --> Client: Web Service Ingest Response 
....
...
actor Client
box "Catalog" #DDEEFF
   participant "Query Service Endpoint" as QueryServiceEndpoint <<Endpoint>>
   participant "Standard Catalog Framework" as StandardCatalogFramework <<CatalogFramework>>
   participant "PreQueryPlugin" as PreQueryPlugin <<PreQueryPlugin>> 
   participant "PostQueryPlugin" as PostQueryPlugin <<PostQueryPlugin>> 
   participant "Solr Catalog Provider" as CatalogProvider <<CatalogProvider>>
end box 
participant "Solr Search Server" as DataSource <<External>>

Client -> QueryServiceEndpoint: Web Service Query Request
QueryServiceEndpoint -> StandardCatalogFramework: query(QueryRequest)
StandardCatalogFramework -> PreQueryPlugin: process(QueryRequest)
PreQueryPlugin --> StandardCatalogFramework: QueryRequest
StandardCatalogFramework -> CatalogProvider: query(QueryRequest)
CatalogProvider -> DataSource: query
 
DataSource --> CatalogProvider: response
CatalogProvider --> StandardCatalogFramework: QueryResponse
StandardCatalogFramework -> PostQueryPlugin: process(QueryResponse)
PostQueryPlugin --> StandardCatalogFramework: QueryResponse
StandardCatalogFramework --> QueryServiceEndpoint: QueryResponse
QueryServiceEndpoint --> Client: Web Service Query Response 
....
...
==== Design
[plantuml, product_download_cache, png]
....
title <b>Product Download and Caching Design</b>
 
class CatalogFramework
class ReliableResourceDownloadManager
interface ResourceRetriever
interface Callable
class ReliableResource
class ReliableResourceInputStream
 
Client --> CatalogFramework : getResource
CatalogFramework --> ReliableResourceDownloadManager : download
ReliableResourceDownloadManager --> ResourceRetriever : retrieve
ResourceRetriever --> RemoteSource : reads product
ResourceRetriever --> ProductInputStream : returns
ReliableResourceDownloadManager --> DownloadsStatusEventPublisher : post events
ReliableResourceDownloadManager --> ReliableResource : creates
ReliableResourceDownloadManager --> ReliableResourceInputStream : creates
ResourceResponse --> ReliableResourceInputStream : contains
ReliableResourceDownloadManager --> ResourceResponse : returns
Client --> ResourceResponse : uses
ReliableResourceDownloadManager --> ResourceCache : add/remove cache/pending entries
ResourceCache --> ReliableResource : caches
ReliableResourceDownloadManager --> DownloadManagerState : updates
ReliableResourceDownloadManager --> ReliableResourceCallable : creates
ReliableResourceDownloadManager --> ResourceRetrievalMonitor : creates
TimerTask <|-- ResourceRetrievalMonitor
ResourceRetrievalMonitor --> ReliableResourceCallable : monitors/interrupts
Client --> ReliableResourceInputStream : reads/closes
Callable <|.. ReliableResourceCallable
ReliableResourceCallable --> ProductInputStream : reads
ReliableResourceCallable --> FileBackedOutputStream : writes to
ReliableResourceCallable --> FileOutputStream : writes to
ReliableResourceInputStream --> FileBackedOutputStream : reads
ReliableResourceInputStream --> DownloadManagerState : reads
....
...
The Schematron Validation Library can be installed and uninstalled using the normal processes described in the 
Configuring DDF
 section.
