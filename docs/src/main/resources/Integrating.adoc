...
The DDF Metrics Reporting application provides access to historical data in a graphic, a comma-separated values file, a spreadsheet, a PowerPoint file, XML, and JSON formats for system metrics collected while DDF is running. Summary reports (weekly, monthly, and yearly) are also provided where all collected metrics are included in the report. Summary reports are available in Excel format.

=== Usage

The DDF Metrics Reporting application provides an Admin UI plugin that adds a new tab to the Platform Application with the title of Metrics. When selected, the Metrics tab displays a list of all of the metrics being collected by DDF, e.g., Catalog Queries, Catalog Queries Federated, Catalog Ingest Created, etc.
...
All of the metric graphs displayed are in PNG format and are displayed on their own page. The user may use the backspace key to return to the main view, or, when selecting the hyperlink for a graph, they can use the right mouse button in the browser to display the graph in a separate browser tab or window, which will keep the Admin UI displayed. The screen shot below is a sample graph of the Catalog Queries metrics data for the previous 15 minutes from when the link was selected. Note that the y-axis label and the title use the metrics name (Catalog Queries) by default. The average min and max of all of the metrics data is summarized in the lower left corner of the graph.
...
|summaryInterval
|(optional) the time unit by which to summarize an summary report +
_This parameter is only available for the Excel format._
...
The metric's historical data can be displayed in several formats, including the PNG format previously mentioned, a CSV file, an Excel .xls file, a PowerPoint .ppt file, an XML file, and a JSON file. The PNG, CSV, and XLS formats are accessed via hyperlinks provided in the Metrics tab. The PPT, XML, and JSON formats are accessed by specifying the format in the custom URL, e.g., `http://<DDF_HOST>:<DDF_PORT>/services/internal/metrics/catalogQueries.json?dateOffset=1800`.
...
The Metrics tab also provides summary reports for the collected metrics. These are reports that include data for all of the collected metrics for the specified time range.

The summary reports provided are:

* available for the last [1,2,3,6]
* _complete_ [hours,days,weeks,months,years]
* summarized by [minute,hour,day,week,month]

Note that the summary reports are aligned to complete units, not run up to the instant when the report is requested.

A summary report in XLS format consists of a single workbook (spreadsheet) with a single worksheet in it, titled with the time range for the entire summary. The worksheet displays:

* the metric's name and whether it is a summary value (sum) or an average value (avg)
* a numbered column for each summary interval, displaying the appropriate value on each row for each metric in that interval

A selector is provided to generate summary reports for a given number of time units, summarized by a given unit. Summary reports for custom time ranges can also be accessed directly via the URL: 
----
http://<DDF_HOST>:<DDF_PORT>/services/internal/metrics/report.<format>?startDate=<start_date_value>&endDate=<end_date_value>&summaryInterval=<interval>
----
where `<format>` is `xls`, the `<start_date_value>` and `<end_date_value>` specify the custom time range for the report, and summaryInterval is the level to roll up the data to. Reports requested via custom URLs will not do any complete unit alignment, in order to allow complete control over the result.

The table below list several examples for custom summary reports. (NOTE: all example URLs begin with:
----
http://<DDF_HOST>:<DDF_PORT>
----
which is omitted in the table for brevity.)
...
|XLS summary report for March 15, 2013 to April 15, 2013, summarized by week
|/services/internal/metrics/report.xls?startDate=2013-03-15T12:00:00-07:00&endDate=2013-04-15T12:00:00-07:00&summaryInterval=week

|XLS summary report for last 8 hours, summarized by hour
|/services/internal/metrics/report.xls?dateOffset=28800&summaryInterval=hour
