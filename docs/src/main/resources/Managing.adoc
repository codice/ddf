...
----
+
. Go to https://localhost:8993/admin and step through the installer leaving all the defaults.
...
|*Security STS Server
|Signature Username
|<Alias> *Only change this filed if you change the alias in the keystore
...
$> vi user.properties
## Edit this line
localhost=localhost,admin
## To be:
<FQDN>=<FQDN>,admin## Save and close the user.properties
----
...
$> keytool -changealias -alias 1 -destalias ddf -keystore serverKeystore.jks
...
$> keytool -import -trustcacerts -alias <CA alias> -file <CA PEM> -keystore truststore.jks
----
[NOTE]
====
...
If you do not need a specific CA to sign your certificates and simply want to use the Demo CA that is shipped with {branding}, do the following:
[NOTE]
====
...
. Unzip {branding} into the desired location. All relative directory paths are relative to the directory in which {branding} is unzipped. This will be referred to as <{branding}_HOME>.
