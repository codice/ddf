...
image::EclipseFormatterWindow.png[Eclipse Code Formatter Window]
...
image::save_actions.png[Save Actions]
...
[NOTE]
====
DDF 2.2.0+ should have this issue resolved by upgrading Karaf to v2.3.x+. Refer to com.atlassian.confluence.macro.MacroExecutionException: The JIRA server returned a trusted apps error: APP_UNKNOWN;	Unknown Application: {0};	["confluence:10311375"] (DDF-46)and KARAF-771 (https://issues.apache.org/jira/browse/KARAF-771) for more details.
====

After downloading the application's KAR file:

. Copy the KAR file to a temp directory.
. Unjar the KAR file in this temp directory.
. Edit the `etc/org.ops4j.pax.url.mvn.cfg` configuration file. 
. Uncomment the `org.ops4j.pax.url.mvn.repositories` property if it is commented out. 
. Append the `org.ops4j.pax.url.mvn.repositories` property with file:`<temp_dir>/<kar_root_dir>/repository`.
. Restart DDF.
. Add the KAR's feature repository URL using either the command line console or the Features tab in the Web Console.

Example:
Assume the application's KAR file is named `new-app-1.2.3.kar`

. Copy `new-app-1.2.3.kar` to `/my_kars temp` directory.
. `unjar xvf /my_kars/new-app-1.2.3.kar`
. Edit the `<DDF_INSTALL_DIR>/etc/org.ops4j.pax.url.mvn.cfg` configuration file.
. Append `file:/my_kars/new-app-1.2.3/repository` to the `org.ops4j.pax.url.mvn.repositories` property.
. Restart DDF.
. At the `ddf@local>` prompt enter `features:addurl file:/my_kars/new-app-1.2.3/repository/new-app/1.2.3/new-app-1.2.3-features.xml`
. At the `ddf@local>` prompt enter `features:list`. The new feature(s) loaded by the KAR file should be displayed and active.
