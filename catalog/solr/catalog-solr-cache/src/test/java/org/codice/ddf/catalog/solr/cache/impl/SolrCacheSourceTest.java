...
public class SolrCacheSourceTest {
  ...
  public void setup() throws Exception {
    ...
    when(queryRequest.getProperties()).thenReturn(ImmutableMap.of("cache-sources", "test"));
    ...
    cacheQueryFactory = mock(CacheQueryFactory.class);
    when(cacheQueryFactory.getQueryRequestWithSourcesFilter(any())).thenReturn(queryRequest);

    solrCacheSource = new SolrCacheSource(solrCache, cacheQueryFactory);
  }

  @Test
  public void testDescribableProperties() {
    assertThat(solrCacheSource.getId(), is("cache"));
