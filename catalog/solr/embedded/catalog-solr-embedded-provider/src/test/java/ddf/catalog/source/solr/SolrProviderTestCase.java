...
public abstract class SolrProviderTestCase {
    ...
    protected static final Logger LOGGER = Logger
            .getLogger(TestSolrProvider.class);
    ...
    protected static void deleteAllIn(SolrCatalogProvider provider) throws IngestException, UnsupportedQueryException {
    ...
    protected static void deleteAllIn(SolrCatalogProvider provider, int methodNameIndex)
        ...
        messageBreak(Thread.currentThread().getStackTrace()[methodNameIndex]
                .getMethodName() + "()");
        ...
        provider.delete(new DeleteRequestImpl(
                ids.toArray(new String[ids.size()])));

        LOGGER.info("Deletion complete. -----------");
    }

    protected void deleteAll() throws IngestException,
            UnsupportedQueryException {
        deleteAllIn(provider);
    }

    protected void queryAndVerifyCount(int count, Filter filter)
    ...
    protected DeleteResponse delete(String[] identifier) throws IngestException {
        DeleteResponse deleteResponse = provider.delete(new DeleteRequestImpl(
                identifier));

        return deleteResponse;
    }

    protected UpdateResponse update(String id, Metacard metacard)
            throws IngestException {
        String[] ids = {id};
        return update(ids, Arrays.asList(metacard));
    }

    protected UpdateResponse update(String[] ids, List<Metacard> list)
            throws IngestException {
        UpdateResponse updateResponse = provider.update(new UpdateRequestImpl(
                ids, list));
    ...
    protected CreateResponse create(List<Metacard> metacards)
            throws IngestException {
        return createIn(metacards, provider);
    }

    protected static CreateResponse createIn(List<Metacard> metacards,
            SolrCatalogProvider provider) throws IngestException {
        CreateResponse createResponse = provider.create(new CreateRequestImpl(
                metacards));
