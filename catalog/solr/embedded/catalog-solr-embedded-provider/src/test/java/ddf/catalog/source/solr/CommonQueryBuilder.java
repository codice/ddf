...
public class CommonQueryBuilder {
	...
	/**
	 * Builds an OGC {@link PropertyIsEqualTo} filter and returns a generic
	 * {@link QueryImpl} with a start index of 1 and request for total count set
	 * to true
	 * 
	 * @param property - the property you are searching
	 * @param value - the value that you are comparing against the property
	...
	/**
	 * Builds {@link QueryImpl} with a start index of 1 and request for total count set
	 * to true that contains all the filter logic to request documents from the given List of ids.
	 * @param ids
	 * @return {@link QueryImpl}
	 */
	public QueryImpl read(List<String> ids) {
		...
		for(String id : ids ) {
			...
			orFilters.add(filterFactory.equals(filterFactory.property(Metacard.ID), filterFactory.literal(id))) ;
	...
	/**
	 * Builds an OGC {@link PropertyIsLike} filter and returns a generic
	 * {@link QueryImpl} with a start index of 1 and request for total count set
	 * to true. 
	 * 
	 * <p> The wildcard is *, single character wildcard is '?', and the escape is "\"
	 * </p>
	 * 
	 * @param property
	 * @param searchPhrase
	 * @return
	 */
	public QueryImpl like(String property, String searchPhrase, boolean isCaseSensitive, boolean isFuzzy) {
		
		QueryImpl query = new QueryImpl( filterFactory.like(filterFactory.property(property), searchPhrase, "*", "?", "\\", isCaseSensitive));
	...
	public Filter equalTo(String property, String searchPhrase, boolean isCaseSensitive) {
		...
		return filterFactory.equal(filterFactory.property(property), filterFactory.literal(searchPhrase),isCaseSensitive);

	}
	
	public QueryImpl contextual(String searchPhrase, boolean isCaseSensitive, boolean isFuzzy) {
		
		QueryImpl query = new QueryImpl( filterFactory.like(filterFactory.property(Metacard.ANY_TEXT), searchPhrase, "*", "?", "\\", isCaseSensitive));
...
//		double[] coords = {x, y} ;
//		
//		QueryImpl query = new QueryImpl( filterFactory.within(Metacard.ANY_GEO, new PointImpl(new DirectPositionImpl(coords), DefaultGeographicCRS.WGS84))); 
//
//		query.setStartIndex(1) ;
//		
//		query.setRequestsTotalResultsCount(true);
...
//	}
	...
	/**
	 * Creates a {@link QueryImpl} that sorts by distance, startIndex = 1, and nearest neighbor at the coordinates given with the WGS84 CRS.
	 * 
...
//		double[] coords = {x, y} ;
//		
//		QueryImpl query = new QueryImpl( filterFactory.intersects(Metacard.ANY_GEO, new PointImpl(new DirectPositionImpl(coords), DefaultGeographicCRS.WGS84))); 
//
//		query.setStartIndex(1) ;
//		
//		query.setRequestsTotalResultsCount(true);
...
//				filterFactory.like(
//				new FuzzyFunction(
//						Arrays.asList((Expression)(filterFactory.property(Metacard.ANY_TEXT))), filterFactory.literal("")), 
//				searchPhrase, 
//				"*", "?", "\\", 
//				isCaseSensitive) ) ;
//				
...
//	}
	...
	public QueryImpl during(String property, Date start, Date end) {
		...
		Filter filter = filterFactory.during(
				filterFactory.property(property), 
