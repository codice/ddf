 ...
 * 
 * Tweaks were made to the BNF for performance and because of mistakes found in
 * the BNF.
 * 
 ...
 * 
 * <br/>
 * <b>Some mistakes that were corrected in the Oracle BNF:</b> <li>Allowing
 * NCName to accept a colon as a valid character (It is not part of the BNF but
 * Oracle DB allows it).</li> <li>
...
public class TextPathParser extends BaseParser<Object> {
	 ...
	 */
	Rule AndExpr() {
		return Sequence(BooleanExpr(), ZeroOrMore(Sequence(Spacing(), AND, Spacing(), BooleanExpr())));
	}

	/**
	 * Original Oracle BNF excerpt <br/>
	 * BooleanExpr ::= RelativeLocationPath | EqualityExpr | ("(" OrExpr ")") |
	 * ("not" "(" OrExpr ")")
	 */
	// @formatter:off
	Rule BooleanExpr() {
	...
	/**
	 * Original Oracle BNF excerpt <br/>
	 * Literal ::= (DoubleQuote [~"]* DoubleQuote) | (SingleQuote [~']*
	 * SingleQuote)
	 */

	@SuppressSubnodes
	Rule Literal() {
		return FirstOf(Sequence(DQ, ZeroOrMore(NoneOf("\"")), DQ), Sequence(SQ, ZeroOrMore(NoneOf("'")), SQ));
	}
