...
public class TestOpenSearchParserImpl {
    ...
    public void populateSpatialDistanceFilterBbox() throws Exception {
        ...
        openSearchParserImpl.populateGeospatial(webClient, spatialFilter, true, Arrays.asList(
                        "q,src,mr,start,count,mt,dn,lat,lon,radius,bbox,polygon,dtstart,dtend,dateName,filter,sort".split(
                                ",")));
    ...
    public void populateNullSpatialFilter() throws Exception {
        ...
        assertThat(urlString, containsString(OpenSearchParserImpl.SEARCH_TERMS));
        WebClient webClient = WebClient.create(URL.toString());
        openSearchParserImpl.populateGeospatial(webClient, (SpatialFilter)null, true, Arrays.asList(
                "q,src,mr,start,count,mt,dn,lat,lon,radius,bbox,polygon,dtstart,dtend,dateName,filter,sort".split(
                        ",")));
        String urlStr = webClient.getCurrentURI().toString();
        assertThat(urlStr, containsString(OpenSearchParserImpl.SEARCH_TERMS));
    ...
    public void populateContextual() {
        ...
        assertThat(urlString, containsString(OpenSearchParserImpl.SEARCH_TERMS));
        WebClient webClient = WebClient.create(URL.toString());
        openSearchParserImpl.populateContextual(webClient,
                searchPhraseMap,
                Arrays.asList(
                        "q,src,mr,start,count,mt,dn,lat,lon,radius,bbox,polygon,dtstart,dtend,dateName,filter,sort".split(
                                ",")));
        String urlStr = webClient.getCurrentURI()
                .toString();
    ...
    public void populateTemporal() {
        ...
        WebClient webClient = WebClient.create(URL.toString());
        openSearchParserImpl.populateTemporal(webClient,
                temporal,
                Arrays.asList(
                        "q,src,mr,start,count,mt,dn,lat,lon,radius,bbox,polygon,dtstart,dtend,dateName,filter,sort".split(
                                ",")));
    ...
    public void populateSearchOptions() throws UnsupportedEncodingException {
        ...
        openSearchParserImpl.populateSearchOptions(webClient,
                queryRequest,
                null,
                Arrays.asList(
                        "q,src,mr,start,count,mt,dn,lat,lon,radius,bbox,polygon,dtstart,dtend,dateName,filter,sort".split(
    ...
    public void populatePolyGeospatial() throws Exception {
        ...
        WebClient webClient = WebClient.create(URL.toString());
        openSearchParserImpl.populateGeospatial(webClient,
                spatial,
                false,
                Arrays.asList(
                        "q,src,mr,start,count,mt,dn,lat,lon,radius,bbox,polygon,dtstart,dtend,dateName,filter,sort".split(
