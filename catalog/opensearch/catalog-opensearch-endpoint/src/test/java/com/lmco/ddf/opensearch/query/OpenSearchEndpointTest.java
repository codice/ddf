...
public class OpenSearchEndpointTest {
    ...
    private static final XLogger LOGGER = new XLogger( LoggerFactory.getLogger(  OpenSearchEndpointTest.class ) );
     ...
     * 
     * This test will verify that the string "local" in the sources passed to OpenSearchEndpoint.processQuery is replaced with
     * the local site name (in this case the mocked name "TestSiteName").  The QueryRequest object is checked when the framework.query
     * is called to retrieve the OpenSearchQuery, which contains the Set of sites.  An assertion within the Answer object for the 
     * call framework.query checks that the sites Set is contains the TEST_SITE_NAME.
     * 
    ...
    public void testProcessQueryForProperHandlingOfSiteNameLOCAL() throws URISyntaxException, UnsupportedQueryException, SourceUnavailableException, FederationException, UnsupportedEncodingException, CatalogTransformerException {
        ...
        when(mockFramework.query(any(QueryRequest.class))).thenAnswer(new Answer<Object>(){
            public Object answer(InvocationOnMock invocation)
            {
                ...
                Assert.assertTrue(((OpenSearchQuery)queryRequest.getQuery()).getSiteIds().contains(TEST_SITE_NAME));
