...
/**
 * Abstract Syntax Tree Node </br>
 * Abstract class that extends built-in Parboiled tree node class. </br>
 * </br>
 * During the parsing of the keyword string, the parser creates and stores instances of this class's subclasses on the
 * processing stack. This allows the parser to build an Abstract Syntax Tree from the keyword string.
 */
...
public abstract class ASTNode extends ImmutableBinaryTreeNode<ASTNode>
{
    ...
    public enum Operator
    {
        ...
        public static Operator getOperatorFromString(String operatorString)
        {
            ...
            else if (operatorString.contains(KeywordTextParser.SPACE_STRING) && operatorString.trim().isEmpty())
            {
                return Operator.AND;
            }
            else
