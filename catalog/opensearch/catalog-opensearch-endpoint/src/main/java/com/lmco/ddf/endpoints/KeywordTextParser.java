...
/**
 * This parser is based on a modified version of the "IC/DoD Keyword Query Language Specification, V2.0" DRAFT (4 September 2012).
 * This spec includes an EBNF that this parser is based on.
 * All changes to that EBNF were made to add variable whitespace handling and to make parsing more efficient.
 */
...
public class KeywordTextParser extends BaseParser<ASTNode>
{
    ...
    Rule KeywordQueryExpression()
    {
        ...
        return Sequence(OptionalWhiteSpace(), Term(), ZeroOrMore(BooleanOperator(), operator.set(match()), Term(), push(new OperatorASTNode(operator.get(), pop(1), pop()))), OptionalWhiteSpace());
    }
     ...
     * <p>
     * The implementation was changed to evaluate OR and NOT first, so all spaces aren't evaluated as ANDs. <br/>
     ...
     * <and> ::= “ AND ” | “ ”;
     * <p>
     * The implementation was changed to allow whitespace and to not require boolean operators to be wrapped in spaces. <br/>
     * and = (optional whitespace, "AND", optional whitespace) | mandatory whitespace;
     * </p>
     */
    Rule And()
    {
        return FirstOf(Sequence(OptionalWhiteSpace(), AND_OPERATOR, OptionalWhiteSpace()), MandatoryWhiteSpace());
    }
    ...
    Rule Phrase()
    {
        ...
        return Sequence(OptionalWhiteSpace(), DBLQUOTE, OptionalWhiteSpace(), push(new PhraseDelimiterASTNode()), Keyword(), ZeroOrMore(Sequence(OptionalWhiteSpace(), Keyword())), StackPhraseRewriteAction, OptionalWhiteSpace(), DBLQUOTE);
    }
    ...
    {
        //only grab leading spaces
        return Sequence(OptionalWhiteSpace(), LPAR, OptionalWhiteSpace(), KeywordQueryExpression(), OptionalWhiteSpace(), RPAR);
    }
   ...
   Rule Keyword()
   {
       //TODO the default value is used to allow the parser to keep running during error recovery... is this right?
    ...
    /**
     * This was added to allow whitespace.
     * optional whitespace = {' '};
     */
    @SuppressNode
    ...
    public class StackPhraseRewriteAction implements Action
    {
        ...
        //there doesn't appear to be a better way to do this in parboiled without accessing the stack directly
