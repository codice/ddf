    ...
    public Filter getFilterFromASTNode(ASTNode astNode, String xpathSelector)
        ...
        if (astNode.isKeyword()) {
            if (xpathSelector != null && !xpathSelector.isEmpty()) {
                ...
                return filterBuilder.attribute(Metacard.ANY_TEXT).is().like()
                        .text(astNode.getKeyword());
            }
        }
        ...
        if (astNode.isOperator()) {
            ...
            case AND:
                return filterBuilder.allOf(getFilterFromASTNode(astNode.left()),
                        getFilterFromASTNode(astNode.right()));
            case OR:
                return filterBuilder.anyOf(getFilterFromASTNode(astNode.left()),
                        getFilterFromASTNode(astNode.right()));
            case NOT: // since NOT really means AND NOT
                return filterBuilder.allOf(getFilterFromASTNode(astNode.left()),
                        filterBuilder.not(getFilterFromASTNode(astNode.right())));
            default:
                throw new IllegalStateException(
                        "Unable to generate Filter from invalid OperatorASTNode.");
            }
        }
