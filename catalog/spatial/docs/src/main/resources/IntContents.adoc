...
The following example shows a new record inserted into the MapInfo Spatial Server core schema.
...
The following example shows the how the newly inserted record could be updated to modify the date field.
...
The following example shows a delete operation that is deleting all records in the core MapInfo Spatial Server schema with a SRS name equal to wgs84.

.Sample XML Transaction Delete Request without Constraint
[source,xml,linenums]
----
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
