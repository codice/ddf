...
The CSW Transactions endpoint only supports `HTTP POST` requests since there is no KVP operations.
...
The following example shows a delete operation that is deleting all records with a SpatialReferenceSystem name equal to `wgs84`.
