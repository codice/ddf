...
public class MetacardToKml {
  ...
  private static Geometry createPolygonGeo(Polygon jtsPoly) {
    ...
    for (com.vividsolutions.jts.geom.Coordinate coord :
        jtsPoly.getExteriorRing().getCoordinates()) {
    ...
    for (int n = 0; n < jtsPoly.getNumInteriorRing(); n++) {
      ...
      com.vividsolutions.jts.geom.Coordinate[] childCoordinates =
          jtsPoly.getInteriorRingN(n).getCoordinates();
      for (com.vividsolutions.jts.geom.Coordinate coord : childCoordinates) {
        kmlCoords.add(new Coordinate(coord.x, coord.y));
      }
    }
    return kmlPoly;
