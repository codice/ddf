...
public class TestGetRecordsResponseConverter {
  ...
  /**
   * This test actually runs the full thread of calling the GetRecordsResponseConverter then calls
  ...
  public void testUnmarshalGetRecordsResponseFull() {
    ...
    when(mockInputManager.getTransformerBySchema(anyString()))
        .thenReturn(new CswRecordConverter(TestCswRecordConverter.getCswMetacardType()));
  ...
  private void getRecords(
    ...
    assertThat(
        response.getSearchResults().getNumberOfRecordsReturned().intValue(), is(expectedReturn));
