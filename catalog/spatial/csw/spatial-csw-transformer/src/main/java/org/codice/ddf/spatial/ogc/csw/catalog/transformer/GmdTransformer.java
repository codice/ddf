...
public class GmdTransformer extends AbstractGmdTransformer implements InputTransformer {
    ...
    public LinkedHashSet<Path> buildPaths() {
        ...
        Arrays.asList(GmdConstants.ABSTRACT_PATH,
    ...
    private void addMetacardAbstract(final XstreamPathValueTracker pathValueTracker,
            MetacardImpl metacard) {
        addFieldsToMetacardIfPresent(pathValueTracker,
                metacard,
    ...
    private void addMetacardCreatedAndModifiedDates(final XstreamPathValueTracker pathValueTracker,
            MetacardImpl metacard) {
    ...
    private void addMetacardKeywords(final XstreamPathValueTracker pathValueTracker,
            MetacardImpl metacard) {
        addFieldsToMetacardIfPresent(pathValueTracker,
                metacard,
                GmdConstants.KEYWORD_PATH,
    ...
    private void addMetacardTopics(final XstreamPathValueTracker pathValueTracker,
            MetacardImpl metacard) {
        addFieldsToMetacardIfPresent(pathValueTracker,
                metacard,
                GmdConstants.TOPIC_CATEGORY_PATH,
    ...
    private void addMetacardAssociations(final XstreamPathValueTracker pathValueTracker,
            MetacardImpl metacard) {
        addFieldsToMetacardIfPresent(pathValueTracker,
                metacard,
                GmdConstants.ASSOCIATION_PATH,
    ...
    private void addMetacardPointOfContact(final XstreamPathValueTracker pathValueTracker,
            MetacardImpl metacard) {
        ...
        if (CollectionUtils.isNotEmpty(deliveryPoints)) {
            ...
            List<String> administrativeAreas =
                    pathValueTracker.getAllValues(toPath(GmdConstants.CONTACT_ADDRESS_ADMINISTRATIVE_AREA_PATH));
    ...
    private void addMetacardAltitude(final XstreamPathValueTracker pathValueTracker,
            MetacardImpl metacard) {
        List<String> altitudeList =
                pathValueTracker.getAllValues(toPath(GmdConstants.ALTITUDE_PATH));
    ...
    private void addMetacardTemporalStartStop(final XstreamPathValueTracker pathValueTracker,
            MetacardImpl metacard) {
    ...
    private void addMetacardResourceStatusCode(final XstreamPathValueTracker pathValueTracker,
            MetacardImpl metacard) {
        addFieldsToMetacardIfPresent(pathValueTracker,
                metacard,
                GmdConstants.RESOURCE_STATUS_PATH,
    ...
    private void addMetacardCountryCodes(final XstreamPathValueTracker pathValueTracker,
            MetacardImpl metacard) {
    ...
    private void addMetacardContactPOCInformation(final XstreamPathValueTracker pathValueTracker,
            MetacardImpl metacard) {
        List<String> organisationNames =
        ...
        List<String> deliveryPoints =
                pathValueTracker.getAllValues(toPath(GmdConstants.POINT_OF_CONTACT_ADDRESS_DELIVERY_POINT_PATH));
        if (CollectionUtils.isNotEmpty(deliveryPoints)) {
            List<String> cities =
                    pathValueTracker.getAllValues(toPath(GmdConstants.POINT_OF_CONTACT_ADDRESS_CITY_PATH));
            List<String> administrativeAreas =
                    pathValueTracker.getAllValues(toPath(GmdConstants.POINT_OF_CONTACT_ADDRESS_ADMINISTRATIVE_AREA_PATH));
            List<String> postalCodes =
                    pathValueTracker.getAllValues(toPath(GmdConstants.POINT_OF_CONTACT_ADDRESS_POSTAL_CODE_PATH));
            List<String> countries =
    ...
    private List<String> filterCountryCodes(List<String> countryCodes) {
        ...
        countryCodes.forEach(countryCode -> {
            if (countryCode.length() == 3) {
                ...
                LOGGER.debug("{} does not match alpha-3 format.  Skipping country code.",
                        countryCode);
            }
        });
    ...
    private void setLocationFromBoundingBox(MetacardImpl metacard, String north, String south,
            String east, String west) {
        try {
            Envelope envelope = new Envelope(Double.parseDouble(east.trim()),
                    Double.parseDouble(west.trim()),
                    Double.parseDouble(south.trim()),
                    Double.parseDouble(north.trim()));
            String wkt = WKT_WRITER_THREAD_LOCAL.get()
                    .write(factory.toGeometry(envelope));
            if (wkt != null) {
                metacard.setAttribute(Core.LOCATION, wkt);
            }
        } catch (NumberFormatException nfe) {
            LOGGER.debug("Unable to parse double from GMD metadata {}, {}, {}, {}",
                    west,
                    east,
                    south,
                    north);
    ...
    private void setLocationFromPolygon(String boundingPolygon, MetacardImpl metacard) {
        try {
            ...
            for (int i = 0; i < stringArray.length - 1; i += 2) {
                coordinates.add(new Coordinate(Double.parseDouble(stringArray[i]),
                        Double.parseDouble(stringArray[i + 1])));
            }

            LinearRing linearRing =
                    factory.createLinearRing(coordinates.toArray(new Coordinate[coordinates.size()]));
            String wkt = WKT_WRITER_THREAD_LOCAL.get()
                    .write(factory.createPolygon(linearRing, null));
            if (wkt != null) {
                metacard.setAttribute(Core.LOCATION, wkt);
            }
        } catch (NumberFormatException nfe) {
            LOGGER.debug("Unable to parse double in {}. Metacard location will not be set.",
                    boundingPolygon);
        }
    }
    ...
    private void setLocationFromLineString(String lineString, MetacardImpl metacard) {
        try {
            ...
            String wkt = WKT_WRITER_THREAD_LOCAL.get()
                    .write(factory.createLineString(coordinates.toArray(new Coordinate[coordinates.size()])));
            if (wkt != null) {
                metacard.setAttribute(Core.LOCATION, wkt);
            }
        } catch (NumberFormatException nfe) {
    ...
    private void setMetacardLocationFromBoundingPolygonPoints(Metacard metacard) {
        try (InputStream inputStream = getSourceInputStream()) {
            ...
            XPath xPath = XPathFactory.newInstance()
                    .newXPath();
            XPathExpression dataExpression =
                    xPath.compile(GmdConstants.BOUNDING_POLYGON_POINT_PATH);
            ...
            if (dataList.getLength() > 0) {
                ...
                for (int i = 0; i < dataList.getLength(); i++) {
                    ...
                    String[] coordinateStrings = node.getTextContent()
                            .split(" ");
    ...
    private void setMetacardDates(Metacard metacard,
        ...
        try (InputStream inputStream = getSourceInputStream()) {
            ...
            if (dataList != null && dataList.getLength() > 0) {
                ...
                if (CollectionUtils.isNotEmpty(dateList) && CollectionUtils.isNotEmpty(dateTypes)
    ...
    public void addFieldToMetacardIfPresent(final XstreamPathValueTracker pathValueTracker,
            MetacardImpl metacard, String value, String metacardField) {
    ...
    public void addFieldsToMetacardIfPresent(final XstreamPathValueTracker pathValueTracker,
            MetacardImpl metacard, String value, String metacardField) {
    ...
    public Path toPath(String stringPath) {
        return new Path(stringPath.replace(GCO_PREFIX, "")
                .replace(GML_PREFIX, ""));
