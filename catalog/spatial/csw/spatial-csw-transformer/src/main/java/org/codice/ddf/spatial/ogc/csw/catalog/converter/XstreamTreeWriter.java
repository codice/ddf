...
public class XstreamTreeWriter {
  ...
  void startVisit(final String node) {
    ...
    for (Path path : pathValues.getPaths()) {
      ...
      if (value != null) {
        ...
        if (isParent(currentPath, path) && path.toString().endsWith(node)) {
  ...
  private boolean isParent(Path currentPath, Path otherPath) {
    return !currentPath.relativeTo(otherPath).toString().contains(PATH_SEP);
  }
    ...
    if (!isAttributeNode(node)) {
      ...
      for (Path path : pathValues.getPaths()) {
        ...
        if (value != null) {
          ...
          if (currentPath.isAncestor(path) && path.toString().endsWith(PATH_SEP + node)) {
