...
public abstract class CswAbstractFilterDelegate<T> extends FilterDelegate<T> {
   ...
   * @param outputFormatValues An {@link DomainType} containing a list of valid Output Formats
   *     supported
   * @param resultTypesValues An {@link DomainType} containing a list of Result Types supported
   */
  public CswAbstractFilterDelegate(
      Operation getRecordsOp, DomainType outputFormatValues, DomainType resultTypesValues) {
    ...
    if (null != resultTypesValues) {
      for (String rt : resultTypesValues.getValue()) {
        try {
          ...
          LOGGER.debug(String.format("\"%s\" is not a ResultType.  Exception: {}", rt), iae);
        }
      }
    }
