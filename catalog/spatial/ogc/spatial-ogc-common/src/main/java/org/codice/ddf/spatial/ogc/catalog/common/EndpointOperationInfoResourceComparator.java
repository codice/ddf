...
public class EndpointOperationInfoResourceComparator extends OperationResourceInfoComparator
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(EndpointOperationInfoResourceComparator.class);
    ...
    if (HTTP_GET.equalsIgnoreCase(httpMethod)) {
      ...
      if (StringUtils.isNotEmpty(queryString)) {
        ...
        MultivaluedMap<String, String> allQueryParams =
            JAXRSUtils.getStructuredParams(queryString, QUERY_PARAM_DELIMITER, false, false);
  ...
  private int getOperationRank(
      OperationResourceInfo operation, String requestName, String requestedService) {
    if (serviceName != null
        && (!serviceName.equalsIgnoreCase(requestedService))
    ...
    return operation.getMethodToInvoke().getName().equalsIgnoreCase(requestName)
        ? 1
        : (operation.getMethodToInvoke().getName().equalsIgnoreCase(UNKNOWN_OPERATION) ? 0 : -1);
  }
}
