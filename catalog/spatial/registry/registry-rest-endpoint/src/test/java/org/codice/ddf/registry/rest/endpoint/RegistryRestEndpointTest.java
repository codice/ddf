...
public class RegistryRestEndpointTest {
  ...
  public void testFullReportWithBlankMetacard() throws IOException {
    ...
    assertThat(
        response.getStatusInfo().getStatusCode(), is(Response.Status.BAD_REQUEST.getStatusCode()));
  }

  @Test
  public void testFullReportWithFederationAdminException()
      throws FederationAdminException, IOException {
    when(federationAdminService.getRegistryObjectByRegistryId(anyString(), anySet()))
        .thenThrow(new FederationAdminException());
  ...
  @Test
  public void testFullReportRegistryPackageNotFound() throws FederationAdminException, IOException {
    when(federationAdminService.getRegistryObjectByRegistryId(anyString(), anySet()))
        .thenReturn(null);
    Response response = restEndpoint.viewRegInfoHtml("metacardId", Arrays.asList("sourceIds"));
    assertThat(
        response.getStatusInfo().getStatusCode(), is(Response.Status.NOT_FOUND.getStatusCode()));
  }
  ...
  public void testFullReportWithIOException() throws IOException, FederationAdminException {
    ...
    when(registryReportBuilder.getHtmlFromRegistryPackage(
            any(RegistryPackageType.class), anyString()))
  ...
  public void testSectionReportWithFederationAdminException()
    ...
    when(federationAdminService.getRegistryObjectByRegistryId(anyString(), anySet()))
        .thenThrow(new FederationAdminException());
    Response response =
        restEndpoint.viewSectionInfoHtml("metacardId", Arrays.asList("sourceIds"), "organizations");
    assertThat(
        response.getStatusInfo().getStatusCode(),
        is(Response.Status.INTERNAL_SERVER_ERROR.getStatusCode()));
  }
  ...
  public void testSectionReportWithUnknownSectionName()
    ...
    when(federationAdminService.getRegistryObjectByRegistryId(anyString(), anySet()))
        .thenReturn(registryPackage);
    Response response =
        restEndpoint.viewSectionInfoHtml(
            "metacardId", Arrays.asList("sourceIds"), "unknownSectionName");
    assertThat(
        response.getStatusInfo().getStatusCode(), is(Response.Status.BAD_REQUEST.getStatusCode()));
  }
  ...
  public void testSummaryReportWithBlankMetacard() throws FederationAdminException, IOException {
    ...
    when(federationAdminService.getRegistryMetacardsByRegistryIds(
            any(Collections.class).singletonList("metcardId")))
        .thenReturn(null);
    assertThat(
        response.getStatusInfo().getStatusCode(), is(Response.Status.NOT_FOUND.getStatusCode()));
  }
  ...
  public void testPublishHelperException() throws Exception {
    doThrow(new FederationAdminException("error"))
        .when(registryPublicationService)
