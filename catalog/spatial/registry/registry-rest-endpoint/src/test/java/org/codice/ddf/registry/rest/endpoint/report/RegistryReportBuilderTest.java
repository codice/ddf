...
public class RegistryReportBuilderTest {
  ...
  public void getErrorHtml() throws IOException {
    ...
    assertThat(Jsoup.parse(html).select(".ErrorMessage").text(), is("New Error Message Here"));
  }
  ...
  public void testOrganizationReportWithSimplifiedRegistryPackage() throws Exception {
    RegistryPackageType registryPackage =
        getRegistryPackageFromResource("/csw-simplified-registry-package.xml");
    String html =
        reportHelper.getHtmlFromRegistryPackage(registryPackage, reportHelper.ORGANIZATIONS);
  ...
  private void assertSummaryReportValues(Document document) {
    ...
    String organizationName =
        document.select(".ValuePair:has(.key):contains(Organization Name)").select(".value").text();
    String phoneNumbers =
        document.select(".ValuePair:has(.key):contains(Phone Numbers)").select(".value").text();
    String emailAddresses =
        document.select(".ValuePair:has(.key):contains(Email Addresses)").select(".value").text();
    String address =
        document.select(".ValuePair:has(.key):contains(Street)").select(".value").text();
    String pointOfContact =
        document.select(".ValuePair:has(.key):contains(Point of Contact)").select(".value").text();
  ...
  private void assertOrganizationReportValues(Document document) {
    ...
    assertThat(document.select(".Address").text(), is("1234 Some Street Phoenix, AZ 85037"));
    assertThat(document.select(".Phone").text(), is("(555) 555-5555 ext. 1234"));
    assertThat(document.select(".Email").text(), is("emailaddress@something.com"));
    assertThat(document.select(".Name").text(), is("Codice"));
    assertThat(
        document.select(".Classifications:contains(classificationScheme)").text(),
        is("classificationScheme"));

    String status =
        document.select(".ValuePair:has(.key):contains(Status)").select(".value").text();
    String parent =
        document.select(".ValuePair:has(.key):contains(parent)").select(".value").text();
    String lid = document.select(".ValuePair:has(.key):contains(Lid)").select(".value").text();
  ...
  private void assertNodeInfo(Document document) {
    String nodeName = document.select(".NodeName:contains(Node Name)").text();
    String inputDataSources =
        document
            .select(".NodeValuePair:has(.NodeKey):contains(inputDataSources)")
            .select(".NodeValue")
            .text();
    String parameters =
        document
            .select(".NodeValuePair:has(.NodeKey):contains(parameters)")
            .select(".NodeValue")
            .text();
    String versionNumber =
        document
            .select(".NodeValuePair:has(.NodeKey):contains(VersionInfo)")
            .select(".NodeValue")
            .first()
            .text();
    String objectType =
        document
            .select(".NodeValuePair:has(.NodeKey):contains(objectType)")
            .select(".NodeValue")
            .text();
  ...
  private void assertServices(Document document) {
    String versionInfo =
        document
            .select(".ServiceValuePair:has(.ServiceKey):contains(VersionInfo)")
            .select(".ServiceValue")
            .text();
    String description =
        document
            .select(".ServiceValuePair:has(.ServiceKey):contains(Description)")
            .select(".ServiceValue")
            .text();
    String id =
        document
            .select(".ServiceValuePair:has(.ServiceKey):contains(id)")
            .select(".ServiceValue")
            .text();
    String name =
        document
            .select(".ServiceValuePair:has(.ServiceKey):contains(Name)")
            .select(".ServiceValue")
            .text();
    String objectType =
        document
            .select(".ServiceValuePair:has(.ServiceKey):contains(objectType)")
            .select(".ServiceValue")
            .text();
    String cswUrl =
        document
            .select(".ServiceValuePair:has(.ServiceKey):contains(cswUrl)")
            .select(".ServiceValue")
            .text();
    String bindingType =
        document
            .select(".ServiceValuePair:has(.ServiceKey):contains(bindingType)")
            .select(".ServiceValue")
            .text();
  ...
  private void assertContacts(Document document) {
    assertThat(
        document.select(".ContactAddress").text(), is("1234 Some Street Phoenix , AZ 85037"));
    assertThat(document.select(".ContactPhone").text(), is("(111) 111-1111 ext. 1234"));
    assertThat(document.select(".ContactEmail").text(), is("emailaddress@something.com"));
    assertThat(document.select(".ContactName").text(), is("foo bar"));

    String id =
        document
            .select(".ContactValuePair:has(.ContactKey):contains(id)")
            .select(".ContactValue")
            .text();
  ...
  private void assertCollections(Document document) {
    String versionInfo =
        document
            .select(".CollectionValuePair:has(.CollectionKey):contains(VersionInfo)")
            .select(".CollectionValue")
            .first()
            .text();
    String description =
        document
            .select(".CollectionValuePair:has(.CollectionKey):contains(Description)")
            .select(".CollectionValue")
            .text();
    String isOpaque =
        document
            .select(".CollectionValuePair:has(.CollectionKey):contains(isOpaque)")
            .select(".CollectionValue")
            .text();
    String parameters =
        document
            .select(".CollectionValuePair:has(.CollectionKey):contains(parameters)")
            .select(".CollectionValue")
            .text();
    String id =
        document
            .select(".CollectionValuePair:has(.CollectionKey):contains(id)")
            .select(".CollectionValue")
            .text();
    String mimeType =
        document
            .select(".CollectionValuePair:has(.CollectionKey):contains(mimeType)")
            .select(".CollectionValue")
            .text();
    String name =
        document
            .select(".CollectionValuePair:has(.CollectionKey):contains(name)")
            .select(".CollectionValue")
            .first()
            .text();
    String contentVersionInfo =
        document
            .select(".CollectionValuePair:has(.CollectionKey):contains(contentVersionInfo)")
            .select(".CollectionValue")
            .first()
            .text();
    assertThat(versionInfo, is("2.10.x"));
  ...
  private RegistryPackageType getRegistryPackageFromResource(String path) throws ParserException {
    ...
    JAXBElement<RegistryPackageType> jaxbRegistryObject =
