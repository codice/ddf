...
public class RegistryRestEndpoint {
  ...
  public Response publish(
      @PathParam("registryId") String registryId, @PathParam("sourceId") String destinationId) {
    ...
    try {
      ...
      String message = String.format("Error publishing node from destination %s", destinationId);
  ...
  public Response unpublish(
    ...
    try {
      ...
      String message = String.format("Error unpublishing node from destination %s", destinationId);
      return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
          .entity(getErrorHtmlString(message))
          .build();
    }
  ...
  @GET
  public Response viewRegInfoHtml(
      @PathParam("registryId") final String registryId,
    ...
    try {
      ...
      registryPackage =
          federationAdminService.getRegistryObjectByRegistryId(registryId, sourceIdsSet);
    } catch (FederationAdminException e) {
      String message = "Error getting registry package.";
      LOGGER.debug(
          "{} For registry id: '{}', optional sources: {}", message, registryId, sourceIds);
      return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
          .entity(getErrorHtmlString(message))
          .build();
    }
    ...
    if (registryPackage == null) {
      ...
      LOGGER.debug(
          "{} For registry id: '{}', optional source ids: {}.", message, registryId, sourceIds);
      return Response.status(Response.Status.NOT_FOUND).entity(getErrorHtmlString(message)).build();
    }

    try {
      html =
          registryReportBuilder.getHtmlFromRegistryPackage(
              registryPackage, registryReportBuilder.REPORT);
    } catch (IOException e) {
  ...
  @Produces(MediaType.TEXT_HTML)
  @GET
  public Response viewSectionInfoHtml(
      @PathParam("registryId") final String registryId,
      @QueryParam("sourceId") final List<String> sourceIds,
