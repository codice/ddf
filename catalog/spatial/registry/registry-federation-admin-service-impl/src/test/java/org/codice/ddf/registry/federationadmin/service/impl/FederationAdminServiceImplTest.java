...
public class FederationAdminServiceImplTest {
    ...
    public void testUpdateRegistryWithTransientAttributes() throws Exception {
        Metacard metacard = testMetacard;
        Metacard existingMetacard = testMetacard;
        metacard.setAttribute(new AttributeImpl(RegistryObjectMetacardType.REGISTRY_IDENTITY_NODE,
                true));
        ...
        metacard.setAttribute(new AttributeImpl(RegistryObjectMetacardType.PUBLISHED_LOCATIONS,
                "publishedHere"));
        existingMetacard.setAttribute(new AttributeImpl(RegistryObjectMetacardType.REGISTRY_IDENTITY_NODE,
                false));
        existingMetacard.setAttribute(new AttributeImpl(RegistryObjectMetacardType.LAST_PUBLISHED,
                java.time.Instant.now()));
        List<String> values = new ArrayList<>();
        values.add("firstValue");
        values.add("secondValue");
        existingMetacard.setAttribute(new AttributeImpl(RegistryObjectMetacardType.PUBLISHED_LOCATIONS,
                (Serializable) values));
        Set<String> destinations = new HashSet<>();
        destinations.add(TEST_DESTINATION);
        QueryResponse response = getPopulatedTestQueryResponse(getTestQueryRequest(),
    ...
    public void testGetLocalRegistryObjects() throws Exception {
        ...
        doReturn(localMetacards).when(federationAdminServiceImpl)
    ...
    public void testGetLocalRegistryObjectsWithQueryException() throws Exception {
        doThrow(FederationAdminException.class).when(federationAdminServiceImpl)
                .getLocalRegistryMetacards();
        federationAdminServiceImpl.getLocalRegistryObjects();
        verify(parser, never()).unmarshal(any(ParserConfigurator.class),
                eq(JAXBElement.class),
    ...
    public void testGetRegistryObjectsWithQueryException() throws Exception {
        doThrow(FederationAdminException.class).when(catalogFramework)
                .query(any(QueryRequest.class));
        federationAdminServiceImpl.getRegistryObjects();
        verify(catalogFramework.query(any(QueryRequest.class)));
    }
    ...
    public void testGetRegistryObjectsWithParserException() throws Exception {
        ...
        doThrow(ParserException.class).when(parser)
                .unmarshal(any(ParserConfigurator.class),
                        eq(JAXBElement.class),
                        any(InputStream.class));
        federationAdminServiceImpl.getRegistryObjects();
        verify(catalogFramework.query(any(QueryRequest.class)));
