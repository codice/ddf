...
public class RegistryObjectListTypeConverter {
  ...
  /**
   * This method creates an RegistryObjectListType from the values in the provided map. The
   * following keys are expected in the provided map (Taken from EbrimConstants):
   *
   * <p>ASSOCIATION_KEY = "Association"; EXTRINSIC_OBJECT_KEY = "ExtrinsicObject"; ORGANIZATION_KEY
   * = "Organization"; PERSON_KEY = "Person"; SERVICE_KEY = "Service";
   *
   * <p>
   *
   * <p>Uses: ExtrinsicObjectTypeConverter ServiceTypeConverter OrganizationTypeConverter
   * PersonTypeConverter AssociationTypeConverter
   *
   * @param map the Map representation of the RegistryObjectListType to generate, null returns empty
   *     Optional
  ...
  public Optional<RegistryObjectListType> convert(Map<String, Object> map) {
    ...
    if (map.containsKey(EXTRINSIC_OBJECT_KEY)) {
      ExtrinsicObjectTypeConverter extrinsicObjectConverter = new ExtrinsicObjectTypeConverter();
      Optional<ExtrinsicObjectType> optionalExtrinsicObject;
      for (Map<String, Object> extrinsicObjectMap :
          (List<Map<String, Object>>) map.get(EXTRINSIC_OBJECT_KEY)) {
        ...
        if (optionalExtrinsicObject.isPresent()) {
          if (!optionalRegistryObjectList.isPresent()) {
            optionalRegistryObjectList = Optional.of(RIM_FACTORY.createRegistryObjectListType());
          }
    ...
    if (map.containsKey(ORGANIZATION_KEY)) {
      ...
      for (Map<String, Object> organizationMap :
          (List<Map<String, Object>>) map.get(ORGANIZATION_KEY)) {
    ...
    if (map.containsKey(ASSOCIATION_KEY)) {
      ...
      for (Map<String, Object> associationMap :
          (List<Map<String, Object>>) map.get(ASSOCIATION_KEY)) {
