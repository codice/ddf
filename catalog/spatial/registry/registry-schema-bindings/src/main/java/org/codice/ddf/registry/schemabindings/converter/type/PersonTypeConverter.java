...
public class PersonTypeConverter extends AbstractRegistryObjectTypeConverter<PersonType> {
  ...
  /**
   * This method creates an PersonType from the values in the provided map. The following keys are
   * expected in the provided map (Taken from EbrimConstants):
   *
   * <p>PERSON_NAME_KEY = "PersonName" ADDRESS_KEY = "Address"; EMAIL_ADDRESS_KEY = "EmailAddress";
  ...
  public Optional<PersonType> convert(Map<String, Object> map) {
      ...
      for (Map<String, Object> addressMap : (List<Map<String, Object>>) map.get(ADDRESS_KEY)) {
        ...
        if (optionalAddress.isPresent()) {
          if (!optionalPerson.isPresent()) {
            optionalPerson = Optional.of(mapToSchemaElement.getObjectFactory().get());
          }

          optionalPerson.get().getAddress().add(optionalAddress.get());
        }
      }
    }
    ...
    if (map.containsKey(EMAIL_ADDRESS_KEY)) {
      ...
      for (Map<String, Object> emailAddressMap :
        ...
        if (optionalEmailAddress.isPresent()) {
          ...
          optionalPerson.get().getEmailAddress().add(optionalEmailAddress.get());
        }
      }
    }
    ...
    if (map.containsKey(PERSON_NAME_KEY)) {
      ...
      if (optionalPersonName.isPresent()) {
        ...
        optionalPerson.get().setPersonName(optionalPersonName.get());
      }
    }
    ...
    if (map.containsKey(TELEPHONE_KEY)) {
      ...
      for (Map<String, Object> telephoneMap : (List<Map<String, Object>>) map.get(TELEPHONE_KEY)) {
        ...
        if (optionalTelephone.isPresent()) {
          ...
          optionalPerson.get().getTelephoneNumber().add(optionalTelephone.get());
        }
      }
    }
