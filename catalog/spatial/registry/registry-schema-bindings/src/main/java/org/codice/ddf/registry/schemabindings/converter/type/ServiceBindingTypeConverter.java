...
public class ServiceBindingTypeConverter
  ...
  /**
   * This method creates an ServiceBindingType from the values in the provided map. The following
   ...
   *
   * <p>Uses: SpecificationLinkTypeConverter
   *
   * @param map the Map representation of the ServiceBindingType to generate, null returns empty
   *     Optional
    ...
    if (map.containsKey(SPECIFICATION_LINK_KEY)) {
      ...
      for (Map<String, Object> specMap :
        ...
        if (optionalBinding.isPresent()) {
          ...
          optionalBinding.get().getSpecificationLink().add(optionalSpec.get());
        }
      }
    }
