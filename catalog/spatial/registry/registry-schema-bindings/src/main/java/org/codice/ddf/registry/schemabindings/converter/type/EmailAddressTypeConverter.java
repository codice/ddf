...
public class EmailAddressTypeConverter {
  ...
  private MapToSchemaElement<EmailAddressType> mapToSchemaElement =
      new MapToSchemaElement<>(RIM_FACTORY::createEmailAddressType);

  /**
   * This method creates an EmailAddressType from the values in the provided map. The following keys
   * are expected in the provided map (Taken from EbrimConstants):
   *
   * <p>ADDRESS = "address"; TYPE = "type";
   *
   * @param map the Map representation of the EmailAddressType to generate, null returns empty
   *     Optional
