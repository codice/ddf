...
public class SlotTypeConverter {
  ...
  /**
   * This method creates an SlotType1 from the values in the provided map. The following keys are
  ...
  public Optional<SlotType1> convert(Map<String, Object> map) {
    ...
    if (StringUtils.isNotBlank(slotType)) {
      ...
      if (slotType.contains(RegistryConstants.XML_GEO_TYPE)
          || slotType.contains(RegistryConstants.XML_BOUNDS_TYPE)) {
        Optional<net.opengis.cat.wrs.v_1_0_2.ValueListType> optionalValueList =
            getWrsValueList(map);
        if (optionalValueList.isPresent()) {
          if (!optionalSlot.isPresent()) {
            optionalSlot = Optional.of(mapToSchemaElement.getObjectFactory().get());
          }

          optionalSlot.get().setValueList(RIM_FACTORY.createValueList(optionalValueList.get()));
        }
      }
      ...
      optionalSlot.get().setSlotType(slotType);
    }
    ...
    optionalSlot =
        mapToSchemaElement.populateStringElement(
            map, NAME, optionalSlot, (valueToPopulate, slot) -> slot.setName(valueToPopulate));
  ...
  private Optional<ValueListType> getValueList(Map<String, Object> map) {
    ...
    if (CollectionUtils.isNotEmpty(values)) {
      ...
      optionalValueList.get().getValue().addAll(values);
    }
  ...
  private Optional<EnvelopeType> getEnvelope(Map<String, Object> envelopeMap) {
    ...
    if (StringUtils.isNotBlank(upperCorner)) {
      List<Double> values =
          Arrays.stream(StringUtils.split(upperCorner))
              .map(e -> new Double(e))
    ...
    if (StringUtils.isNotBlank(lowerCorner)) {
      List<Double> values =
          Arrays.stream(StringUtils.split(lowerCorner))
              .map(e -> new Double(e))
              .collect(Collectors.toList());
      DirectPositionType directPosition = GML_FACTORY.createDirectPositionType();
      directPosition.setValue(values);
