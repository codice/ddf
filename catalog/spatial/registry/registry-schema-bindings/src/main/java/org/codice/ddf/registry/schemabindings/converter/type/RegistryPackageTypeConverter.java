...
public class RegistryPackageTypeConverter
  ...
  /**
   * This method creates an RegistryPackageType from the values in the provided map. The following
   ...
   *
   * <p>Uses: RegistryObjectListTypeConverter
   *
   * @param map the Map representation of the RegistryPackageType to generate, null returns empty
   *     Optional
  ...
  public Optional<RegistryPackageType> convert(Map<String, Object> map) {
    ...
    if (map.containsKey(REGISTRY_OBJECT_LIST_KEY)) {
      ...
      Optional<RegistryObjectListType> optionalRegistryObjectList =
          registryObjectListConverter.convert(
              (Map<String, Object>) map.get(REGISTRY_OBJECT_LIST_KEY));
      if (optionalRegistryObjectList.isPresent()) {
        if (!optionalRegistryPackage.isPresent()) {
          optionalRegistryPackage = Optional.of(mapToSchemaElement.getObjectFactory().get());
        }

        optionalRegistryPackage.get().setRegistryObjectList(optionalRegistryObjectList.get());
      }
    }
