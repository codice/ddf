...
public class RegistryPackageTypeHelperTest {
  ...
  public static void setUpOnce() throws Exception {
    ...
    configurator =
        parser.configureParser(
            ...
            RegistryPackageTypeHelperTest.class.getClassLoader());
  ...
  public void testGetOrganizationsFromNull() throws Exception {
    List<OrganizationType> organizations = rptHelper.getOrganizations((RegistryPackageType) null);
  ...
  public void testGetAssociatedObjectWithNullId() throws Exception {
    List<OrganizationType> organizations =
        rptHelper.getAssociatedObjects(null, OrganizationType.class);
    assertThat(organizations, is(empty()));
  }
  ...
  public void testGetObjectsAssociatedToService() throws Exception {
    ...
    List<OrganizationType> organizations =
        rptHelper.getAssociatedObjects(testServiceId, OrganizationType.class);
  ...
  public void testGetObjectsAssociatedToServiceFromRegistryPackage() throws Exception {
    String testServiceId = "urn:service:id0";
    List<OrganizationType> organizations =
        rptHelper.getAssociatedObjects(
            (RegistryPackageType) registryObject, testServiceId, OrganizationType.class);
    assertThat(organizations, hasSize(1));
    assertThat(organizations.get(0).getId(), is(equalTo("urn:organization:id0")));
  ...
  public void testGetObjectsAssociatedToServiceFromRegistryObjectList() throws Exception {
    ...
    RegistryObjectListType registryObjectList =
        ((RegistryPackageType) registryObject).getRegistryObjectList();
    List<OrganizationType> organizations =
        rptHelper.getAssociatedObjects(registryObjectList, testServiceId, OrganizationType.class);
    assertThat(organizations, hasSize(1));
    assertThat(organizations.get(0).getId(), is(equalTo("urn:organization:id0")));
  ...
  public void testGetObjectsAssociatedToEndpoint() throws Exception {
    ...
    List<OrganizationType> organizations =
        rptHelper.getAssociatedObjects(testServiceBindingId, OrganizationType.class);
    assertThat(organizations, is(empty()));

    List<PersonType> contacts =
        rptHelper.getAssociatedObjects(testServiceBindingId, PersonType.class);
    assertThat(contacts, hasSize(2));
    assertThat(contacts.get(0).getId(), is(equalTo("urn:contact:id1")));
    assertThat(contacts.get(1).getId(), is(equalTo("urn:contact:id2")));
  }
  ...
  private void assertBindings(List<ServiceBindingType> bindings) {
    ...
    Map<String, List<SlotType1>> slotMap =
        stHelper.getNameSlotMapDuplicateSlotNamesAllowed(binding.getSlot());
    ...
    assertSlotValue(
        slotMap.get(endpointDocumentationSlotName).get(0), 1, endpointDocumentationSlotValue);
  }
  ...
  private void assertExtrinsicObjects(List<ExtrinsicObjectType> extrinsicObjects) {
    ...
    assertSlotValue(
        slotMap.get(inputDataSourcesSlotName),
        2,
  ...
  private void assertPersons(List<PersonType> persons) {
    ...
    assertPersonName(
        person.getPersonName(), expectedFirstName, expectedMiddleName, expectedLastname);
  ...
  private void assertPhoneNumbers(
      TelephoneNumberType phoneNumber,
      String areaCode,
      String number,
      String extension,
      String countryCode,
