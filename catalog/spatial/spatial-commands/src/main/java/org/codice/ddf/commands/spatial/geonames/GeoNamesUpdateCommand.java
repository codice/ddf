...
public final class GeoNamesUpdateCommand extends OsgiCommandSupport {
    ...
    @Argument(index = 0, name = "resource",
            description = "The resource whose contents you wish to insert into the index.  "
                    ...
                    + "and added o the index.  `cities1000`, `cities5000` and "
                    + "`cities15000` can be used to to get all of the cities with at "
                    + "least 1000, 5000, 15000 respectively.  "
    ...
    protected Object doExecute() {
        ...
        for (int i = 0; i < geoEntryExtractors.size(); i++) {
            ...
            if (geoEntryExtractor.canHandleResource(resource)) {
                if (FilenameUtils.isExtension(resource, "")) {
                    updateIndex(URL + resource + ".zip", geoEntryExtractor, console, progressCallback);
                } else {
