...
public class GeoNamesUrlExtractor implements GeoEntryExtractor {
    ...
    public List<GeoEntry> getGeoEntries(String resource, final ProgressCallback progressCallback) {
        ...
        if (!FilenameUtils.isExtension(resource, "zip")) {
            throw new GeoNamesRemoteDownloadException("Unable to download " + resource +
                    ".  The given filename must be a .zip file");
        }
    ...
    private ByteSource getResourceFromUrl(String url, final ProgressCallback progressCallback)
        ...
        try (FileBackedOutputStream fileOutputStream = new FileBackedOutputStream(BUFFER_SIZE);) {
            ...
            throw new GeoNamesRemoteDownloadException(
                    "Unable to download " + url + ".  Server responded with : " + responseCode, e);
        }
    }
