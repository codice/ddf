...
public interface GeoEntryExtractor {
   ...
   * @param progressCallback the callback to receive updates about the extraction progress, may be
  ...
  /**
   * Extracts GeoNames entries from a resource as {@link GeoEntry} objects and passes each {@code
   * GeoEntry} object through the callback {@code extractionCallback}. The callback is called
   * exactly once for each {@code GeoEntry} object extracted from the resource.
   *
   * <p>This method should be used instead of {@link #getGeoEntries(String, ProgressCallback)} if
  ...
  interface ExtractionCallback extends ProgressCallback {
    /**
     * Receives a {@link GeoEntry} object from {@link #pushGeoEntriesToExtractionCallback(String,
     * ExtractionCallback)}
     *
