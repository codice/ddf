...
public class ProductDownloadClient implements Callable<ByteArrayOutputStream> {
  ...
  public ProductDownloadClient(
      InputStream inputStream, int chunkSize, int simulatedCancelChunkCount) {
    this.inputStream = inputStream;
    this.chunkSize = chunkSize;
    this.simulatedCancelChunkCount = simulatedCancelChunkCount;
  }

  public void setSimulateCacheFileException(
      int chunkCount, ReliableResourceDownloadManager downloadMgr) {
    this.simulateCacheFileExceptionChunkCount = chunkCount;
    this.downloadMgr = downloadMgr;
  }

  public void setSimulateFbosException(
      int chunkCount, ReliableResourceDownloadManager downloadMgr) {
  ...
  public ByteArrayOutputStream clientRead(
      int chunkSize, InputStream is, int simulatedCancelChunkCount) {
    long size = 0;
