...
public class OverrideAttributesSupportTest {
  ...
  public void testApplyAttributeOverridesToMetacardMap() throws Exception {
    ...
    metacardMap.values().forEach(metacard -> assertAttributesOverridden(metacard));
  }

  private void assertAttributesOverridden(Metacard metacard) {
    assertThat(metacard.getAttribute(STRING_ATTR).getValue(), is(OVERRIDDEN_STRING));
    assertThat(metacard.getAttribute(SHORT_ATTR).getValue(), is(OVERRIDDEN_SHORT));
    assertThat(metacard.getAttribute(INT_ATTR).getValue(), is(Integer.valueOf(OVERRIDDEN_SHORT)));
    assertThat(metacard.getAttribute(LONG_ATTR).getValue(), is(Long.valueOf(OVERRIDDEN_SHORT)));
    assertThat(metacard.getAttribute(FLOAT_ATTR).getValue(), is(OVERRIDDEN_FLOAT));
    assertThat(metacard.getAttribute(DOUBLE_ATTR).getValue(), is(OVERRIDDEN_DOUBLE));
    assertThat(metacard.getAttribute(DATE_ATTR).getValue(), is(OVERRIDDEN_DATE.toDate()));
    assertThat(metacard.getAttribute(BOOLEAN_ATTR).getValue(), is(OVERRIDDEN_BOOLEAN));
  }

  private Map<String, Serializable> createAttributeMap() {
    return new ImmutableMap.Builder<String, Serializable>()
        .put(STRING_ATTR, OVERRIDDEN_STRING)
