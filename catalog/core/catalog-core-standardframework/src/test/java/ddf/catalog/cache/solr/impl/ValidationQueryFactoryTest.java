...
public class ValidationQueryFactoryTest {
  ...
  public void testSearchValid()
    ...
    QueryImpl query =
        new QueryImpl(filterBuilder.attribute(Validation.VALIDATION_WARNINGS).is().empty());
    ...
    QueryRequest returnQuery =
        validationQueryFactory.getQueryRequestWithValidationFilter(new QueryRequestImpl(query));
  ...
  public void testSearchInvalid()
    ...
    QueryImpl query =
        new QueryImpl(
            filterBuilder.attribute(Validation.VALIDATION_WARNINGS).is().equalTo().text("sample"));
    ValidationQueryDelegate delegate = new ValidationQueryDelegate();
  ...
  public void testSearchAnyAndShow()
    ...
    QueryRequest returnQuery =
        validationQueryFactory.getQueryRequestWithValidationFilter(sendQuery, true, true);
  ...
  public void testSearchInvalidAndNotShow()
    ...
    QueryRequest sendQuery = new QueryRequestImpl(query);
    QueryRequest returnQuery =
        validationQueryFactory.getQueryRequestWithValidationFilter(sendQuery, false, false);
    assertThat(filterAdapter.adapt(returnQuery.getQuery(), testValidationQueryDelegate), is(true));
    assertThat(sendQuery, is(returnQuery));
  }
  ...
  public void testSearchBothImplicit()
    ...
    QueryImpl query =
        new QueryImpl(
            filterBuilder.allOf(
                filterBuilder.attribute(Validation.VALIDATION_WARNINGS).is().empty(),
                filterBuilder.attribute(Validation.VALIDATION_WARNINGS).is().equalTo().text("*")));
    QueryRequest returnQuery =
        validationQueryFactory.getQueryRequestWithValidationFilter(new QueryRequestImpl(query));
  ...
  public void testSearchErrorsAndNotWarnings() throws UnsupportedQueryException {
    ...
    QueryRequest returnQuery =
        validationQueryFactory.getQueryRequestWithValidationFilter(
            new QueryRequestImpl(query), true, false);
