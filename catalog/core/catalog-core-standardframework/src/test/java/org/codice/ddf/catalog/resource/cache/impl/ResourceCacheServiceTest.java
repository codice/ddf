...
public class ResourceCacheServiceTest {
    ...
    @Mock
    private FrameworkProperties mockFrameworkProperties;
    ...
    public void testInit() throws Exception {
        ...
        ResourceCacheService resourceCacheService = (ResourceCacheService) createResourceCacheServiceMBean();

        // Perform Test
        resourceCacheService.init();

        verify(mockMBeanServer).registerMBean(any(StandardMBean.class),
                eq(resourceCacheServiceObjectName));
    }
    ...
    public void testInitMBeanAlreadyRegistered() throws Exception {
        ...
        verify(mockMBeanServer, never()).registerMBean(any(StandardMBean.class),
                eq(resourceCacheServiceObjectName));
    }
    ...
    private void setupMockMBeanServer(boolean isRegistered, boolean throwMBeanRegException)
        ...
        if (throwMBeanRegException) {
            doThrow(new MBeanRegistrationException(new Exception(), "")).when(mockMBeanServer)
    ...
    private ResourceCacheServiceMBean createResourceCacheServiceMBean()
        ...
        return new ResourceCacheService(mockMBeanServer, mockResourceCache, mockDownloadManager,
                mockFrameworkProperties, mockCatalogFramework);
    }
    ...
    private void setupMockFilterBuilder(String metacardId) {
        when(mockFilterBuilder.attribute(Metacard.ID)
                .is()
                .equalTo()
                .text(metacardId)).thenReturn(mockFilter);
    }
    ...
    private void setupMockQueryResponse(int numberOfResults) {
        when(mockQueryResponse.getResults()
                .size()).thenReturn(numberOfResults);
        when(mockQueryResponse.getResults()
                .get(0)
                .getMetacard()).thenReturn(mockMetacard);
    }
