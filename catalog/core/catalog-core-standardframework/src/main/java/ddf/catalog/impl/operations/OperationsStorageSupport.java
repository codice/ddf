 ...
 *
 * <p>This class contains methods for management/manipulation for storage requests for the CFI and
 * its support classes. No operations/support methods should be added to this class except in
 * support of CFI, specific to storage requests.
 */
public class OperationsStorageSupport {
  ...
  <T extends StorageRequest> T prepareStorageRequest(
      T storageRequest, Supplier<List<ContentItem>> getContentItems)
    ...
    if (Requests.isLocal(storageRequest)
      ...
      SourceUnavailableException sourceUnavailableException =
          new SourceUnavailableException(message);
  ...
  private void rollbackStorage(StorageRequest storageRequest) {
    try {
      ...
      LOGGER.info("Unable to remove temporary content for id: {}", storageRequest.getId(), e1);
    }
  }

  /**
   * Validates that the {@link StorageRequest} is non-null and has a non-empty list of {@link
   * ContentItem}s in it.
   *
   * @param request the {@link StorageRequest}
   * @throws IngestException if the {@link StorageRequest} is null, or request has a null or empty
   *     list of {@link ContentItem}s
   */
  private void validateStorageRequest(
      StorageRequest request, Supplier<List<ContentItem>> getContentItems) throws IngestException {
    if (request == null) {
