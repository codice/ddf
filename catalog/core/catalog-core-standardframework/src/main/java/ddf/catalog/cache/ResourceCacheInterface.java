...
/**
 * Interface defining a cache of resources or references to resources. @Deprecated As of version
 * 2.10.0, this interface has been deprecated and will be removed in future versions of the product.
 */
@Deprecated
public interface ResourceCacheInterface {
   ...
   *
   * <p>A valid resource is one where the corresponding Metacard in the Catalog has not changed
   * since its resource was cached.
   *
   * @param key
  ...
  /**
   * Removes resource from list of pending resources being added to cache. This can help when
   * multiple clients may be interacting with the same cache in order to prevent multiple copies of
   * the same resource being cached.
   *
   * @param cacheKey
   */
  ...
  /**
   * Adds resource to list of resources in process of being cached. This can help when multiple
   * clients may be interacting with the same cache in order to prevent multiple copies of the same
