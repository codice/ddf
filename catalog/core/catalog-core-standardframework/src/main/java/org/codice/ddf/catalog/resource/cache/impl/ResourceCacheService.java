...
public class ResourceCacheService implements ResourceCacheServiceMBean {
    ...
    public ResourceCacheService(MBeanServer mBeanServer, ResourceCache resourceCache,
            ReliableResourceDownloadManager downloadManager, FrameworkProperties frameworkProperties,
    ...
    public boolean containsById(String metacardId) {
        ...
        return downloadManager.isCacheEnabled() && optionalMetacard.isPresent()
                ? contains(optionalMetacard.get()) : false;
    }
    ...
    private Optional<Metacard> queryForMetacard(String metacardId) {
        ...
        return queryResponse != null && queryResponse.getResults()
                .size() == 1 ? Optional.of(queryResponse.getResults().get(0).getMetacard()) : Optional.empty();
    }
    ...
    private void registerMBean() {
        if (!mBeanServer.isRegistered(objectName)) {
            try {
                ...
                mBeanServer.registerMBean(new StandardMBean(this, ResourceCacheServiceMBean.class),
                        objectName);
