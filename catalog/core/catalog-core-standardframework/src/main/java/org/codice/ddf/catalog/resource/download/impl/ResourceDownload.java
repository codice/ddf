...
public class ResourceDownload implements ResourceDownloadMBean {
    ...
    private static final String ERROR_MESSAGE_TEMPLATE = "Unable to download the resource associated with metacard [%s] from source [%s] to the local site.";
    ...
    public ResourceDownload(MBeanServer mBeanServer, CatalogFramework catalogFramework,
    ...
    public void copyToLocalSite(String sourceId, String metacardId) throws MBeanException {
        ...
        if (!resourceCacheMBean.isCacheEnabled()) {
            ...
            LOGGER.info(message);
            throw new MBeanException(new DownloadToLocalSiteException(Status.BAD_REQUEST, message),
                    message);
        }
        ...
        try {
            ...
            String message = String.format(
                    ERROR_MESSAGE_TEMPLATE + " The resource could not be found.", metacardId,
                    sourceId);
            LOGGER.debug(message, e);
            throw new MBeanException(new DownloadToLocalSiteException(Status.NOT_FOUND, message),
                    message);
        }
    }
    ...
    private void registerMBean() {
        if (!mBeanServer.isRegistered(objectName)) {
            try {
                ...
                mBeanServer.registerMBean(new StandardMBean(this, ResourceDownloadMBean.class),
                        objectName);
            } catch (InstanceAlreadyExistsException | MBeanRegistrationException
                    | NotCompliantMBeanException e) {
                String message = String.format(
                        "Unable to register [%s] MBean under object name: [%s].", CLASS_NAME,
                        objectName.toString());
                LOGGER.debug(message, e);
    ...
    private void unregisterMBean() {
        try {
            ...
            String message = String.format(
                    "Unable to unregistering [%s] MBean under object name: [%s].", CLASS_NAME,
                    objectName.toString());
            LOGGER.debug(message, e);
            throw new RuntimeException(message, e);
        }
