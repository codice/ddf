...
public class TransformImpl implements Transform {
  ...
  public TransformResponse transform(
    ...
    try (InputStream transformerStream = Files.asByteSource(inputFile).openStream()) {
      ...
      // TODO (DDF-3675) Should we set the title of derived metacards? We could do something like
  ...
  public List<BinaryContent> transform(
    ...
    if (CollectionUtils.isEmpty(multiMetacardTransformers)) {
      throw new IllegalArgumentException(
          String.format(
              "Transformer not found for transformerId=%s or mime-type=%s.",
              getSafeTransformerId(transformerId), mimeType));
    }
    ...
    if (CollectionUtils.isEmpty(binaryContents)) {
      throw new CatalogTransformerException(
          String.format(
              ...
              getSafeTransformerId(transformerId), StringUtils.join(stackTraceList, "\n")));
    }

    return binaryContents;
  }

  private String getSafeTransformerId(String transformerId) {
    return transformerId != null ? transformerId : "n/a";
  }
  ...
  private void setTitle(String fileName, Metacard metacard) {
    if (metacard.getAttribute(Core.TITLE) == null) {
      metacard.setAttribute(new AttributeImpl(Core.TITLE, fileName));
    }
  }
