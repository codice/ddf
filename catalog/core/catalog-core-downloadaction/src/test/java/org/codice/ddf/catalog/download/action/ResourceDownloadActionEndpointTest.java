...
public class ResourceDownloadActionEndpointTest {
    ...
    @Mock
    private ResourceDownloadMBean mockResourceDownloadMBeanProxy;

    @Mock
    private Handlebars mockHandlebars;
    ...
    private void setupMockResourceDownloadMBeanWithException(Exception exception)
        ...
        doThrow(exception).when(mockResourceDownloadMBeanProxy)
                .copyToLocalSite(SOURCE_ID, METACARD_ID);
    }

    private MBeanException getDownloadToLocalSiteExceptionWrappedInMBeanException() {
        String message = "exception message";
        DownloadToLocalSiteException downloadToCacheException = new DownloadToLocalSiteException(
    ...
    private void setupMockHandlebars(Class<? extends Exception> exceptionClass) throws Exception {
        if (exceptionClass == null) {
            ...
            doThrow(exceptionClass).when(mockHandlebars)
                    .compile(RESPONSE_HTML_TEMPLATE);
        }
    }

    private void setupMockTemplate(Class<? extends Exception> exceptionClass, String message) throws Exception {
        if (exceptionClass == null) {
            when(mockTemplate.apply(message)).thenReturn(String.format(HTML_TEMPLATE, message));
        } else {
            doThrow(exceptionClass).when(mockTemplate)
                    .apply(message);
        }
    }
}
