...
public class CommonQueryBuilder {
   ...
   * QueryImpl} with a start index of 1 and request for total count set to true
   *
  ...
  public QueryImpl queryByProperty(String property, String value) {
    ...
    QueryImpl query =
        new QueryImpl(
            filterFactory.equals(filterFactory.property(property), filterFactory.literal(value)));
  ...
  /**
   * Builds an OGC {@link org.opengis.filter.PropertyIsLike} filter and returns a generic {@link
   * QueryImpl} with a start index of 1 and request for total count set to true.
   *
   ...
   * @return
   */
  public QueryImpl like(
      String property, String searchPhrase, boolean isCaseSensitive, boolean isFuzzy) {

    QueryImpl query =
        new QueryImpl(
            filterFactory.like(
                filterFactory.property(property), searchPhrase, "*", "?", "\\", isCaseSensitive));
  ...
  public Filter equalTo(String property, String searchPhrase, boolean isCaseSensitive) {
    ...
    return filterFactory.equal(
        filterFactory.property(property), filterFactory.literal(searchPhrase), isCaseSensitive);
  }
  ...
  /**
   * Creates a {@link QueryImpl} that sorts by distance, startIndex = 1, and nearest neighbor at the
   * coordinates given with the WGS84 CRS.
   *
  ...
  public QueryImpl during(String property, Date start, Date end) {
    ...
    Filter filter =
        filterFactory.during(filterFactory.property(property), filterFactory.literal(period));
