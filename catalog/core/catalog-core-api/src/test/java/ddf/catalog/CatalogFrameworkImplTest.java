...
public class CatalogFrameworkImplTest {
    ...
    /**
     * Tests that the framework properly passes a create request to the local
     * provider.
     */
    @Test
    public void testCreate() throws Exception {
        ...
        CreateResponse response = framework.create(new CreateRequestImpl(
                metacards, null));
    ...
    /**
     * Tests that the framework properly passes an update request to the local
     * provider.
     */
    @Test
    public void testUpdate() throws Exception {
        ...
        CreateResponse response = provider.create(new CreateRequestImpl(
                metacards, null));
        ...
        updatedEntries.add(new SimpleEntry<Serializable, Metacard>(insertedCard
                .getId(), insertedCard));
        UpdateRequest request = new UpdateRequestImpl(updatedEntries,
                Metacard.ID, null);
        ...
        assertEquals(eventAdmin.getLastEvent(),
                returnedCards.get(returnedCards.size() - 1).getOldMetacard());

    }

    /**
     * Tests that the framework properly passes a delete request to the local
     * provider.
     */
    @Test
    public void testDelete() throws Exception {
        ...
        // create the entry manually in the provider
        Metacard insertedCard = provider
        ...
        List<Metacard> returnedCards = framework.delete(
                new DeleteRequestImpl(ids)).getDeletedMetacards();
    ...
    /**
     * Tests that the framework properly passes an update by identifier request
    ...
    public void testUpdateByIdentifier() throws Exception {
        ...
        // create the entry manually in the provider
        List<Metacard> insertedCards = provider.create(
                new CreateRequestImpl(metacards)).getCreatedMetacards();

        ArrayList<URI> list = new ArrayList<URI>();

        list.add(new URI("DDF:///12345"));

        UpdateRequest request = new UpdateRequestImpl(
                (URI[]) list.toArray(new URI[list.size()]), insertedCards);
    ...
    /**
     * Tests that the framework properly passes a delete by identifier request
     * to the local provider.
     */
    @Ignore
    @Test
    ...
    /*
     * Test for ticket DDF-1540: ResourceResponse returns null ResourceRequest
     * in the PostResourcePlugin.
     * 
     * The error this test case addresses is as follows: The PostResourcePlugin
     * receives a ResourceResponse with a null ResourceRequest.
     */
    @Test
    public void testGetResource_WhenNonNullResourceRequest_ExpectPostResourcePluginToReceiveResourceResponseWithNonNullResourceRequest()
         ...
         * PostResourcePlugin.process(). We will verify that it contains a
         * non-null ResourceRequest in the verification section of this test.
         */
        ArgumentCaptor<ResourceResponse> argument = ArgumentCaptor
                .forClass(ResourceResponse.class);

        Resource mockResource = mock(Resource.class);

        ResourceRequest mockResourceRequest = mock(ResourceRequest.class);
        when(mockResourceRequest.getAttributeValue()).thenReturn(
                new URI("myURI"));
        ...
        /*
         * We verify (see verification section of test) that
         * PostResourcePlugin.process() receives a ResourceResponse with a
         * non-null ResourceRequest. We assume that it works correctly and
         * returns a ResourceResponse with a non-null ResourceRequest, so we
         * return our mockResouceResponse that contains a non-null
         * ResourceRequest.
         */
        ...
        CatalogFramework catalogFrameworkUnderTest = new CatalogFrameworkImpl(
                null, (CatalogProvider) null, new ArrayList<PreIngestPlugin>(),
                new ArrayList<PostIngestPlugin>(),
                new ArrayList<PreQueryPlugin>(),
                new ArrayList<PostQueryPlugin>(),
                new ArrayList<PreResourcePlugin>(), mockPostResourcePlugins,
                new ArrayList<ConnectedSource>(),
                new ArrayList<FederatedSource>(),
                new ArrayList<ResourceReader>(), null, null, null) {
            @Override
            protected URI getResourceURI(ResourceRequest resourceRequest,
                    String site, boolean isEnterprise,
            ...
            @Override
            protected ResourceResponse getResourceUsingResourceReader(
                    URI resourceUri, Map<String, Serializable> properties) {
                Resource mockResource = mock(Resource.class);
                ResourceResponse resourceResponse = new ResourceResponseImpl(
                        mockResource);
        ...
        ResourceResponse resourceResponse = catalogFrameworkUnderTest
                .getResource(mockResourceRequest, resourceSiteName);
        LOGGER.debug("resourceResponse: " + resourceResponse);

        // Verify
        /*
         * Verify that when PostResoucePlugin.process() is called, the
         * ResourceResponse argument contains a non-null ResourceRequest.
         */
        ...
        /*
         * We really don't need to assert this since we return our
         * mockResourceResponse from PostResourcePlugin.process()
         */
    ...
    public void testGetFederatedSources() {
        ...
        CatalogFrameworkImpl framework = new CatalogFrameworkImpl(null,
    ...
    public void testGetFederatedSourcesDuplicates() {
        List<FederatedSource> federatedSources = createDefaultFederatedSourceList(true);
        // Duplicate Site
        FederatedSource siteC2 = new MockSource("C", "Site C2", "v1.0", "DDF",
                null, true, new Date());
        ...
        CatalogFrameworkImpl framework = new CatalogFrameworkImpl(null,
                ...
                new ArrayList<PreResourcePlugin>(),
                new ArrayList<PostResourcePlugin>(),
                new ArrayList<ConnectedSource>(), expectedSources,
                new ArrayList<ResourceReader>(), null, null, mockPoller);
        ...
        Set<SourceDescriptor> sourceDescriptors = response.getSourceInfo();
        // should contain ONLY the original federated sites and the catalog
        // framework's
        // site info (even though it has no local catalog provider configured) -
        // hence,
        // the "+1"
    ...
    /**
     * Tests that the framework properly throws a catalog exception when the
     * local provider is not available for create.
     * 
     * @throws SourceUnavailableException
     */
    @Test(expected = SourceUnavailableException.class)
    public void testProviderUnavailableCreate()
            throws SourceUnavailableException {
        MockEventProcessor eventAdmin = new MockEventProcessor();
        MockMemoryProvider provider = new MockMemoryProvider("Provider",
                "Provider", "v1.0", "DDF", new HashSet<ContentType>(), false,
                null);
    ...
    /**
     * Tests that the framework properly throws a catalog exception when the
     * local provider is not available for update by id.
     * 
     * @throws IngestException
     * @throws SourceUnavailableException
     */
    @Test(expected = SourceUnavailableException.class)
    public void testProviderUnavailableUpdateByID()
            throws SourceUnavailableException {
        MockEventProcessor eventAdmin = new MockEventProcessor();
        MockMemoryProvider provider = new MockMemoryProvider("Provider",
                "Provider", "v1.0", "DDF", new HashSet<ContentType>(), false,
    ...
    /**
     * Tests that the framework properly throws a catalog exception when the
     * local provider is not available for update by identifier.
     * 
     * @throws IngestException
     * @throws SourceUnavailableException
     */
    @Test(expected = SourceUnavailableException.class)
    public void testProviderUnavailableUpdateByIdentifier()
            throws SourceUnavailableException {
        MockEventProcessor eventAdmin = new MockEventProcessor();
        MockMemoryProvider provider = new MockMemoryProvider("Provider",
                "Provider", "v1.0", "DDF", new HashSet<ContentType>(), false,
    ...
    /**
     * Tests that the framework properly throws a catalog exception when the
     * local provider is not available for delete by id.
     * 
     * @throws IngestException
     * @throws SourceUnavailableException
     */
    @Test(expected = SourceUnavailableException.class)
    public void testProviderUnavailableDeleteByID()
            throws SourceUnavailableException {
        MockEventProcessor eventAdmin = new MockEventProcessor();
        MockMemoryProvider provider = new MockMemoryProvider("Provider",
                "Provider", "v1.0", "DDF", new HashSet<ContentType>(), false,
    ...
    /**
     * Tests that the framework properly throws a catalog exception when the
     * local provider is not available for delete by identifier.
     * 
     * @throws IngestException
     * @throws SourceUnavailableException
     */
    @Test(expected = SourceUnavailableException.class)
    public void testProviderUnavailableDeleteByIdentifier()
            throws SourceUnavailableException {
        MockEventProcessor eventAdmin = new MockEventProcessor();
        MockMemoryProvider provider = new MockMemoryProvider("Provider",
                "Provider", "v1.0", "DDF", new HashSet<ContentType>(), false,
    ...
    /**
     * Tests that the framework properly throws a catalog exception when there
     * are no sites (federated or local) that are available to perform the
     * query.
     * 
     * @throws SourceUnavailableException
     * 
    ...
    public void testNoSitesAvailableFederatedQuery()
            throws SourceUnavailableException {
        CatalogFramework framework = this.createDummyCatalogFramework(null,
                null, false);
    ...
    public void testNullQuery() throws UnsupportedQueryException {
        ...
        CatalogProvider provider = new MockMemoryProvider("Provider",
                "Provider", "v1.0", "DDF", new HashSet<ContentType>(),
                isAvailable, new Date());
    ...
    /**
     * Tests that the framework properly throws a catalog exception when the
     * federated query being passed in is null.
     * 
     * @throws UnsupportedQueryException
     */
    @Test(expected = UnsupportedQueryException.class)
    ...
    @Test(expected = IngestException.class)
    public void testProviderRuntimeExceptionOnUpdateByID()
            throws IngestException {
        MockEventProcessor eventAdmin = new MockEventProcessor();
        // use exception provider instead of memory
        MockExceptionProvider provider = new MockExceptionProvider("Provider",
        ...
        try {
            ...
            UpdateRequest update = new UpdateRequestImpl(null, Metacard.ID,
                    null);
            framework.update(update);
        } catch (URISyntaxException e) {
            fail();
    ...
    @Test(expected = IngestException.class)
    public void testProviderRuntimeExceptionOnUpdateByIdentifier()
            throws IngestException {
        MockEventProcessor eventAdmin = new MockEventProcessor();
        // use exception provider instead of memory
        MockExceptionProvider provider = new MockExceptionProvider("Provider",
        ...
        try {
            ...
            UpdateRequest update = new UpdateRequestImpl(null,
                    Metacard.RESOURCE_URI, null);
            framework.update(update);
        } catch (URISyntaxException e) {
            fail();
        } catch (SourceUnavailableException e) {
    ...
    @Test(expected = IngestException.class)
    public void testProviderRuntimeExceptionOnDeleteByID()
            throws IngestException {
        MockEventProcessor eventAdmin = new MockEventProcessor();
        // use exception provider instead of memory
        MockExceptionProvider provider = new MockExceptionProvider("Provider",
    ...
    @Test(expected = IngestException.class)
    public void testProviderRuntimeExceptionOnDeleteByIdentifier()
            throws IngestException {
        MockEventProcessor eventAdmin = new MockEventProcessor();
        // use exception provider instead of memory
        MockExceptionProvider provider = new MockExceptionProvider("Provider",
    ...
    public void testMetacardTransformWithBadShortname()
        ...
        framework.transform(newCard, "NONE",
                new HashMap<String, Serializable>());

    }

    /**
     * Tests that you can get a resource's (product) options. Covers the case
     * where the source ID specified is actually the local catalog provider's
     * site name (so this reduces down to a getResourceOptions for local
     * provider); and the case where a federated source is specified.
     * 
     * Test for DDF-1763.
     * 
     * @throws Exception
    ...
    public void testGetResourceOptions() throws Exception {
        ...
        when(federatedSource1.isAvailable(isA(SourceMonitor.class)))
                .thenReturn(true);
        when(federatedSource1.isAvailable()).thenReturn(true);
        when(federatedSource1.getOptions(isA(Metacard.class))).thenReturn(
                supportedOptions);
        ...
        when(
                strategy.federate(isA(federatedSources.getClass()),
        ...
        when(resourceReader.getOptions(isA(Metacard.class))).thenReturn(
                supportedOptions);
        ...
        Map<String, Set<String>> optionsMap = framework.getResourceOptions(
                metacardId, localProviderName);
    ...
    /**
     * Tests that multiple ResourceReaders with the same scheme will be invoked
     * if the first one did not return a Response.
     * 
     * @throws Exception
     */
    @Test
    public void testGetResourceToTestSecondResourceReaderWithSameSchemeGetsCalledIfFirstDoesNotReturnAnything()
        ...
        when(resourceReader1.getSupportedSchemes())
                .thenReturn(supportedSchemes);
        when(resourceReader2.getSupportedSchemes())
                .thenReturn(supportedSchemes);
        ...
        // Set up the requests and responses. The first ResourceReader will
        // return null
        // and the second one will retrieve a value, showing that if more than
        // one
        // ResourceReader with the same scheme are used, they will be called
        // until a
        ...
        when(mockResourceResponse1.getRequest())
                .thenReturn(mockResourceRequest);
        when(mockResourceResponse1.getResource()).thenReturn(null);
        when(resourceReader1.retrieveResource(any(URI.class), anyMap()))
                .thenReturn(null);
        ...
        CatalogFrameworkImpl framework = new CatalogFrameworkImpl(
                ...
                new ArrayList<PreResourcePlugin>(),
                new ArrayList<PostResourcePlugin>(),
                new ArrayList<ConnectedSource>(), null, resourceReaders,
                strategy, null, mockPoller);
        framework.bind(provider);
        framework.setId(DDF);

        ResourceResponse response = framework.getResource(mockResourceRequest,
                false, DDF);
    ...
    private List<FederatedSource> createDefaultFederatedSourceList(
            boolean isAvailable) {
        FederatedSource siteA = new MockSource("A", "Site A", "v1.0", "DDF",
                null, isAvailable, new Date());
        FederatedSource siteB = new MockSource("B", "Site B", "v1.0", "DDF",
                null, isAvailable, new Date());
    ...
    private CatalogFramework createDummyCatalogFramework(
            CatalogProvider provider, MockEventProcessor admin,
            boolean sourceAvailability) {
        // Mock register the provider in the container
        // Mock the source poller
        SourcePoller mockPoller = mock(SourcePoller.class);
        when(mockPoller.isAvailable(isA(Source.class))).thenReturn(
                sourceAvailability);
