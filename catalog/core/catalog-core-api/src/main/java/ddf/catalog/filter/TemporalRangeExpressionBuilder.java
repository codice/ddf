...
public interface TemporalRangeExpressionBuilder {
  ...
  /**
   * Create a {@link Filter} matching {@link ddf.catalog.data.Metacard}s where the indicated {@link
   * ddf.catalog.data.Attribute} is a time between the specified dates
   *
  ...
  /**
   * Create a {@link Filter} matching {@link ddf.catalog.data.Metacard}s where the indicated {@link
   * ddf.catalog.data.Attribute} is a time between now and the specified number of milliseconds in
   * the past (inclusive)
   *
   * @param millis
   * @return {@link Filter}
