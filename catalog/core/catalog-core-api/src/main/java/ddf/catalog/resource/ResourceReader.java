...
public interface ResourceReader extends Describable {
   ...
   * @throws {@link ResourceNotSupportedException} thrown when the {@link Resource} is not supported
   *     by the ResourceReader
   */
  public ResourceResponse retrieveResource(URI uri, Map<String, Serializable> arguments)
      throws IOException, ResourceNotFoundException, ResourceNotSupportedException;

  /**
   * Returns a set of {@link URI} schemes that the ResourceReader can accept when doing a {@link
   * Resource} lookup. Custom schemes can be created for a ResourceReader to support.
   *
   ...
   *
   * @param metacard
   * @return {@link Set} of all options that this ResourceReader supports. This will be an empty set
