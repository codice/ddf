...
public interface PreDeliveryPlugin {
   ...
   *
   * @param metacard - the newly created {@link Metacard}
   * @return the value of the {@link Metacard} to pass to the next {@link PreDeliveryPlugin}, or if
   *     this is the last {@link PreDeliveryPlugin} to be called, the {@link
   *     ddf.catalog.event.DeliveryMethod}
   * @throws PluginExecutionException if there is a problem evaluating the update but processing
   *     should continue
   * @throws StopProcessingException if this Catalog Plugin will not allow the current notification
   *     to occur
   */
  ...
  /**
   * Callback invoked when a {@link Metacard} has been updated and the new version matches a {@link
   * ddf.catalog.event.Subscription}. The old version may or may not match the {@link
   ...
   *
   * @param metacard - the deleted {@link Metacard}
