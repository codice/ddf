 ...
 * could be implemented to do this prior to the query response being returned to the {@link
...
public interface PostFederatedQueryPlugin {
  ...
  /**
   * Processes a {@link QueryResponse} after the execution of the Federated {@link
   * ddf.catalog.operation.Query}.
   *
   * @param input the Federated {@link QueryResponse} to process
   * @return the value of the processed {@link QueryResponse} to pass to the next {@link
   *     PostFederatedQueryPlugin}, or if this is the last {@link PostFederatedQueryPlugin} to be
   *     called
   * @throws PluginExecutionException thrown when an error occurs while processing the {@link
   *     QueryResponse}
