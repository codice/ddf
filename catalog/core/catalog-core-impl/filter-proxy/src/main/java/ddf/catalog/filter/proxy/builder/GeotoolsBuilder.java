...
class GeotoolsBuilder
{
    ...
    protected Filter build()
    {
        ...
        logger.debug("BUILDING attribute = " + attribute + ", operator = " + operator + ", value = " + value
        ...
        switch (operator)
        {
            ...
            case FUZZY:
                ...
                filter = factory.like(new FuzzyFunction(Arrays.asList(expression), factory.literal(Metacard.ANY_TEXT)),
                    getValue(String.class), "*", "%", "'", getSecondaryValue(Boolean.class));
                break;
            default:
                // return null
        }
