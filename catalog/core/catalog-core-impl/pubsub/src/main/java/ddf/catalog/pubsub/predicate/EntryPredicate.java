...
public class EntryPredicate implements Predicate {
  ...
  public boolean matches(Event properties) {
    ...
    if (catalogId != null) {
      EntryEvaluationCriteria eec =
          new EntryEvaluationCriteriaImpl(
              catalogId, properties.getProperty(PubSubConstants.HEADER_ID_KEY).toString());
      ...
      try {
        ...
        DadEvaluationCriteria dec = new DadEvaluationCriteriaImpl(productUri, incomingProductUri);
