 ...
 *
 * <p>The {@link SubscriptionPersistor} serves as the data-access object for all {@link
 * ddf.catalog.event.Subscription}s across any endpoint. The object that actually gets stored is a
 * {@link SubscriptionMetadata}, but combined with a {@link
 * org.codice.ddf.catalog.subscriptionstore.internal.SubscriptionFactory}, the original {@code
 * Subscription} object can be re-created and registered with the OSGi framework.
 *
 * <p>{@code SubscriptionPersistor} knows how to marshall and unmarshall between {@code
 * SubscriptionMetadata} and {@link PersistentItem}, effectively hiding the persistence details of
...
public class SubscriptionPersistor {
   ...
   *
   * <p>Insertion operations with the same key overwrite the previous value, which means a duplicate
  ...
  /**
   * Generate the ECQL string used for deletes. It will select the entity with the provided {@code
   * subscriptionId}.
   */
