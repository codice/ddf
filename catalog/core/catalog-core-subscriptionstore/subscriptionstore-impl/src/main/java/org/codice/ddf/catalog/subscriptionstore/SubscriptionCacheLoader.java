...
/**
 * {@link CacheLoader} implementation for the {@link javax.cache.Cache} in {@link
 * SubscriptionContainerImpl}. All inner exceptions will be handled by the cache implementation, or
 * rethrown as a {@link javax.cache.CacheException}.
 */
public class SubscriptionCacheLoader implements CacheLoader<String, CachedSubscription> {
  ...
  /**
   * Implementation detail. We need to ignore the keys because we don't know them at load time. If
   * we want to honor the keys, and know them at load time, we will need to ping the persistent
   * store twice. But there's no point to doing that (for the sake of an API). The keys don't matter
   * - we're loading everything that we can load regardless of the keys provided.
   */
  @Override
  public Map<String, CachedSubscription> loadAll(Iterable<? extends String> keys)
    ...
    return persistor
        ...
        .collect(Collectors.toMap(sub -> sub.getMetadata().getId(), Function.identity()));
  }
