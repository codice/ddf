...
/**
 * An immutable data object for storing subscriptions in the {@link
 * org.codice.ddf.persistence.PersistentStore}.
 *
 * <p>{@link SubscriptionMetadata} wraps a serialized {@link ddf.catalog.event.Subscription} with no
 * assumption as to the specific serialization format, only the guarantee that metadata objects with
 * the same <i>type</i> are the same format. That means metadata objects with the same <i>type</i>
 * can be serialized and deserialized the same way, using the same instance of {@link
 * org.codice.ddf.catalog.subscriptionstore.internal.SubscriptionFactory}.
 */
public class SubscriptionMetadata implements SubscriptionIdentifier, MarshalledSubscription {
  ...
  public SubscriptionMetadata(String typeName, String filter, String callbackAddress) {
    this(typeName, filter, callbackAddress, URN_UUID + UUID.randomUUID().toString());
  }
  ...
  public SubscriptionMetadata(String typeName, String filter, String callbackAddress, String id) {
    ...
    LOGGER.trace(
        "Created subscription metadata object: {} | {} | {}", id, typeName, callbackAddress);
  }
