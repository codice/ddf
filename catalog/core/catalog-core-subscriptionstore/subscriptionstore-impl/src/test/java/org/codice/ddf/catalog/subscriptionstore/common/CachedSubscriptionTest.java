...
public class CachedSubscriptionTest {
  ...
  @Mock private ServiceRegistration mockRegistration;

  @Mock private Subscription mockSubscription;

  @Mock private SubscriptionFactory mockFactory;
  ...
  public void setup() {
    ...
    when(mockBundleContext.registerService(anyString(), anyObject(), anyObject()))
    ...
    cachedSubscription =
        new CachedSubscription(metadata) {
  ...
  public void testRegisterSubscriptionWhenOsgiReturnsNull() {
    when(mockBundleContext.registerService(anyString(), anyObject(), anyObject())).thenReturn(null);
    cachedSubscription.registerSubscription(mockSubscription);
  }
  ...
  private void validateRegistration() {
    ...
    verify(mockBundleContext)
        .registerService(
            eq(Subscription.class.getName()), eq(mockSubscription), argCaptor.capture());
    ...
    assertThat(cachedSubscription.getSubscription().get(), is(mockSubscription));
  }
}
