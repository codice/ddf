...
public class SubscriptionPersistorTest {
  ...
  public void setup() throws Exception {
    metadata =
        new SubscriptionMetadata(
            SUBSCRIPTION_TYPE, SUBSCRIPTION_FILTER, SUBSCRIPTION_CALLBACK, SUBSCRIPTION_ID);
  ...
  public void testInsertThrowsException() throws Exception {
    doThrow(PersistenceException.class).when(mockPersistentStore).add(anyString(), any(Map.class));
    persistor.insert(metadata);
  }
  ...
  public void testDeleteThrowsException() throws Exception {
    doThrow(PersistenceException.class).when(mockPersistentStore).delete(anyString(), anyString());
    persistor.delete(SUBSCRIPTION_ID);
  }
  ...
  public void testGetThrowsException() throws Exception {
    doThrow(PersistenceException.class).when(mockPersistentStore).get(anyString());
  ...
  private void verifyIfMetadataIsCorrect(SubscriptionMetadata resultMetadata) {
    ...
    assertThat("Metadata IDs should match. ", metadata.getId().equals(resultMetadata.getId()));
    assertThat(
        "Metadata types should match. ",
        metadata.getTypeName().equals(resultMetadata.getTypeName()));
    assertThat(
        "Metadata callbacks should match. ",
        metadata.getCallbackAddress().equals(resultMetadata.getCallbackAddress()));
    assertThat(
        "Metadata filters should match. ", metadata.getFilter().equals(resultMetadata.getFilter()));
  }
  ...
  private static class SingleItemPersistentStore implements PersistentStore {
    ...
    public void add(String type, Map<String, Object> properties) throws PersistenceException {
      assertThat("Version should be present", properties.containsKey("version_txt"), is(true));
      data = properties;
    }
