 ...
 * set of keys does not matter. The backing store is the single source of truth. And the tests
 * reflect this.
 *
...
public class SubscriptionCacheLoaderTest {
  ...
  @Mock private SubscriptionPersistor mockPersistor;

  @Mock private SubscriptionMetadata mockMetadata;
  ...
  private void validateLoadAllResults(Map<String, CachedSubscription> map) {
    validateSubscriptionMetadata(
        map.get("id1").getMetadata(), "t1", "f1", asValidUrl("cb1"), "id1");
    validateSubscriptionMetadata(
        map.get("id2").getMetadata(), "t2", "f2", asValidUrl("cb2"), "id2");
    validateSubscriptionMetadata(
        map.get("id3").getMetadata(), "t3", "f3", asValidUrl("cb3"), "id3");
  }

  private void validateSubscriptionMetadata(
      SubscriptionMetadata metadata, String type, String filter, String callback, String id) {
    notNull(metadata);
