...
public class DeleteCommand extends SubscriptionsCommand
{
    ...
    static final String NO_SUBSCRIBERS_FOUND_MSG = "Unable to delete any subscriptions because no Subscriber services found";
    ...
    @Option(name = "filter", required = false, aliases = { "-f" }, multiValued = false, 
        description = "Allows user to specify any type of LDAP filter rather than deleting on single subscription ID.\n" +
    ...
    protected Object doExecute() throws Exception {
        ...
        {
            ServiceReference[] serviceReferences = bundleContext.getServiceReferences(SUBSCRIBER_SERVICE_PID, null);
            ...
            {
                LOGGER.debug( "Found " + serviceReferences.length + " service references for " + SUBSCRIBER_SERVICE_PID);
                ...
                for (String subscriptionId : subscriptionIds)
                {
                    for (ServiceReference ref : serviceReferences)
                    {
                        ...
                        // NOTE: Not using the status returned by deleteSubscription() here because it indicates
                        // whether the attempt to delete the subscription was successful (i.e., no exceptions
                        // encountered while trying to unregister Subscription service or delete DurableSubscription).
                        // The status does not reflect if the Subscription was actually successfully deleted. Hence the
                        // need to call the getSubscriptions() method using this subscriptionId to verify the
                        // subscription has been actually deleted.
                        // (Example: If 3 Subscribers existed and one subscription was being deleted, only one of the
                        // Subscribers would actually delete the Subscription, but all 3 Subscribers would return a status
                        // of true indicating their attempt to delete the Subscription was successful).
                    ...
                    {
                        console.println(RED_CONSOLE_COLOR + UNABLE_TO_DELETE_MSG + subscriptionId + DEFAULT_CONSOLE_COLOR);
                    }
                }
                
                // Print summary of deletions to console
                console.println();
                console.printf("Deleted %d subscriptions out of %s subscriptions found.", deletedSubscriptionsCount, subscriptionIds.size());
                console.println();
            }
        }
