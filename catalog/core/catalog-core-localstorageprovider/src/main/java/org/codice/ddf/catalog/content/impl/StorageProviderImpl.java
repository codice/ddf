...
public class StorageProviderImpl implements StorageProvider {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(StorageProviderImpl.class);
        ...
        if (file != null) {
            ...
            if (mimeType == null || DEFAULT_MIME_TYPE.equals(mimeType)) {
                try {
                    ...
                    LOGGER.debug("Unable to determine mime type using Java Files service.", e);
                    mimeType = DEFAULT_MIME_TYPE;
                }
            }
            ...
            try {
                ...
                LOGGER.warn("Unable to retrieve size of file.", e);
            }
    ...
    public DeleteStorageResponse delete(DeleteStorageRequest deleteRequest)
        ...
        for (Metacard metacard : deleteRequest.getMetacards()) {
            ...
            if (Files.exists(contentIdDir)) {
                ContentItem deletedContentItem = new ContentItemImpl(metacard.getId(), null, "", "",
                        0, metacard);
        ...
        if (Files.exists(requestIdDir)) {
            ...
            try {
                ...
                throw new StorageException(
                        "Unable to retrieve contents of temporary content storage for id: " + id,
                        e);
            }
            try {
                for (Path contentIdDir : contentIdDirs) {
                    ...
                    try {
                        ...
                        try {
                            if (Files.deleteIfExists(target)) {
                                LOGGER.debug(
                                        "Remove existing content id directory for commit: " + id);
                            }
                            ...
                            throw new StorageException(
                                    "Unable to commit changes for request: " + id);
                        }
                    }
                }
            } finally {
            ...
            try {
                for (Metacard metacard : itemsToBeDeleted) {
                    ...
                    if (!Files.exists(contentIdDir)) {
                        throw new StorageException(
                                "File doesn't exist for id: " + metacard.getId());
                    }
                    ...
                    try {
                        ...
                        throw new StorageException("Could not delete file: " + metacard.getId());
                    }
                }
            } finally {
                rollback(id);
    ...
    private ContentItem generateContentFile(ContentItem item, Path contentDirectory)
        ...
        ContentItemImpl contentItem = new ContentItemImpl(item.getId(), null, item.getMimeType()
    ...
    public void setBaseContentDirectory(final String baseDirectory) throws IOException {
        ...
        if (!baseDirectory.isEmpty()) {
            ...
            try {
                ...
                path = System.getProperty("karaf.home");
                directory = Paths.get(path, DEFAULT_CONTENT_REPOSITORY, DEFAULT_CONTENT_STORE);
            }
        } else {
