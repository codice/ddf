...
class SourcePollerSpec extends Specification {
    ...
    @Unroll
    def 'test bind #expectedSourceStatus Source'() {
        given:
        final SourcePoller sourcePoller = new SourcePoller()
        ...
        then:
        assertSourceStatus(sourcePoller, mockSource, expectedSourceStatus)
    ...
    def 'test bind Source availability timeout'() {
        given:
        final SourcePoller sourcePoller = new SourcePoller()
    ...
    private static boolean assertSourceStatus(final SourcePoller sourcePoller, final Source source, final SourceStatus expectedSourceStatus) {
        await("source status is " + expectedSourceStatus).atMost(5, TimeUnit.SECONDS).until {
