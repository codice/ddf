...
public class ServiceFactoryImpl<T> implements ServiceFactory<T> {
    private SortedSet<ServiceReference<T>> serviceSet = new ConcurrentSkipListSet<ServiceReference<T>>(new ServiceComparator());
    private T service;
    ...
    public ServiceFactoryImpl(Class bundleClass) {
        ...
        if (bundleClass == null) {
            throw new IllegalArgumentException("ServiceFactoryImpl(): constructor argument 'bundleClass' may not be null.");
        }
