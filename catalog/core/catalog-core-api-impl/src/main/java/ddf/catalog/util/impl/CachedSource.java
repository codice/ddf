...
public class CachedSource implements Source {
  ...
  /**
   * Returns the known SourceStatus of this Cached Source. The value represents the cached status of
   * the last check for the source. It can either be available, unavailable, or never been tested
   * (unchecked).
   *
  ...
  /**
   * Updates the cached Source with the current status of the wrapped source. The wrapped source is
   * tested to be available, and if it is available, the contentTypes will also be cached.
   */
  public void checkStatus() {
    try {
      ...
      if (source.isAvailable()) {
        LOGGER.debug(
            "Source [{}] with id [{}] is available.  Updating cached values.",
            source,
            source.getId());
      ...
      LOGGER.debug(
          "Failed to check Source [{}] with id [{}]] availability.  " + "Clearing cached values.",
          source,
          source.getId());
      setSourceStatus(SourceStatus.UNAVAILABLE);
      clearContentTypes();
