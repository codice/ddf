...
public class TemporalResultComparator implements Comparator<Result> {
   ...
   * descending.
   *
   * @param sortOrder the temporal sort order, if null is passed in, then the default SortOrder
   *     applies, which is SortOrder.DESCENDING.
   */
   ...
   * @param sortOrder the temporal sort order, if null is passed in, then the default SortOrder
   *     applies, which is SortOrder.DESCENDING.
   * @param temporalAttribute the name of the attribute to sort on. Default is Metacard.EFFECTIVE
   */
  public TemporalResultComparator(SortOrder sortOrder, String temporalAttribute) {
    ...
    LOGGER.debug(
        "Comparing on Temporal Attribute: '{}' with Sort Order: '{}'",
