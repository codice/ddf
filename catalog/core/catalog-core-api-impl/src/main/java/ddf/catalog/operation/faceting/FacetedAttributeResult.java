...
public class FacetedAttributeResult {
    ...
    /**
     * Instantiates a FacetedAttributeResult representing a portion of the results of a faceted
     * query. A FacetedAttributeResult is representative of a single attribute's faceting results,
     * and zero to many FacetedAttributeResults may make up a complete faceted query result.
     ...
     */
    public FacetedAttributeResult(String attributeName, List<String> attributeValues, List<Long> valueCounts) {
        ...
        if (attributeValues.size() != valueCounts.size()) {
            LOGGER.debug("Creating result with unmatched field values or counts. Values: {}, Counts: {}",
                    attributeValues.size(), valueCounts.size());
        }
