...
@Command(
  ...
  name = "transformers",
  description = "Provides information on available transformers."
)
public class TransformersCommand extends CatalogCommands {
  ...
  protected Object executeWithSubject() throws Exception {
    ...
    List<ServiceReference<InputTransformer>> serviceReferences =
        (List<ServiceReference<InputTransformer>>)
            bundleContext.getServiceReferences(InputTransformer.class, "(id=*)");

    List<TransformerProperties> transformersProperties =
        serviceReferences.stream().map(TransformerProperties::new).collect(Collectors.toList());
