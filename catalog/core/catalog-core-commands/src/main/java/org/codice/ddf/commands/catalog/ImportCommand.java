...
import ddf.catalog.util.impl.CatalogRuntimeException;
...
public class ImportCommand extends CatalogCommands {
    ...
    protected Object executeWithSubject() throws Exception {
        ...
        InputTransformer transformer = getServiceByFilter(InputTransformer.class,
                String.format("(%s=%s)",
                        "id",
                        DEFAULT_TRANSFORMER_ID)).orElseThrow(() -> new CatalogRuntimeException(
        ...
        if (unsafe) {
            ...
            if (!zipValidator.validateZipFile(importFile)) {
                throw new CatalogRuntimeException("Signature on zip file is not valid");
            }
        }
    ...
    private File initImportFile(String importFile) {
        ...
        if (!file.exists()) {
            throw new CatalogRuntimeException("File does not exist: " + importFile);
        }

        if (!FilenameUtils.isExtension(importFile, "zip")) {
            throw new CatalogRuntimeException("File must be a zip file: " + importFile);
        }
