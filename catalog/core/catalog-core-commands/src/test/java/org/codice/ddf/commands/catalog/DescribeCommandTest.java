...
public class DescribeCommandTest {
    ...
    public void setUp() throws Exception {
        ...
        doReturn(TEST_CATALOG_PROVIDER_TITLE).when(catalogProvider)
                .getTitle();
        doReturn(TEST_CATALOG_PROVIDER_DESCRIPTION).when(catalogProvider)
                .getDescription();
        doReturn(TEST_CATALOG_PROVIDER_ID).when(catalogProvider)
                .getId();
        doReturn(TEST_CATALOG_PROVIDER_VERSION).when(catalogProvider)
                .getVersion();
        describeCommand.catalogProvider = catalogProvider;

        CatalogFramework catalogFramework = mock(CatalogFramework.class);
        doReturn(TEST_CATALOG_FRAMEWORK_TITLE).when(catalogFramework)
                .getTitle();
        doReturn(TEST_CATALOG_FRAMEWORK_DESCRIPTION).when(catalogFramework)
                .getDescription();
        doReturn(TEST_CATALOG_FRAMEWORK_ID).when(catalogFramework)
                .getId();
        doReturn(TEST_CATALOG_FRAMEWORK_VERSION).when(catalogFramework)
                .getVersion();
    ...
    public void testDescribeProvider() throws Exception {
        ...
        assertThat(consoleOutput.getOutput(),
                containsString("title=" + TEST_CATALOG_PROVIDER_TITLE));
        assertThat(consoleOutput.getOutput(),
    ...
    public void testDescribeFramework() throws Exception {
        ...
        describeCommand.executeWithSubject();
        assertThat(consoleOutput.getOutput(),
                containsString("title=" + TEST_CATALOG_FRAMEWORK_TITLE));
        assertThat(consoleOutput.getOutput(),
