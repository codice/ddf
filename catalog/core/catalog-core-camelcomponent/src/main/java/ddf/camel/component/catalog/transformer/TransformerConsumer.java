...
public class TransformerConsumer extends DefaultConsumer {
  ...
  /**
   * Constructs the consumer for the custom Camel CatalogComponent. This {@link Consumer} would map
   * to a Camel <from> route node with a URI like <code>catalog:queryresponsetransformer</code>
   *
   * @param endpoint the Camel endpoint that created this consumer
   * @param processor
   */
  public TransformerConsumer(
      Class<?> transformerClass, CatalogEndpoint endpoint, Processor processor) {
  ...
  protected <T> T transform(
      Class<T> objClass, Object upstreamResponse, Map<String, Serializable> arguments)
