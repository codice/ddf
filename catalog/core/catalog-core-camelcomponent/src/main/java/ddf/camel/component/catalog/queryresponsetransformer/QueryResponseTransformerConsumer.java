...
public class QueryResponseTransformerConsumer extends TransformerConsumer
  ...
  private static final transient Logger LOGGER =
      LoggerFactory.getLogger(QueryResponseTransformerConsumer.class);

  /**
   * Constructs the consumer for the custom Camel CatalogComponent. This {@link
   * org.apache.camel.Consumer} would map to a Camel <from> route node with a URI like <code>
   * catalog:queryresponsetransformer</code>
   *
  ...
  public QueryResponseTransformerConsumer(CatalogEndpoint endpoint, Processor processor) {
    ...
    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug(
          "INSIDE QueryResponseTransformerConsumer constructor for " + endpoint.getTransformerId());
    }
  }
   ...
   * (non-Javadoc)
   *
   * @see ddf.catalog.transform.QueryResponseTransformer#transform(SourceResponse
   * upstreamResponse, Map<String, Serializable> arguments)
   */
  @Override
