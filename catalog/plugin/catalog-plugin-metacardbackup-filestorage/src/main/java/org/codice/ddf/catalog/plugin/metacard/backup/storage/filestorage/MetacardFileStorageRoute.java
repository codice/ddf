...
/**
 * Creates a camel route for storing metacards from post-ingest on the local file system.
 * This route will transform the metacard using the configured metacard transformer prior
 * to storage.
 */
public class MetacardFileStorageRoute extends MetacardStorageRoute {
    ...
    public void configure() throws Exception {
        String metacardRouteId = "metacard-" + UUID.randomUUID()
                .toString();
        from("catalog:postingest").split(method(ResponseMetacardActionSplitter.class,
                "split(${body})"))
                .to("direct:" + metacardRouteId);
        from("direct:" + metacardRouteId).setHeader(METACARD_TRANSFORMER_ID_RTE_PROP,
                simple(metacardTransformerId, String.class))
                .setHeader(METACARD_BACKUP_INVALID_RTE_PROP,
                        simple(String.valueOf(backupInvalidMetacards), Boolean.class))
                .setHeader(METACARD_BACKUP_KEEP_DELETED_RTE_PROP,
