...
public class QueryMonitorPluginImpl implements QueryMonitorPlugin {
    ...
    public QueryResponse process(QueryResponse input)
        ...
        if (!removeActiveSearch((UUID) input.getRequest()
            ...
            QueryResponseImpl queryResponse = new QueryResponseImpl(input.getRequest(),
                    new ArrayList<>(),
                    0);
            queryResponse.closeResultQueue();
            Set<ProcessingDetails> processingDetails = Collections.singleton(new ProcessingDetailsImpl(QueryMonitorPlugin.class.getCanonicalName(),
