...
public class TestQueryMonitor {
    ...
    public void setUp() {
        ...
        doReturn(activeSearchMap).when(queryMonitorPlugin)
                .getActiveSearches();
    ...
    public void testActiveSearchesEmptyAndNullMap() {
        ...
        doReturn(null).when(queryMonitorPlugin)
                .getActiveSearches();
    ...
    private ActiveSearch generateActiveSearch(String name, String uuid) {
        ...
        doReturn(SOURCE).when(source)
                .getId();
        Date date = new Date();
        ActiveSearch activeSearch = mock(ActiveSearch.class);
        doReturn(source).when(activeSearch)
                .getSource();
        doReturn(name).when(activeSearch)
                .getClientInfo();
        doReturn(CQL).when(activeSearch)
                .getCQL();
        doReturn(date).when(activeSearch)
                .getStartTime();
        doReturn(UUID.fromString(uuid)).when(activeSearch)
                .getUniqueID();
        return activeSearch;
    }
