...
/**
 * Handler for incoming FTP requests from the client. Supports the PUT, MPUT,
 * DELE, MKD, RETR, RMD, APPE, RNTO, STOU, and SITE operations. When PUTing a new
 * file, it is not temporarily written to the file system. Instead, the new
 * file's data stream is ingested directly into the catalog. If PUT submits
 * a file that begins with a dot (eg. .foo), then the data is not immediately
 * stored into the catalog. Instead, the system waits for a RNTO command for
 * the dot-file and then stores the data in the catalog. Clients may PUT multiple
 * dot-files and then execute multiple RNTO commands. Any dot-files that are not
 * renamed at the end of the ftp session will be discarded and resources will
 * be released. The CWD command always return a successful response. This is
 * to support clients that use a MKDIR/CWD/PUT command sequence.
 */
public class FtpRequestHandler extends DefaultFtplet {
    ...
    /**
     * Adds the filename and {@link TemporaryFileBackedOutputStream} to the map. If the object in the ftp session
     * is not a map or is null, then a new map is added to the session. Returns
     * the TFBOS that passed in.
     */
    private TemporaryFileBackedOutputStream addTempFileToSession(FtpSession ftpSession,
            String filename, TemporaryFileBackedOutputStream temporaryFileBackedOutputStream) {

        Map<String, TemporaryFileBackedOutputStream> map =
                findTempFileMap(ftpSession).orElseGet(() -> {
    ...
    private Optional<TemporaryFileBackedOutputStream> findTempFileInSession(FtpSession ftpSession,
            String filename) {
        return findTempFileMap(ftpSession).filter(m -> m.containsKey(filename))
    ...
    private void removeTempFileFromSession(FtpSession ftpSession, String filename) {
        findTempFileMap(ftpSession).ifPresent(m -> {
            Optional.ofNullable(m.get(filename))
    ...
    private void removeAllTempFilesFromSession(FtpSession ftpSession) {
        findTempFileMap(ftpSession).map(Map::keySet)
                .ifPresent(filenames -> filenames.forEach(filename -> removeTempFileFromSession(
                        ftpSession,
                        filename)));
            ...
            if (createResponse != null) {
                if (LOGGER.isDebugEnabled()) {
                    createResponse.getCreatedMetacards()
                            .forEach(metacard -> LOGGER.debug("Content item created with id = {}",
                                    metacard.getId()));
                }
    ...
    public FtpletResult onSite(FtpSession session, FtpRequest request)
            throws FtpException, IOException {
        session.write(new DefaultFtpReply(FtpReply.REPLY_202_COMMAND_NOT_IMPLEMENTED,
