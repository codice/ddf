...
function calculateAvailableAttributesFromActive(selectionInterface) {
    return selectionInterface.getActiveSearchResults().reduce(function (currentAvailable, result) {
        currentAvailable = _.union(currentAvailable, Object.keys(result.get('metacard').get('properties').toJSON()));
        return currentAvailable;
    }, []);
}
...
function calculateSummaryAttributes() {
    ...
    var propertiesArray = userPropertyArray.length > 0 ? userPropertyArray : properties.summaryShow;
...
module.exports = Marionette.LayoutView.extend({
    ...
    initialize: function (options) {
        ...
        this.listenTo(this.options.selectionInterface, 'reset:activeSearchResults add:activeSearchResults', this.onBeforeShow);
        this.listenTo(user.get('user').get('preferences'), 'change:inspector-summaryShown', this.handleSummaryShown);
    ...
    handleSave: function () {
        var prefs = user.get('user').get('preferences');
        prefs.set('inspector-summaryShown', this.attributeSelector.currentView.model.get('value')[0]);
        prefs.savePreferences();
    }
});