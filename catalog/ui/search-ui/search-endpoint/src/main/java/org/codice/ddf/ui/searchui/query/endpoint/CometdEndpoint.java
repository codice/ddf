...
public class CometdEndpoint extends CometDServlet {
  ...
  public CometdEndpoint(
    ...
    this.searchController =
        new SearchController(framework, actionRegistry, filterAdapter, executorService);
    ...
    if (bayeuxServer != null) {
      ...
      bayeuxServer.setSecurityPolicy(
          new DefaultSecurityPolicy() {
            ...
            @Override
            public boolean canHandshake(
                BayeuxServer server, ServerSession session, ServerMessage message) {

              if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(
                    "canHandshake ServerSession: {}   canHandshake ServerMessage: {}",
                    session,
                    message);
              }
