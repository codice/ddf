...
public class NotificationController extends AbstractEventController {
   ...
   * notifications published on the {@link Notification#NOTIFICATION_TOPIC_ROOT} topic from the OSGi
   ...
   *     <p>
   *     <ul>
   *       <li>{@link Notification#NOTIFICATION_KEY_APPLICATION}
   *       <li>{@link Notification#NOTIFICATION_KEY_MESSAGE}
   *       <li>{@link Notification#NOTIFICATION_KEY_TIMESTAMP}{@link
   *           Notification#NOTIFICATION_KEY_TITLE}
   *       <li>{@link Notification#NOTIFICATION_KEY_USER_ID}
   *     </ul>
   */
  @Override
  public void handleEvent(Event event) throws IllegalArgumentException {
    ...
    if (null == event.getProperty(Notification.NOTIFICATION_KEY_APPLICATION)
      ...
      String message =
          String.format(
              "Event [%s] property is null or empty", Notification.NOTIFICATION_KEY_APPLICATION);
    ...
    if (null == event.getProperty(Notification.NOTIFICATION_KEY_MESSAGE)
      ...
      String message =
          String.format(
              "Event [%s] property is null or empty", Notification.NOTIFICATION_KEY_MESSAGE);
      throw new IllegalArgumentException(message);
    }

    if (null == event.getProperty(Notification.NOTIFICATION_KEY_TIMESTAMP)) {
      String message =
          String.format("Event [%s] property is null", Notification.NOTIFICATION_KEY_TIMESTAMP);
      throw new IllegalArgumentException(message);
    }
    ...
    if (null == event.getProperty(Notification.NOTIFICATION_KEY_TITLE)
      ...
      String message =
          String.format(
              "Event [%s] property is null or empty", Notification.NOTIFICATION_KEY_TITLE);
      throw new IllegalArgumentException(message);
    }
    ...
    if (null != recipient) {
      ...
      LOGGER.debug(
          "Session with ID [{}] is not connected to the server. " + "Ignoring notification",
          sessionId);
    }
  }
    ...
    if (MapUtils.isEmpty(data)) {
      ...
      if (CollectionUtils.isNotEmpty(notifications)) {
        queuePersistedMessages(
            remote, notifications, "/" + Notification.NOTIFICATION_TOPIC_BROADCAST);
      }
    } else {
  ...
  public void deletePersistentNotification(
      ServerSession serverSession, ServerMessage serverMessage) {
