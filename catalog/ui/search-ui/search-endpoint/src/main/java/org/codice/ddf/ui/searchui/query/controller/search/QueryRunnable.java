...
public abstract class QueryRunnable implements Runnable {
  ...
  protected void addResults(Collection<Result> responseResults) {
    results.putAll(
        Maps.uniqueIndex(
            responseResults, result -> getResultKey(result != null ? result.getMetacard() : null)));
  }
  ...
  private void collectResponses(QueryRequest request, QueryResponse response)
    ...
    int adjustedStartIndex = request.getQuery().getStartIndex();
    while (resultSize != 0 && resultSize < request.getQuery().getPageSize()) {
      ...
      QueryResponse singleResponse = searchController.getFramework().query(requestCopy);
      response.getResults().addAll(singleResponse.getResults());
      response.getProcessingDetails().addAll(singleResponse.getProcessingDetails());
      response.getProcessingErrors().addAll(singleResponse.getProcessingErrors());
      resultSize = singleResponse.getResults().size();
    }
  }
