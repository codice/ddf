...
public abstract class QueryRunnable implements Runnable {
    ...
    protected static final Map<String, Serializable> CACHE_PROPERTIES = Collections
    ...
    @SuppressWarnings("serial")
    protected static final Map<String, Serializable> UPDATE_PROPERTIES = Collections
            .unmodifiableMap(new HashMap<String, Serializable>() {
                {
                    put("mode", "update");
                }
    ...
    protected String extractQueryWkt(Filter filter, FilterAdapter filterAdapter) {
        String wkt = "";
        try {
            wkt = filterAdapter.adapt(filter, new WktExtractionFilterDelegate());
    ...
    protected void normalizeDistances(String searchWkt, Map<String, Result> results) {
        if (StringUtils.isNotBlank(searchWkt)) {
            for (Map.Entry<String, Result> entry : results.entrySet()) {
                Result result = entry.getValue();
                if (result.getMetacard() != null && StringUtils
                        .isNotBlank(result.getMetacard().getLocation())) {
                    try {
                        Shape query = SPATIAL_CONTEXT.readShapeFromWkt(searchWkt);
                        Shape location = SPATIAL_CONTEXT
                                .readShapeFromWkt(result.getMetacard().getLocation());

                        double distance = DistanceUtils.degrees2Dist(SPATIAL_CONTEXT
                                        .calcDistance(location.getCenter(), query.getCenter()),
                        ...
                        LOGGER.debug("Unable to parse WKT to calculate distance", e);
                    }
                }
            }
        }
