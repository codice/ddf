...
public class SourceQueryRunnable extends QueryRunnable {
  ...
  private void cleanup() {
    if (shouldNormalizeRelevance && search.isFinished()) {
      try {
        solrIndexFuture.get(FUTURE_TIMEOUT_SECONDS, TimeUnit.SECONDS).shutdown();
  ...
  private void sort(List<Result> responseResults) {
    List<Result> sortedResults =
        Ordering.from(sortComparator).immutableSortedCopy(results.values());

    responseResults.clear();
    responseResults.addAll(
        sortedResults.size() > maxResults ? sortedResults.subList(0, maxResults) : sortedResults);
  }
