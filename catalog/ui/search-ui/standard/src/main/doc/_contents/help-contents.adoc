...
UI is turned off by default; however, it can be turn on through the Admin UI.
...
==== Turning on Search UI

The Search UI is turned off by default, but can be turned on through the Admin UI.

. Go to *Admin UI*
. Go to *System* tab
. Go to *Feature* tab
. Install feature: *search-ui*
