...
module.exports = Marionette.LayoutView.extend({
    ...
    onBeforeShow: function() {
        this.listToday.show(new NotificationGroupView({
        ...
        this.listPreviousOne.show(new NotificationGroupView({
            filter: function(model) {
        ...
        this.listPreviousTwo.show(new NotificationGroupView({
            filter: function(model) {
                return moment().diff(model.get('sentAt'), 'days') === 2;
            },
            date: moment().subtract(2, 'days').format('dddd')
        }), {
            replaceElement: true
        });
        this.listPreviousThree.show(new NotificationGroupView({
            filter: function(model) {
                return moment().diff(model.get('sentAt'), 'days') === 3;
            },
            date: moment().subtract(3, 'days').format('dddd')
        }), {
            replaceElement: true
        });
        this.listPreviousFour.show(new NotificationGroupView({
            filter: function(model) {
                return moment().diff(model.get('sentAt'), 'days') === 4;
            },
            date: moment().subtract(4, 'days').format('dddd')
        }), {
            replaceElement: true
        });
        this.listPreviousFive.show(new NotificationGroupView({
            filter: function(model) {
                return moment().diff(model.get('sentAt'), 'days') === 5;
            },
            date: moment().subtract(5, 'days').format('dddd')
        }), {
            replaceElement: true
        });
        this.listPreviousSix.show(new NotificationGroupView({
            filter: function(model) {
                return moment().diff(model.get('sentAt'), 'days') === 6;
            },
            date: moment().subtract(6, 'days').format('dddd')
        }), {
            replaceElement: true
        });
        this.listOlder.show(new NotificationGroupView({
            filter: function(model) {
