...
function checkValidation(model) {
    if (model.get('id')) {
        ...
        setTimeout(function() {
            $.whenAll.apply(this, [
                $.get('/intrigue/internal/metacard/' + model.get('id') + '/attribute/validation').then(function(response) {
                    model.set({
                        issues: model.get('issues') || response.length > 0
                    });
                }),
                $.get('/intrigue/internal/metacard/' + model.get('id') + '/validation').then(function(response) {
                    model.set({
                        issues: model.get('issues') || response.length > 0
                    });
                })
