...
public class Cataloger {
     ...
     */
    public String createMetacard(ddf.content.operation.CreateResponse input, Metacard metacard)
        ...
        if (metacard != null) {
            logger.debug("Creating catalog CreateRequest with metacard  (ID = " + metacard.getId()
                    + ")");
            final CreateRequestImpl catalogCreateRequest = new CreateRequestImpl(
                    Collections.singletonList(metacard), input.getRequest()
                    .getProperties());
            try {
    ...
    public String updateMetacard(ddf.content.operation.UpdateResponse input, String productUri,
            Metacard metacard) throws PluginExecutionException {
        ...
        if (metacard != null) {
            ...
            final UpdateRequestImpl catalogUpdateRequest = new UpdateRequestImpl(productUris,
                    metacards);
            catalogUpdateRequest.getProperties()
                    .putAll(input.getRequest()
                            .getProperties());
            ...
            try {
                ...
                if (updatedMetacards != null && updatedMetacards.size() == 1) {
                    ...
                    updatedCatalogId = updatedMetacard.getNewMetacard()
                            .getId();
    ...
    // never
    // provided as an input from the client (via ContentRestEndpoint)
    public String deleteMetacard(ddf.content.operation.DeleteResponse input)
            throws PluginExecutionException {
        String productUri = input.getContentItem()
                .getUri();
        ...
        if (productUri != null) {
            ...
            final DeleteRequestImpl catalogDeleteRequest = new DeleteRequestImpl(new URI[] {uri},
                    input.getRequest()
                            .getProperties());
