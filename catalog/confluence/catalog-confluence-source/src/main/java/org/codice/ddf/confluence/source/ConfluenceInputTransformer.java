...
public class ConfluenceInputTransformer implements InputTransformer {
    ...
    private Metacard transformConfluenceResult(Object json, String baseUrl, String id)
        ...
        parseBody(metacard,
                json,
                metacard.getAttribute(Confluence.TYPE)
                        .getValue()
                        .toString());
    ...
    private void parseBasicInfo(MetacardImpl metacard, Object json, String id)
        ...
        metacard.setAttribute(Confluence.TYPE, confluenceType);

        metacard.setTitle(getString(json, "title"));

        metacard.setAttribute(Confluence.JSON_RESPONSE, json.toString());

        metacard.setAttribute(Confluence.SPACE, getStringOrDefault(json, null, "space", "key"));
    }

    private void parseLabels(MetacardImpl metacard, Object json)
            throws CatalogTransformerException {
    ...
    private void parseHistory(MetacardImpl metacard, Object json)
            throws CatalogTransformerException {
    ...
    private void parseLinks(MetacardImpl metacard, Object json, String baseUrl)
        ...
        metacard.setAttribute(Confluence.BASE_URL, baseUrl);

        Object links = getRequiredJsonElement(json, "_links");

        metacard.setAttribute(Confluence.LINK,
                String.format("%s%s", baseUrl, getString(links, "webui")));

        String downloadUrl = getStringOrDefault(links, null, "download");
        if (downloadUrl != null) {
            metacard.setAttribute(Metacard.RESOURCE_DOWNLOAD_URL,
                    String.format("%s%s", baseUrl, downloadUrl));
            metacard.setAttribute(Metacard.RESOURCE_URI,
                    String.format("%s%s", baseUrl, downloadUrl));
    ...
    private void parseBody(MetacardImpl metacard, Object json, String confluenceType) {
        ...
        if (body != null) {
            String cleanedText = body.toString()
                    .replaceAll("<.*?>", " ");
            ...
            if (StringUtils.isNotEmpty(description)) {
                ...
                metacard.setAttribute(Metacard.METADATA,
                        String.format(PRODUCT_XML, confluenceType, xmlSafeString));
    ...
    private void parseRestrictions(MetacardImpl metacard, Object json)
        ...
        if (restrictions != null) {
            ...
            metacard.setAttribute(Confluence.RESTRICTION_USERS, userRestrictions);
            metacard.setAttribute(Confluence.RESTRICTION_GROUPS, groupRestrictions);
        }
    }

    private Map<String, Object> getJsonObject(InputStream stream)
            throws CatalogTransformerException {
        ...
        try {
            ...
            throw new CatalogTransformerException("Invalid json. Could not parse: " + jsonString,
                    re);
        }
    }
    ...
    private List getJsonArray(Object object, String... keys) throws CatalogTransformerException {
        ...
        if (!(jsonArray instanceof List)) {
            throw new CatalogTransformerException(String.format("Given element is not an array: %s",
                    String.join("/", keys)));
        }
    ...
    private Object getRequiredJsonElement(Object object, String... keys)
        ...
        if (json == null) {
            throw new CatalogTransformerException(String.format(
                    "Could not retrieve required value from json: %s",
                    String.join("/", keys)));
        }
        return json;
    }
