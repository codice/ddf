...
public class ConfluenceFilterDelegate extends SimpleFilterDelegate<String> {
    ...
    public String propertyIsEqualTo(String propertyName, String literal, boolean isCaseSensitive) {
        return getConfluenceParameter(propertyName,
                literal,
                param -> param.getEqualExpression(literal));
    }

    @Override
    public String propertyIsLike(String propertyName, String literal, boolean isCaseSensitive) {
        return getConfluenceParameter(propertyName, literal, param -> Arrays.stream(literal.split(
                " "))
    ...
    public String after(String propertyName, Date date) {
        return getConfluenceParameter(propertyName,
                null,
                param -> param.getGreaterThanExpression(new SimpleDateFormat(DATE_FORMAT).format(
                        date)));
    }
    ...
    public String during(String propertyName, Date startDate, Date endDate) {
        return getConfluenceParameter(propertyName,
                null,
                param -> param.getGreaterThanExpression(new SimpleDateFormat(DATE_FORMAT).format(
                        startDate)) + " AND " + param.getLessThanExpression(new SimpleDateFormat(
                        DATE_FORMAT).format(endDate)));
    }

    private String getConfluenceParameter(String propertyName, String literal,
            Function<ConfluenceQueryParameter, String> function) {
