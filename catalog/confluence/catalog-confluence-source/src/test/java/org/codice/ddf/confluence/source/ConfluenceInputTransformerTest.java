...
public class ConfluenceInputTransformerTest {
    ...
    @Before
    public void setup() {
        transformer = new ConfluenceInputTransformer(new MetacardTypeImpl("confluence",
                (List) null));
    }
    ...
    public void testAttachmentTransform() throws Exception {
        ...
        assertThat(mcard.getAttribute(Associations.EXTERNAL)
                .getValues()
                .get(0)
                .toString(), equalTo("/spaces/DDF"));
        ...
        assertThat(mcard.getAttribute(Metacard.RESOURCE_URI)
                        .getValue()
                        .toString(),
    ...
    private void validatePageMetacard(Metacard mcard, String expectedId, String baseUrl,
        ...
        List<Serializable> associations = mcard.getAttribute(Associations.EXTERNAL)
                .getValues();
        assertThat(associations.get(associations.size() - 1)
                .toString(), equalTo(url + "/spaces/DDF"));
        ...
        if (checkRestrictions) {
            assertThat(mcard.getAttribute(Security.ACCESS_INDIVIDUALS)
                    .getValues()
                    .contains("first.last"), is(true));
            assertThat(mcard.getAttribute(Security.ACCESS_GROUPS)
                    .getValues()
