...
public class ConfluenceSourceTest {
    ...
    public void setup() {
        ...
        confluence = new TestConfluenceSource(adapter,
                encryptionService,
                transformer,
                reader,
                factory);
    ...
    public void testQuery() throws Exception {
        QueryRequest request = new QueryRequestImpl(new QueryImpl(builder.attribute("anyText")
                .is()
                .like()
                .text("searchValue"),
                1,
                ...
                1000));
        InputStream entity =
                new ByteArrayInputStream(JSON_RESPONSE.getBytes(StandardCharsets.UTF_8));
    ...
    public void testQueryNullSort() throws Exception {
        QueryRequest request = new QueryRequestImpl(new QueryImpl(builder.attribute("anyText")
                .is()
                .like()
                .text("searchValue"), 1, 1, null, false, 1000));
        InputStream entity =
                new ByteArrayInputStream(JSON_RESPONSE.getBytes(StandardCharsets.UTF_8));
        when(clientResponse.getEntity()).thenReturn(entity);
        when(clientResponse.getStatus()).thenReturn(Response.Status.OK.getStatusCode());
    ...
    public void testFailedQuery() throws Exception {
        QueryRequest request = new QueryRequestImpl(new QueryImpl(builder.attribute("anyText")
                ...
                false,
                1000));
        InputStream entity = new ByteArrayInputStream("Something bad happened".getBytes(
    ...
    public void testQueryWithSpace() throws Exception {
        ...
        SourceResponse response = confluence.query(request);
        assertThat(response.getHits(), is(1L));
        Metacard mcard = response.getResults()
                .get(0)
                .getMetacard();
        assertThat(mcard, notNullValue());
        assertThat(mcard.getAttribute("attrib1")
                .getValue(), is("val1"));
        assertThat(mcard.getAttribute("attrib2")
                .getValues()
                .size(), is(3));
    }
    ...
    public void testNonConfluenceQuery() throws Exception {
        QueryRequest request = new QueryRequestImpl(new QueryImpl(builder.attribute("metacard-tags")
                .is()
                .like()
                .text("nonConfluecneTag"),
                1,
                1,
                new SortByImpl("title", SortOrder.DESCENDING),
                false,
    ...
    public void testQueryNonConfluenceAttribute() throws Exception {
        QueryRequest request = new QueryRequestImpl(new QueryImpl(builder.attribute("someAttribute")
                .is()
                .like()
                .text("someValue"), 1, 1, null, false, 1000));
    ...
    private static String getFileContent(String filePath) {
        try {
            return IOUtils.toString(ConfluenceSourceTest.class.getClassLoader()
                    .getResourceAsStream(filePath), StandardCharsets.UTF_8.toString());
        } catch (IOException e) {
