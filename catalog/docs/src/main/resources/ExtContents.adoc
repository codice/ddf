...
The {branding} Catalog makes extensive use of the Whiteboard Design Pattern.
...
in terms of Extensible Markup Language (XML) and key-value pairs (KVP). The {branding} Catalog Framework does not use the XML representation of
...
To abstract developers from the complexities of working with the Filter interface directly and implementing the {branding} Profile of the Filter specification, the {branding} Catalog includes an API, primarily in `ddf.filter`, to build Filters using a fluent API.
...
The `commons-ddf`bundle, located in `<DDF_HOME_SOURCE_DIRECTORY>/common/commons-ddf`, provides utilities and functionality commonly used across other {branding} components, such as the endpoints and providers. 
...
`ddf.util.XPathHelper` provides convenience methods for executing XPath operations on XML.
...
This example is located in the {branding} trunk at `sdk/sample-plugins/ddf/sdk/plugin/presubscription`.
...
The Standard Catalog Framework provides the reference implementation of a Catalog Framework that implements all requirements of the {branding} Catalog API. 
...
|event.topics
|`ddf/catalog/event/CREATED`, `ddf/catalog/event/UPDATED`, d`df/catalog/event/DELETED`
...
{branding} and the underlying OSGi technology can serve as a robust infrastructure for developing frameworks that complement the {branding} Catalog.

Recommendations for Framework Development

. Provide extensibility similar to that of the {branding} Catalog.
...
. Make use of the {branding} Catalog wherever possible to store, search, and transform information.
...
{branding} Catalog instances can also be federated to each other.
Therefore, a {branding} Catalog can also act as a federated source to another {branding} Catalog.
...
These settings are used to provide default security settings for the Title, Description, and Security elements in a record.
The purpose of these defaults is that many providers fail to deliver a classification and Owner/Producer with the metadata returned.
These default settings are used if a metadata record is returned without security settings. *This feature can be enabled/disabled*.
...
* After the file is replaced, either restart {branding} or the restart the CDR Open Search Service to reload the property values.
====
...
[NOTE]
====
A code example of a Federated Source delivered with {branding} can be found in `ddf.catalog.source.solr`
====
...
Import the {branding} Catalog API Filter package and the reference implementation package of the Filter Adapter in the bundle manifest  (in addition to any other required packages). +
...
The Tika Input Transformer's main purpose is to ingest these types of content into the {branding} Content Repository and the Metadata Catalog.
...
====== Using
Use the Tika Input Transformer for ingesting Microsoft documents, OpenOffice documents, or PDF documents into the {branding} Content Repository and/or the Metadata Catalog.
...
[NOTE]
====
An example of using an Apache Camel route to define an `InputTransformer` in a blueprint file and deploying it as a bundle to an OSGi container can be found in the {branding} SDK examples at `ddf/sdk/sample-transformers/xslt-identity-input-transformer`
====
...
. Add `service-rss-transformer/src/main/resources/OSGI-INF/ddf/xslt-response-queue-transformer/rss.xsl`.
...
Events are generated by the {branding} Catalog Framework as metacards are created/updated/deleted and the Standard Event Processor is called since it is also a Post-Ingest Plugin.
...
To create a subscription in {branding} the developer needs to implement the `ddf.catalog.event.Subscription` interface. 
...
The following is an implementation stub of `Subscription` that comes with {branding} and is available at `ddf.catalog.event.impl.SubscriptionImpl`.
...
Refer to the {branding} Catalog API Javadoc for more information about the methods required for implementing the interface. 
====
