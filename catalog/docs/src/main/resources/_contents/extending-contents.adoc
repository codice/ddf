...
[TIP]
====
Refer to the Javadoc for more information on all Requests and Responses in the `${ddf-branding}.catalog.operation` and `${ddf-branding}.catalog.event` packages.
====
...
.. These settings can also be changed by editing the file `<INSTALL_DIRECTORY>/etc/${ddf-branding}/${ddf-branding}.DefaultSiteSecurity.cfg`
