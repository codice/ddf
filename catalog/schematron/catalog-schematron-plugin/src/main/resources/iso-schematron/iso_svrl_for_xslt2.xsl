...
<xsl:template name="process-assert">
	...
	<svrl:failed-assert test="{$test}" >
		...
		<!-- BAH BEGIN: modified since our flags map to nodes in an xml file instead of containing plain text -->
			<axsl:attribute name="flag">
				<xsl:choose>
				<!-- For the few cases we do have that are plain text, output them without changing anything -->
					<xsl:when test="not(starts-with($flag,'$'))">
						<xsl:value-of select="$flag" />
					</xsl:when>
				<!-- For the node mapping cases, create a nested value-of call to extract the data -->
					<xsl:otherwise>
		...
		<!-- BAH END: modified since our flags map to nodes in an xml file instead of containing plain text -->
