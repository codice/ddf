...
public class TestJarSigner {
  ...
  public void setUp() throws IOException {
    File zipFile =
        new File(TestJarSigner.class.getClassLoader().getResource("unsigned.zip").getPath());
    unsignedZipFile =
        new File(zipFile.getParentFile().getAbsolutePath() + File.separator + "newZip.zip");

    FileUtils.copyFile(zipFile, unsignedZipFile);
    keyStorePath = TestJarSigner.class.getClassLoader().getResource("serverKeystore.jks").getPath();
    zipValidator = new ZipValidator();

    zipValidator.setSignaturePropertiesPath(
        TestJarSigner.class.getResource("/signature.properties").getPath());
    zipValidator.init();
