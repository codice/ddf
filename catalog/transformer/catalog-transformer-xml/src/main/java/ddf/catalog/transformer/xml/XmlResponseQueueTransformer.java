...
public class XmlResponseQueueTransformer extends AbstractXmlTransformer
            ...
            if (resultList.size() < threshold) {
                ...
                try {
                    for (Result result : resultList) {
                        ...
                        StringWriter tmpWriter = this.metacardMarshaller.marshall(metacard);
                        sw.append(tmpWriter.getBuffer());
                    }
    ...
    public BinaryContent transform(SourceResponse response, Map<String, Serializable> args)
        ...
        try {
            ...
            PrintWriter writer = this.printWriterProvider.build(stringWriter, Metacard.class);
            ...
            if (response.getResults() != null && !response.getResults().isEmpty()) {
                StringWriter metacardContent = fjp
                        .invoke(new MetacardForkTask(ImmutableList.copyOf(response.getResults()),
                                fjp, geometryTransformer, threshold, this.metacardMarshaller));
            ...
            return new BinaryContentImpl(bais, this.mimeType);
        } catch (Exception e) {
