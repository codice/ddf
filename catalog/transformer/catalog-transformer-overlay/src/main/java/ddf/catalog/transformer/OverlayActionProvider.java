...
public class OverlayActionProvider implements ActionProvider {
    ...
    private static final Pattern OVERLAY_TRANSFORMER_ID_PATTERN = Pattern.compile(
            "^" + Pattern.quote(OVERLAY_PREFIX) + "\\w+");
    ...
    public OverlayActionProvider(Predicate<Metacard> hasOverlay, String transformerId) {
        ...
        Validate.isTrue(OVERLAY_TRANSFORMER_ID_PATTERN.matcher(transformerId)
                        .matches(),
                "The transformer ID must match the pattern "
                        + OVERLAY_TRANSFORMER_ID_PATTERN.pattern());
